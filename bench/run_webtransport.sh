...
Environment knobs:
  ...
  RUN_JS_NATIVE              Include native JS/trevrpc-c WebTransport server. Default: 1
...
initialize_output() {
    ...
    : >"$COMMAND_LOG"
    rm -f "$RAW_DIR"/*.txt
    printf 'run,browser,server,shape,latency_us,throughput_ops_s,iterations,elapsed_s,source\n' >"$SAMPLES_CSV"
...
append_webtransport_csv() {
    ...
    awk -v run="$run" -v browser="$browser" -v server="$server" -v source="$source" -F '' '
        match($0, /^([^:]+):[[:space:]]+([0-9.]+) ops\/s \(([0-9]+) iterations in ([0-9.]+)s\)/, m) {
            ...
            printf "%s,%s,%s,%s,%.3f,%.3f,%s,%s,%s\n", run, browser, server, m[1], latency_us, ops, m[3], m[4], source
        }
...
aggregate_samples_csv() {
    awk -F, '
        ...
        BEGIN {
            print "browser,server,shape,measurements,latency_us_median,latency_us_min,latency_us_max,throughput_ops_s,iterations_per_measurement,elapsed_s_total,source"
        }
        NR == 1 {
            next
        }
        ...
        END {
            for (i = 1; i <= order_count; i++) {
                ...
                printf "%s,%s,%s,%d,%.3f,%.3f,%.3f,%.3f,%.0f,%.3f,%s\n", \
                    browser[key], server[key], shape[key], measurements[key], latency, latency_min[key], latency_max[key], throughput, median(iterations[key]), elapsed_total[key], source[key]
            }
        }
    ' "$SAMPLES_CSV" >"$CSV"
}
...
| JS native server included | \`$RUN_JS_NATIVE\` |
...
EOF
        ...
        function print_table_header(shape) {
            printf "\n### `%s`\n\n", shape
            print "| Browser client | Server | Median latency us/op | Latency min..max us/op | Median throughput ops/s | Source |"
            print "| --- | --- | ---: | ---: | ---: | --- |"
        }
        NR > 1 {
            id = NR - 1
        ...
        END {
            for (shape_index = 1; shape_index <= shape_count; shape_index++) {
                ...
                for (row_index = 1; row_index <= row_count; row_index++) {
                    ...
                    printf "| `%s` | `%s` | %.3f | %.3f..%.3f | %.0f | `%s` |\n", browser[id], server[id], latency[id], latency_min[id], latency_max[id], throughput[id], source[id]
                }
            }
        }' "$CSV"
...
Reported latency is the median across measurements. The min..max column shows the observed latency range across repeated browser measurements. Tables are split by RPC shape and sorted by median latency ascending.
...
build_prerequisites() {
    if [[ "$RUN_C" == "1" || "$RUN_GO" == "1" || "$RUN_JS_NATIVE" == "1" ]]; then
...
run_webtransport_benchmarks() {
    ...
    if [[ "$RUN_JS_NATIVE" == "1" ]]; then
        start_server webtransport-js-native-server env "TREVRPC_WEBTRANSPORT_ORIGIN=$STATIC_ORIGIN" node trevrpc-js/bench/rpc_split_native.js server "$cert_file" "$key_file"
        port=$START_SERVER_PORT
        run_browser_against_server js-native js_native_webtransport "$port" "$cert_file"
        stop_rpc_servers
    fi
}
