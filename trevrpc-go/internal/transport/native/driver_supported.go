...
func NewEngine(provider trevrpcc.Provider, config EngineConfig) (*Engine, error) {
	if provider == nil {
		return nil, errors.New("native Engine requires a provider")
	}
	runtime, err := provider.NewEngine(config)
	if err != nil {
		return nil, err
	}
	adapted, err := newProviderRuntime(runtime)
	if err != nil {
		return nil, err
	}
	return newDriver(adapted, config, "Engine", provider.Metadata().Name)
}

func NewTransport(provider trevrpcc.Provider, config EngineConfig) (*Engine, error) {
	if provider == nil {
		return nil, errors.New("native Transport requires a provider")
	}
	runtime, err := provider.NewTransport(trevrpcc.TransportConfig(config))
	if err != nil {
		return nil, err
	}
	adapted, err := newProviderRuntime(runtime)
	...
	return newDriver(adapted, config, "Transport", provider.Metadata().Name)
}

func newDriver(runtime driverRuntime, config EngineConfig, runtimeName, providerName string) (*Engine, error) {
...
func reapRuntime(
	...
	for attempt := range runtimeReaperMaxAttempts {
