...
func generateClientMethod(buffer *bytes.Buffer, serviceName, fullServiceName string, method methodInfo) {
	...
	case method.clientStreaming && method.serverStreaming:
		...
		fmt.Fprintf(buffer, "\treturn trevrpc.BidirectionalStreaming[%s, %s](ctx, c.transport, %q, %q, requests, func() %s { return &%s{} }, mergeTrevrpcCallOptions(c.options, options)...)\n", method.inputType, method.outputType, fullServiceName, method.protoName, method.outputType, strings.TrimPrefix(method.outputType, "*"))
	case method.clientStreaming:
		fmt.Fprintf(buffer, "func (c *%sClient) %s(ctx context.Context, requests trevrpc.MessageStream[%s], options ...trevrpc.CallOption) (%s, error) {\n", serviceName, method.name, method.inputType, method.outputType)
		fmt.Fprintf(buffer, "\treturn trevrpc.ClientStreaming[%s, %s](ctx, c.transport, %q, %q, requests, func() %s { return &%s{} }, mergeTrevrpcCallOptions(c.options, options)...)\n", method.inputType, method.outputType, fullServiceName, method.protoName, method.outputType, strings.TrimPrefix(method.outputType, "*"))
