...
static void
require_status(const char *operation, QUIC_STATUS actual, QUIC_STATUS expected)
{
    if (actual != expected) {
        fprintf(stderr,
                "%s returned 0x%llx, expected 0x%llx\n",
                operation,
...
attest_loaded_provider(void)
{
    ...
    if (realpath(info.dli_fname, resolved) == NULL) {
        fprintf(stderr,
                "unable to resolve loaded MsQuic provider %s: %s\n",
                info.dli_fname,
                strerror(errno));
        exit(EXIT_FAILURE);
    }
...
static void
attest_global_descriptor(const QUIC_API_TABLE *api)
{
    struct trev_reset_stream_at_dialects descriptor = {0};
    uint32_t length = 0;
    QUIC_STATUS status = api->GetParam(
        NULL,
        TREV_QUIC_PARAM_GLOBAL_RESET_STREAM_AT_DIALECTS,
        &length,
        NULL);

#if EXPECT_DESCRIPTOR
    require_status("zero-length descriptor query",
                   status,
    ...
    status = api->GetParam(
        ...
        &length,
        NULL);
    require_status("null descriptor query",
                   status,
...
#else
    require_status("stock descriptor query",
                   status,
                   QUIC_STATUS_INVALID_PARAMETER);
#endif
    ...
    status = api->SetParam(
        ...
        sizeof(descriptor),
        &descriptor);
    require_status("global descriptor mutation",
                   status,
                   QUIC_STATUS_INVALID_PARAMETER);
}

static void
attest_configuration_mask(const QUIC_API_TABLE *api)
{
    ...
    status = api->SetParam(
        configuration,
        TREV_QUIC_PARAM_CONFIGURATION_RESET_STREAM_AT_DIALECT_MASK,
        sizeof(mask),
        &mask);
#if EXPECT_DESCRIPTOR
    ...
    status = api->SetParam(
        ...
        sizeof(mask),
        &mask);
    require_status("unknown configuration mask bit",
                   status,
                   QUIC_STATUS_INVALID_PARAMETER);
    ...
    status = api->GetParam(
        ...
        &length,
        &mask);
    require_status("configuration mask get after rejection",
                   status,
                   QUIC_STATUS_SUCCESS);
    if (length != sizeof(mask) || mask != REQUESTED_MASK) {
...
#else
    require_status("stock configuration mask set",
                   status,
                   QUIC_STATUS_INVALID_PARAMETER);
    ...
    status = api->GetParam(
        ...
        &length,
        &mask);
    require_status("stock configuration mask get",
                   status,
                   QUIC_STATUS_INVALID_PARAMETER);
#endif
