...
let
  ...
  patchCount = builtins.length (
    builtins.filter (patch: patch == msquicPatch) (libmsquic.patches or [ ])
  );
  provenance = builtins.toJSON {
    package = "libmsquic-static";
...
assert lib.assertMsg (
  patchCount == 1
) "libmsquic-static requires the MsQuic reset patch exactly once";
libmsquic.overrideAttrs (previous: {
  pname = "libmsquic-static";
  cmakeFlags = (previous.cmakeFlags or [ ]) ++ [
    # Keep compiler provenance useful without embedding Nix's ephemeral build
    # directory in the distributable archive.
  ...
  passthru = (previous.passthru or { }) // {
    ...
    trevrpcResetStreamAtPatchApplications = patchCount;
  };
