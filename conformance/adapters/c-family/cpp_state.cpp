...
void classify_error(const trevrpc::Error& native, const cf::RpcStreamHarness* harness,
                    cf_error* error) {
  switch (native.kind()) {
  case trevrpc::Error::Kind::Rpc:
    cf_error_set(error, "remote_status", static_cast<std::uint32_t>(native.status()->code()));
    return;
  ...
  const auto diagnostic =
      harness == nullptr ? TREVRPC_WIRE_DIAGNOSTIC_NONE : harness->receive_diagnostic();
  if (native.code() == CF_WIRE_ERR_FRAME_TOO_LARGE || native.code() == -EMSGSIZE) {
    cf_error_set(error, "frame_too_large", TREVRPC_RPC_STATUS_RESOURCE_EXHAUSTED);
  } else if (diagnostic == TREVRPC_WIRE_DIAGNOSTIC_UNSUPPORTED_FRAME_KIND) {
    cf_error_set(error, "unsupported_frame_kind", TREVRPC_RPC_STATUS_INVALID_ARGUMENT);
...
std::optional<std::vector<std::byte>> serialize_payload(const StatePayload& payload,
                                                        cf_error* error) {
...
start_scripted_stream(cf::RpcStreamHarness& harness, const cf_command* command,
  ...
  for (std::size_t i = 0; i < command->frame_count; ++i) {
    const int native_error =
        harness.push_frame(std::span(command->frames[i].data, command->frames[i].len));
    if (native_error != 0) {
...
int run_server(const cf_command* command, cf_json* payload, cf_error* error) {
  ...
  auto stream = start_scripted_stream(harness, command, TREVRPC_RPC_KIND_SERVER_STREAMING);
  if (!stream) {
  ...
  for (;;) {
    ...
    for (std::size_t i = 0; i < messages.size(); ++i) {
      ...
      cf_json_append_hex(payload, reinterpret_cast<const std::uint8_t*>(messages[i].data()),
                         messages[i].size());
      cf_json_append_char(payload, '}');
    }
    if (!messages.empty()) {
    ...
    cf_json_append_u64_string(payload, static_cast<std::uint32_t>(terminal.code()));
    ...
    cf_json_append_hex(payload, reinterpret_cast<const std::uint8_t*>(terminal.message().data()),
    ...
    for (std::size_t i = 0; i < entries.size(); ++i) {
      ...
      cf_json_append_hex(payload, reinterpret_cast<const std::uint8_t*>(entries[i].key.data()),
                         entries[i].key.size());
      cf_json_append(payload, ",\"value_hex\":");
      cf_json_append_hex(payload, reinterpret_cast<const std::uint8_t*>(entries[i].value.data()),
...
int run_client(const cf_command* command, cf_json* payload, cf_error* error) {
  cf::RpcStreamHarness harness;
  auto stream = start_scripted_stream(harness, command, TREVRPC_RPC_KIND_CLIENT_STREAMING);
  if (!stream) {
    classify_error(stream.error(), &harness, error);
    return -1;
  }
  ...
  for (;;) {
    ...
    cf_json_append_hex(payload, reinterpret_cast<const std::uint8_t*>(first_response->data()),
                       first_response->size());
    result = payload->failed ? -1 : 0;
    break;
  }
...
extern "C" int cf_cpp_state_dispatch(const cf_command* command, cf_json* payload, cf_error* error) {
