...
public:
  explicit ScriptedStream(std::initializer_list<std::vector<std::uint8_t>> frames,
  ...
  [[nodiscard]] int fail_receive(int error) { return harness_.fail_receive(error); }

  [[nodiscard]] std::size_t close_count() const { return harness_.close_count(); }
...
template <typename Call> void close_once(Call& call, const ScriptedStream& scripted) {
  call.close();
  call.close();
...
void test_rpc_error() {
  ...
  REQUIRE(result.error().status()->code() == trevrpc::StatusCode::InvalidArgument);
  close_once(call, scripted);
}
...
void test_terminal_trailing_precedence() {
  ...
  }
  {
    ...
    ClientCall call(scripted.take());
    auto receiving = std::async(std::launch::async, [&call] { return call.receive(); });
...
void test_remote_status_precedes_cardinality() {
  ScriptedStream scripted(
      {{0x22, 0x03, 0x1a, 0x01, 0x61}, {0x22, 0x03, 0x1a, 0x01, 0x62}, {0x08, 0x01, 0x10, 0x0e}});
  ClientCall call(scripted.take());
  ...
  REQUIRE(terminal.error().status()->code() == trevrpc::StatusCode::Unavailable);
  close_once(call, scripted);
}

void test_missing_terminal_precedes_cardinality() {
  ScriptedStream scripted({{0x22, 0x03, 0x1a, 0x01, 0x61}, {0x22, 0x03, 0x1a, 0x01, 0x62}});
  ClientCall call(scripted.take());
  REQUIRE(call.receive());
  REQUIRE(call.receive());
  auto terminal = call.receive();
  ...
  REQUIRE(terminal.error().message() == "response stream ended before terminal status");
  close_once(call, scripted);
}
...
void test_cardinality_after_clean_ok() {
  ...
  {
    ScriptedStream scripted(
        {{0x22, 0x03, 0x1a, 0x01, 0x61}, {0x22, 0x03, 0x1a, 0x01, 0x62}, {0x08, 0x01}});
    ClientCall call(scripted.take());
    REQUIRE(call.receive());
    REQUIRE(call.receive());
    auto terminal = call.receive();
