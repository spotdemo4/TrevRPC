...
struct adapter_stream {
    ...
    trevrpc_engine_reservation* send_stopped_reservation;
};
...
static int publish_reserved_event(msquic_provider* adapter,
    ...
    return adapter->host->publish_reserved(adapter->engine, reservation, &spec);
}
...
static int enqueue_event(msquic_provider* adapter, msquic_event* event, bool mandatory) {
    ...
    if (event->reservation != NULL) {
        ...
        result = adapter->host->publish_reserved(adapter->engine, reservation, &event->spec);
    } else {
        result = adapter->host->publish_event(adapter->engine, &event->spec);
    }
    return result;
}
...
static void adapter_maybe_stopped_locked(msquic_provider* adapter) {
    if (adapter->state == TREVRPC_ENGINE_STATE_STOPPING && !adapter->stopped_reported && adapter->live_listeners == 0 &&
        ...
        adapter->host->stopped(adapter->engine, adapter->terminal_status, 0);
    }
}
...
static bool callback_enter(msquic_provider* adapter, adapter_object* object) {
    if (adapter->host->callback_enter(adapter->engine) != 0) {
        return false;
    }
...
static void callback_leave(msquic_provider* adapter, adapter_object* object) {
    ...
    adapter->host->callback_leave(adapter->engine);
}
...
static adapter_stream* stream_alloc(adapter_connection* connection,
    ...
    int result = 0;
    if (owns_ready) {
        result = adapter->host->reserve_mandatory(adapter->engine, &ready_reservation);
    }
    if (result == 0 && owns_terminal) {
        result = adapter->host->reserve_mandatory(adapter->engine, &terminal_reservation);
    }
    if (result == 0) {
        result = adapter->host->reserve_mandatory(adapter->engine, &stream->receive_fin_reservation);
    }
    if (result == 0) {
        result = adapter->host->reserve_mandatory(adapter->engine, &stream->send_stopped_reservation);
    }
    if (result == 0) {
    ...
    if (result == 0) {
        ...
        adapter->host->cancel_reservation(adapter->engine, stream->send_stopped_reservation);
        stream->send_stopped_reservation = NULL;
        adapter->host->cancel_reservation(adapter->engine, stream->receive_fin_reservation);
        stream->receive_fin_reservation = NULL;
        if (owns_terminal) {
            adapter->host->cancel_reservation(adapter->engine, terminal_reservation);
        }
        if (owns_ready) {
            adapter->host->cancel_reservation(adapter->engine, ready_reservation);
        }
...
static void adapter_fail_stop(msquic_provider* adapter, int status, const char* message) {
    ...
    pthread_mutex_unlock(&adapter->mutex);
    adapter->host->fail(adapter->engine, status, 0);
}
...
static int create_owned_receive(
    msquic_provider* adapter, msquic_receive* receive, trevrpc_engine_receive** out_receive) {
#ifdef TREVRPC_ENGINE_MSQUIC_TESTING
...
#endif
    return adapter->host->receive_create_owned(receive->data,
        receive->len,
...
static int provider_stream_receive_frame(
    ...
    result = create_owned_receive(adapter, node->receive, &detached);
    if (result != 0) {
        if (!stream->readable_pending) {
            stream->readable_pending = true;
            stream->readable_epoch++;
...
static int publish_send_completion(adapter_send* send, int status) {
    ...
    int result = adapter->host->publish_reserved(adapter->engine, reservation, &spec);
    free(send->data);
    free(send);
    return result;
}
...
static void publish_connection_terminal(adapter_connection* connection) {
    ...
    adapter_native_close_complete(adapter, handle);
    adapter->host->cancel_reservation(adapter->engine, unused_ready);
    (void)publish_reserved_event(adapter,
        terminal,
...
static void publish_stream_terminal(adapter_stream* stream, bool failed, int status) {
    ...
    trevrpc_engine_reservation* unused_send_stopped;
    pthread_mutex_lock(&adapter->mutex);
    ...
    stream->receive_fin_reservation = NULL;
    unused_send_stopped = stream->send_stopped_reservation;
    stream->send_stopped_reservation = NULL;
    ...
    adapter_native_close_complete(adapter, handle);
    adapter->host->cancel_reservation(adapter->engine, unused_ready);
    adapter->host->cancel_reservation(adapter->engine, unused_receive_fin);
    adapter->host->cancel_reservation(adapter->engine, unused_send_stopped);
    (void)publish_reserved_event(adapter,
        terminal,
...
static void publish_send_stopped(adapter_stream* stream) {
    msquic_provider* adapter = stream->base.adapter;
    pthread_mutex_lock(&adapter->mutex);
    if (stream->send_stopped_published || stream->send_stopped_reservation == NULL) {
        pthread_mutex_unlock(&adapter->mutex);
        return;
    }
    stream->send_stopped_published = true;
    trevrpc_engine_reservation* reservation = stream->send_stopped_reservation;
    stream->send_stopped_reservation = NULL;
    uint64_t application_error = stream->application_error;
    uint32_t event_flags =
        stream->base.event_flags & ~(TREVRPC_ENGINE_EVENT_FLAG_LOCAL | TREVRPC_ENGINE_EVENT_FLAG_PEER);
    pthread_mutex_unlock(&adapter->mutex);

    (void)publish_reserved_event(adapter,
        reservation,
        TREVRPC_ENGINE_EVENT_SEND_STOPPED,
        event_flags | TREVRPC_ENGINE_EVENT_FLAG_TERMINAL | TREVRPC_ENGINE_EVENT_FLAG_PEER |
            TREVRPC_ENGINE_EVENT_FLAG_PEER_RESET,
        -ECANCELED,
        TREVRPC_ENGINE_OBJECT_STREAM,
        stream->base.token,
        stream->parent,
...
static void adapter_scheduler_drain(msquic_provider* adapter) {
    ...
    for (;;) {
        ...
        int pin_result = adapter->host->operation_pin(adapter->engine);
        if (pin_result != 0) {
            bool complete = false;
            pthread_mutex_lock(&stream->send_gate);
        ...
        if (QUIC_FAILED(status)) {
            ...
            adapter->host->operation_unpin(adapter->engine);
            if (stream_shutdown_drained(stream))
                publish_stream_terminal(stream, false, 0);
        }
        adapter_object_unpin(&stream->base);
...
static int adapter_adopt_accepted_connection(msquic_provider* adapter,
    ...
    connection->admission_granted = true;
    add_result = adapter->host->reserve_mandatory(adapter->engine, &connection->base.ready_reservation);
    if (add_result == 0)
        add_result = adapter->host->reserve_mandatory(adapter->engine, &connection->base.terminal_reservation);
    if (add_result == 0)
        add_result = registry_add(adapter, &connection->base, TREVRPC_ENGINE_OBJECT_CONNECTION, &token);
    if (add_result != 0) {
        adapter->host->cancel_reservation(adapter->engine, connection->base.terminal_reservation);
        adapter->host->cancel_reservation(adapter->engine, connection->base.ready_reservation);
...
static QUIC_STATUS QUIC_API adapter_stream_callback(HQUIC handle, void* context, QUIC_STREAM_EVENT* event) {
    ...
    case QUIC_STREAM_EVENT_PEER_SEND_SHUTDOWN: {
        pthread_mutex_lock(&stream->mutex);
        bool receive_aborted = stream->receive_aborted;
        pthread_mutex_unlock(&stream->mutex);
        if (!receive_aborted) {
            publish_receive_fin(stream, TREVRPC_ENGINE_EVENT_FLAG_CLEAN_FIN, 0);
        }
...
static int provider_stream_abort_receive(
    ...
    adapter_stream* stream = (adapter_stream*)object;
    pthread_mutex_lock(&stream->mutex);
    bool invoke = !stream->receive_aborted;
    pthread_mutex_unlock(&stream->mutex);
    ...
    (void)discard_stream_receives(stream, &released_receive_credit);
...
static void adapter_object_free(adapter_object* object) {
    ...
    adapter->host->cancel_reservation(adapter->engine, object->ready_reservation);
    adapter->host->cancel_reservation(adapter->engine, object->terminal_reservation);
    if (object->kind == TREVRPC_ENGINE_OBJECT_STREAM) {
        adapter_stream* stream = (adapter_stream*)object;
        if (stream->base.handle != NULL) {
        ...
        while (stream->pending_sends != NULL) {
            ...
            adapter->host->cancel_reservation(adapter->engine, send->completion_reservation);
            free(send->data);
            free(send);
        }
        adapter->host->cancel_reservation(adapter->engine, stream->receive_fin_reservation);
...
int trevrpc_engine_msquic_adopt_accepted_connection_with_host_v1(const trevrpc_engine_provider_host_v1* host,
    ...
    if (host == NULL || host->struct_size < sizeof(*host) ||
        host->struct_version != TREVRPC_ENGINE_PROVIDER_STRUCT_VERSION_1 || engine == NULL || config == NULL ||
        accepted == NULL || out_connection == NULL)
        return -EINVAL;
    adapter = host->provider_context(engine);
    if (adapter == NULL || adapter->host != host)
        return -EINVAL;
    result = host->callback_enter(engine);
    if (result != 0)
        return result;
...
int trevrpc_engine_msquic_adopt_accepted_connection_v1(trevrpc_engine* engine,
    ...
    trevrpc_msquic_accepted_connection* accepted,
    trevrpc_engine_handle_v1* out_connection) {
    return trevrpc_engine_msquic_adopt_accepted_connection_with_host_v1(
        trevrpc_engine_provider_host_v1_get(), engine, config, accepted, out_connection);
}
#endif
...
int trevrpc_engine_msquic_provider_descriptor_create_v1(const trevrpc_engine_provider_host_v1* host,
    ...
    out_descriptor->struct_size = sizeof(*out_descriptor);
    out_descriptor->struct_version = TREVRPC_ENGINE_PROVIDER_STRUCT_VERSION_1;
    result = validate_provider_config(provider_config);
    if (result != 0) {
        return result;
    }
    ...
    pthread_mutex_unlock(&adapter->mutex);
    out_descriptor->operations = &MsQuicProviderOps;
    out_descriptor->context = adapter;
    out_descriptor->owner_cookie = adapter->owner;
    return 0;
}
...
int trevrpc_engine_msquic_create_v1(const trevrpc_engine_config_v1* engine_config,
    ...
    *out_engine = NULL;
    result = trevrpc_engine_msquic_provider_descriptor_create_v1(
        trevrpc_engine_provider_host_v1_get(), engine_config, provider_config, &descriptor);
    if (result != 0) {
        return result;
    }
    return trevrpc_engine_provider_adopt_v1(engine_config, &descriptor, out_engine);
}
#endif
