...
static int h3_event_get_info(trevrpc_rpc_transport* transport,
...
static int h3_event_get_admission_info(trevrpc_rpc_transport* transport,
    const trevrpc_rpc_transport_event* event,
...
static int h3_event_get_protocol_info(trevrpc_rpc_transport* transport,
    const trevrpc_rpc_transport_event* event,
...
static int h3_admission_respond(
    trevrpc_rpc_transport* transport, const trevrpc_rpc_transport_event* event_const, uint16_t status) {
    (void)transport;
...
static void h3_event_release(trevrpc_rpc_transport* transport, trevrpc_rpc_transport_event* event) {
    if (event == NULL)
        return;
    if (!event->admission_decided && (event->info.kind == TREVRPC_RPC_TRANSPORT_EVENT_HTTP3_ADMISSION ||
                                         event->info.kind == TREVRPC_RPC_TRANSPORT_EVENT_WEBTRANSPORT_ADMISSION))
        (void)h3_admission_respond(transport, event, 500);
    h3_event_node_free(&event);
}
static int h3_receive_get_info(trevrpc_rpc_transport* transport,
...
static void h3_receive_release(trevrpc_rpc_transport* transport, trevrpc_rpc_transport_receive* receive) {
    (void)transport;
    free(receive);
}
