...
#include "trevrpc_engine_provider_host_validate.h"
...
typedef struct trevrpc_rpc_transport_engine {
    ...
    const trevrpc_engine_provider_host_v1* engine_host;
...
static const trevrpc_engine_provider_runtime_v1* trevrpc_rpc_transport_engine_runtime(
    trevrpc_rpc_transport* transport) {
    return trevrpc_rpc_transport_engine_from_base(transport)->engine_host->runtime;
}

static trevrpc_engine* trevrpc_rpc_transport_engine_instance(trevrpc_rpc_transport* transport) {
    return trevrpc_rpc_transport_engine_from_base(transport)->engine;
}
...
static int trevrpc_rpc_transport_engine_endpoint_config(const trevrpc_engine_provider_host_v1* host,
    const trevrpc_rpc_transport_endpoint_config* source,
    ...
    if (result != 0)
        return result;
    (void)host->runtime->endpoint_config_init(destination, sizeof(*destination));
...
int trevrpc_rpc_transport_engine_adopt_accepted_connection(trevrpc_rpc_transport* transport,
    ...
    if (result != 0)
        return result;
    result = trevrpc_rpc_transport_engine_endpoint_config(adapter->engine_host, config, &source, &credential_files, 0);
    if (result != 0) {
        trevrpc_credential_cleanup_owner_finish(&adapter->credential_cleanup, &credential_files, &cleanup_lease);
        return result;
    }
    result = trevrpc_engine_msquic_adopt_accepted_connection_with_host_v1(
        adapter->engine_host, adapter->engine, &source, accepted, &target);
    trevrpc_credential_cleanup_owner_finish(&adapter->credential_cleanup, &credential_files, &cleanup_lease);
    if (result == 0)
...
static int trevrpc_rpc_transport_engine_get_wake_source(
    ...
    int result = trevrpc_rpc_transport_engine_runtime(transport)->wake_source_init(&source, sizeof(source));
    if (result != 0) {
        return result;
    }
    result = trevrpc_rpc_transport_engine_runtime(transport)->get_wake_source(
        trevrpc_rpc_transport_engine_instance(transport), &source);
    if (result == 0) {
...
static int trevrpc_rpc_transport_engine_next_event(
    ...
    int result = trevrpc_rpc_transport_engine_runtime(transport)->next_event(
        trevrpc_rpc_transport_engine_instance(transport), &event);
    if (result == 0) {
...
static int trevrpc_rpc_transport_engine_event_get_info(trevrpc_rpc_transport* transport,
    const trevrpc_rpc_transport_event* event,
    trevrpc_rpc_transport_event_info* info) {
    trevrpc_engine_event_info_v1 source;
    int result = trevrpc_rpc_transport_engine_runtime(transport)->event_info_init(&source, sizeof(source));
    if (result != 0) {
        return result;
    }
    result =
        trevrpc_rpc_transport_engine_runtime(transport)->event_get_info((const trevrpc_engine_event*)event, &source);
    if (result != 0) {
        return result;
    }
...
static int trevrpc_rpc_transport_engine_event_get_protocol_info(trevrpc_rpc_transport* transport,
    const trevrpc_rpc_transport_event* event,
    ...
    if (event == NULL || info == NULL)
        return -EINVAL;
    result = trevrpc_rpc_transport_engine_runtime(transport)->event_info_init(&source, sizeof(source));
    if (result != 0)
        return result;
    result =
        trevrpc_rpc_transport_engine_runtime(transport)->event_get_info((const trevrpc_engine_event*)event, &source);
...
static void trevrpc_rpc_transport_engine_event_release(
    trevrpc_rpc_transport* transport, trevrpc_rpc_transport_event* event) {
    trevrpc_rpc_transport_engine_runtime(transport)->event_release((trevrpc_engine_event*)event);
}

static int trevrpc_rpc_transport_engine_receive_get_info(trevrpc_rpc_transport* transport,
    const trevrpc_rpc_transport_receive* receive,
    trevrpc_rpc_transport_receive_info* info) {
    trevrpc_engine_receive_info_v1 source;
    int result = trevrpc_rpc_transport_engine_runtime(transport)->receive_info_init(&source, sizeof(source));
    if (result != 0) {
        return result;
    }
    result = trevrpc_rpc_transport_engine_runtime(transport)->receive_get_info(
        (const trevrpc_engine_receive*)receive, &source);
    if (result == 0) {
        info->flags = source.flags;
        info->data = source.data;
        info->data_len = source.data_len;
...
static void trevrpc_rpc_transport_engine_receive_release(
    trevrpc_rpc_transport* transport, trevrpc_rpc_transport_receive* receive) {
    trevrpc_rpc_transport_engine_runtime(transport)->receive_release((trevrpc_engine_receive*)receive);
}
...
static int trevrpc_rpc_transport_engine_get_diagnostics(
    ...
    int result = trevrpc_rpc_transport_engine_runtime(transport)->diagnostics_init(&source, sizeof(source));
    if (result != 0) {
        return result;
    }
    result = trevrpc_rpc_transport_engine_runtime(transport)->get_diagnostics(
        trevrpc_rpc_transport_engine_instance(transport), &source);
    if (result != 0) {
        return result;
    }
...
static int trevrpc_rpc_transport_engine_endpoint_listen(trevrpc_rpc_transport* transport,
    ...
    if (result != 0)
        return result;
    result = trevrpc_rpc_transport_engine_endpoint_config(adapter->engine_host, config, &source, &credential_files, 1);
    if (result != 0) {
        trevrpc_credential_cleanup_owner_finish(&adapter->credential_cleanup, &credential_files, &cleanup_lease);
        return result;
    }
    result = trevrpc_rpc_transport_engine_runtime(transport)->listen(
        trevrpc_rpc_transport_engine_instance(transport), &source, &target);
    trevrpc_credential_cleanup_owner_finish(&adapter->credential_cleanup, &credential_files, &cleanup_lease);
    if (result == 0) {
...
static int trevrpc_rpc_transport_engine_endpoint_get_port(
    ...
    return trevrpc_rpc_transport_engine_runtime(transport)->listener_get_port(
        trevrpc_rpc_transport_engine_instance(transport), trevrpc_rpc_transport_engine_handle(listener), port);
}
...
static int trevrpc_rpc_transport_engine_endpoint_dial(trevrpc_rpc_transport* transport,
    ...
    result = trevrpc_rpc_transport_engine_runtime(transport)->dial(
        trevrpc_rpc_transport_engine_instance(transport), &source, operation_id, &target);
    trevrpc_credential_cleanup_owner_finish(&adapter->credential_cleanup, &credential_files, &cleanup_lease);
    if (result == 0) {
...
static int trevrpc_rpc_transport_engine_dial_cancel(
    ...
    return trevrpc_rpc_transport_engine_runtime(transport)->dial_cancel(
        trevrpc_rpc_transport_engine_instance(transport), trevrpc_rpc_transport_engine_handle(connection));
}
...
static int trevrpc_rpc_transport_engine_stream_open(trevrpc_rpc_transport* transport,
    ...
    trevrpc_engine_handle_v1 target;
    int result = trevrpc_rpc_transport_engine_runtime(transport)->connection_open_bidi_stream(
        trevrpc_rpc_transport_engine_instance(transport),
...
static int trevrpc_rpc_transport_engine_stream_send(trevrpc_rpc_transport* transport,
    ...
    const uint8_t* body,
    size_t body_len) {
    return trevrpc_rpc_transport_engine_runtime(transport)->stream_send_frame(
        trevrpc_rpc_transport_engine_instance(transport),
...
static int trevrpc_rpc_transport_engine_stream_receive(trevrpc_rpc_transport* transport,
    ...
    int result = trevrpc_rpc_transport_engine_runtime(transport)->stream_receive_frame(
        trevrpc_rpc_transport_engine_instance(transport), trevrpc_rpc_transport_engine_handle(stream), &receive);
    if (result == 0) {
...
static int trevrpc_rpc_transport_engine_stream_finish_send(
    ...
    return trevrpc_rpc_transport_engine_runtime(transport)->stream_finish_send(
        trevrpc_rpc_transport_engine_instance(transport), trevrpc_rpc_transport_engine_handle(stream));
}
...
static int trevrpc_rpc_transport_engine_stream_abort_receive(
    ...
    return trevrpc_rpc_transport_engine_runtime(transport)->stream_abort_receive(
        trevrpc_rpc_transport_engine_instance(transport), trevrpc_rpc_transport_engine_handle(stream), error_code);
}

static int trevrpc_rpc_transport_engine_stream_abort_send(
    trevrpc_rpc_transport* transport, trevrpc_rpc_transport_handle stream, uint64_t error_code) {
    return trevrpc_rpc_transport_engine_runtime(transport)->stream_abort_send(
        trevrpc_rpc_transport_engine_instance(transport), trevrpc_rpc_transport_engine_handle(stream), error_code);
}

static int trevrpc_rpc_transport_engine_stream_abort(
    trevrpc_rpc_transport* transport, trevrpc_rpc_transport_handle stream, uint64_t error_code) {
    return trevrpc_rpc_transport_engine_runtime(transport)->stream_abort(
        trevrpc_rpc_transport_engine_instance(transport), trevrpc_rpc_transport_engine_handle(stream), error_code);
}

static int trevrpc_rpc_transport_engine_stream_close(
    trevrpc_rpc_transport* transport, trevrpc_rpc_transport_handle stream) {
    return trevrpc_rpc_transport_engine_runtime(transport)->stream_close(
        trevrpc_rpc_transport_engine_instance(transport), trevrpc_rpc_transport_engine_handle(stream));
}
...
static int trevrpc_rpc_transport_engine_connection_close(
    ...
    return trevrpc_rpc_transport_engine_runtime(transport)->connection_close(
        trevrpc_rpc_transport_engine_instance(transport), trevrpc_rpc_transport_engine_handle(connection), error_code);
}
...
static int trevrpc_rpc_transport_engine_listener_close(
    ...
    return trevrpc_rpc_transport_engine_runtime(transport)->listener_close(
        trevrpc_rpc_transport_engine_instance(transport), trevrpc_rpc_transport_engine_handle(listener));
}

static int trevrpc_rpc_transport_engine_close(trevrpc_rpc_transport* transport) {
    return trevrpc_rpc_transport_engine_runtime(transport)->close(trevrpc_rpc_transport_engine_instance(transport));
}

static int trevrpc_rpc_transport_engine_drain(trevrpc_rpc_transport* transport) {
    trevrpc_rpc_transport_engine* adapter = trevrpc_rpc_transport_engine_from_base(transport);
    int result = trevrpc_rpc_transport_engine_runtime(transport)->drain(adapter->engine);
...
static void trevrpc_rpc_transport_engine_destroy(trevrpc_rpc_transport* transport) {
    trevrpc_rpc_transport_engine* adapter = trevrpc_rpc_transport_engine_from_base(transport);
    (void)trevrpc_rpc_transport_engine_runtime(transport)->release(adapter->engine);
...
int trevrpc_rpc_transport_engine_adopt_with_host(
    const trevrpc_engine_provider_host_v1* host, trevrpc_engine* engine, trevrpc_rpc_transport** out_transport) {
    ...
    *out_transport = NULL;
    result = trevrpc_engine_provider_host_validate_v1(host);
    if (result != 0) {
        return result;
    }
...
int trevrpc_rpc_transport_engine_adopt(trevrpc_engine* engine, trevrpc_rpc_transport** out_transport) {
    return trevrpc_rpc_transport_engine_adopt_with_host(trevrpc_engine_provider_host_v1_get(), engine, out_transport);
}
#endif
