...
static int trevrpc_transport_msquic_create_rpc_private_with_host(const trevrpc_engine_provider_host_v1* host,
    ...
    trevrpc_engine_provider_descriptor_v1 engine_descriptor;
    ...
    *out_transport = NULL;
    ...
    result = host->runtime->config_init(&engine_config, sizeof(engine_config));
    if (result != 0) {
        return result;
    }
    ...
    result = trevrpc_engine_msquic_provider_descriptor_create_v1(
        host, &engine_config, &engine_provider_config, &engine_descriptor);
    if (result != 0) {
        return result;
    }
    result = host->runtime->adopt(&engine_config, &engine_descriptor, &engine);
    if (result != 0) {
        return result;
    }
    result = trevrpc_rpc_transport_engine_adopt_with_host(host, engine, &native_transport);
    if (result != 0) {
        (void)host->runtime->release(engine);
        return result;
    }
...
int trevrpc_transport_msquic_provider_descriptor_create_v1(const trevrpc_engine_provider_host_v1* host,
    ...
    trevrpc_rpc_transport* provider = NULL;
    int result;
    if (out_descriptor == NULL) {
        return -EINVAL;
    }
    memset(out_descriptor, 0, sizeof(*out_descriptor));
    result = trevrpc_transport_msquic_create_rpc_private_with_host(host, transport_config, provider_config, &provider);
    if (result != 0) {
        return result;
    }
    result = trevrpc_rpc_transport_provider_descriptor_create_v1(provider, out_descriptor);
    if (result != 0) {
        trevrpc_rpc_transport_destroy(provider);
    }
    return result;
}
...
int trevrpc_transport_msquic_create_rpc_private(const trevrpc_transport_config_v1* transport_config,
    const trevrpc_transport_msquic_config_v1* provider_config,
    trevrpc_rpc_transport** out_transport) {
    return trevrpc_transport_msquic_create_rpc_private_with_host(
        trevrpc_engine_provider_host_v1_get(), transport_config, provider_config, out_transport);
}
...
int trevrpc_transport_msquic_create_v1(const trevrpc_transport_config_v1* transport_config,
    ...
    *out_transport = NULL;
    result = trevrpc_transport_msquic_provider_descriptor_create_v1(
        trevrpc_engine_provider_host_v1_get(), transport_config, provider_config, &descriptor);
    if (result != 0) {
        return result;
    }
