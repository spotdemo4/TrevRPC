...
struct CxPlatEvent {
    CXPLAT_EVENT Handle;
    CxPlatEvent() noexcept {
        CxPlatEventInitialize(&Handle, FALSE, FALSE);
    }
    CxPlatEvent(bool ManualReset) noexcept {
        CxPlatEventInitialize(&Handle, ManualReset, FALSE);
    }
    CxPlatEvent(CXPLAT_EVENT event) noexcept : Handle(event) {
    }
    ~CxPlatEvent() noexcept {
        CxPlatEventUninitialize(Handle);
    }
    ...
    CXPLAT_EVENT* operator&() noexcept {
        return &Handle;
    }
    operator CXPLAT_EVENT() const noexcept {
        return Handle;
    }
    void Set() {
        CxPlatEventSet(Handle);
    }
    void Reset() {
        CxPlatEventReset(Handle);
    }
    void WaitForever() {
        CxPlatEventWaitForever(Handle);
    }
    bool WaitTimeout(uint32_t TimeoutMs) {
        return CxPlatEventWaitWithTimeout(Handle, TimeoutMs);
    }
};

struct CxPlatRundown {
    CXPLAT_RUNDOWN_REF Ref;
    CxPlatRundown() noexcept {
        CxPlatRundownInitialize(&Ref);
    }
    ~CxPlatRundown() noexcept {
        CxPlatRundownUninitialize(&Ref);
    }
    ...
    bool Acquire() noexcept {
        return CxPlatRundownAcquire(&Ref);
    }
    void Release() noexcept {
        CxPlatRundownRelease(&Ref);
    }
    void ReleaseAndWait() {
        CxPlatRundownReleaseAndWait(&Ref);
    }
};
...
struct CxPlatLock {
    CXPLAT_LOCK Handle;
    CxPlatLock() noexcept {
        CxPlatLockInitialize(&Handle);
    }
    ~CxPlatLock() noexcept {
        CxPlatLockUninitialize(&Handle);
    }
    ...
    void Acquire() noexcept {
        CxPlatLockAcquire(&Handle);
    }
    void Release() noexcept {
        CxPlatLockRelease(&Handle);
    }
};
...
struct CxPlatLockDispatch {
    ...
    CxPlatLockDispatch() noexcept {
        CxPlatDispatchLockInitialize(&Handle);
    }
    ~CxPlatLockDispatch() noexcept {
        CxPlatDispatchLockUninitialize(&Handle);
    }
    ...
    void Acquire() noexcept {
        CxPlatDispatchLockAcquire(&Handle);
    }
    void Release() noexcept {
        CxPlatDispatchLockRelease(&Handle);
    }
};
#pragma warning(pop)
...
struct CxPlatPool {
    CXPLAT_POOL Handle;
    CxPlatPool(uint32_t Size, uint32_t Tag = 0, bool IsPaged = false) noexcept {
        CxPlatPoolInitialize(IsPaged, Size, Tag, &Handle);
    }
    ~CxPlatPool() noexcept {
        CxPlatPoolUninitialize(&Handle);
    }
...
template <class _Ty> struct CxPlatRemoveReference {
    using type = _Ty;
    using _Const_thru_ref_type = const _Ty;
};

template <class _Ty> using CxPlatRemoveReferenceT = typename CxPlatRemoveReference<_Ty>::type;
...
#endif
template <typename T, uint32_t Tag = 'lPxC', bool Paged = false> class CxPlatPoolT {
    CXPLAT_POOL Pool;

  public:
    CxPlatPoolT() noexcept {
        CxPlatPoolInitialize(Paged, sizeof(T), Tag, &Pool);
    }
    ~CxPlatPoolT() noexcept {
    ...
    template <class... Args> T* Alloc(Args&&... args) noexcept {
...
struct CxPlatHashTable {
    ...
    CxPlatHashTable() noexcept {
        Initialized = CxPlatHashtableInitializeEx(&Table, CXPLAT_HASH_MIN_SIZE);
    }
    ~CxPlatHashTable() noexcept {
        if (Initialized) {
    ...
    void Insert(CXPLAT_HASHTABLE_ENTRY* Entry) noexcept {
        CxPlatHashtableInsert(&Table, Entry, Entry->Signature, nullptr);
    }
    void Remove(CXPLAT_HASHTABLE_ENTRY* Entry) noexcept {
        CxPlatHashtableRemove(&Table, Entry, nullptr);
    }
    ...
    CXPLAT_HASHTABLE_ENTRY* LookupEx(
        uint64_t Signature, bool (*Equals)(CXPLAT_HASHTABLE_ENTRY* Entry, void* Context), void* Context) noexcept {
        CXPLAT_HASHTABLE_LOOKUP_CONTEXT LookupContext;
        CXPLAT_HASHTABLE_ENTRY* Entry = CxPlatHashtableLookup(&Table, Signature, &LookupContext);
        while (Entry != NULL) {
            if (Equals(Entry, Context))
                return Entry;
...
class CxPlatWatchdog {
  ...
  public:
    CxPlatWatchdog(
        uint32_t WatchdogTimeoutMs, const char* Name = "cxplat_watchdog", bool WriteToConsole = false) noexcept
...
struct QuicAddr {
    ...
    QUIC_ADDRESS_FAMILY GetFamily() const {
        return QuicAddrGetFamily(&SockAddr);
    }
    uint16_t GetPort() const {
        return QuicAddrGetPort(&SockAddr);
    }
    void SetPort(uint16_t Port) noexcept {
        QuicAddrSetPort(&SockAddr, Port);
    }
    operator const QUIC_ADDR*() const noexcept {
        return &SockAddr;
    }
};
...
template <class T> class UniquePtr {
  public:
    ...
    bool operator==(T* _ptr) const noexcept {
        return ptr == _ptr;
    }
    bool operator!=(T* _ptr) const noexcept {
        return ptr != _ptr;
...
template <class T> class UniquePtrArray {
    T* ptr;
...
class MsQuicApi : public QUIC_API_TABLE {
  ...
  public:
    MsQuicApi(MsQuicOpenVersionFn _OpenFn = MsQuicOpenVersion, MsQuicCloseFn _CloseFn = MsQuicClose) noexcept
        : CloseFn(_CloseFn) {
    ...
    QUIC_STATUS GetInitStatus() const noexcept {
        return InitStatus;
    }
    bool IsValid() const noexcept {
        return QUIC_SUCCEEDED(InitStatus);
    }
};
...
struct MsQuicExecution {
    ...
    uint32_t Count{0};
    MsQuicExecution(QUIC_EVENTQ* EventQ,
        QUIC_GLOBAL_EXECUTION_CONFIG_FLAGS Flags = QUIC_GLOBAL_EXECUTION_CONFIG_FLAG_NONE,
        uint32_t PollingIdleTimeoutUs = 0) noexcept
        : Count(1) {
    ...
    void Initialize(_In_ QUIC_GLOBAL_EXECUTION_CONFIG_FLAGS Flags, // Used for datapath type
        _In_ uint32_t PollingIdleTimeoutUs,
        _In_reads_(this->Count) QUIC_EXECUTION_CONFIG* Configs) {
        Executions = new (std::nothrow) QUIC_EXECUTION*[Count];
        if (Executions != nullptr) {
            auto Status = MsQuic->ExecutionCreate(Flags, PollingIdleTimeoutUs, Count, Configs, Executions);
...
struct MsQuicRegistration {
    ...
    MsQuicRegistration(_In_ bool AutoCleanUp = false) noexcept : CloseAllConnectionsOnDelete(AutoCleanUp) {
        InitStatus = MsQuic->RegistrationOpen(nullptr, &Handle);
    }
    MsQuicRegistration(_In_z_ const char* AppName,
        QUIC_EXECUTION_PROFILE Profile = QUIC_EXECUTION_PROFILE_LOW_LATENCY,
        _In_ bool AutoCleanUp = false) noexcept
    ...
    ~MsQuicRegistration() noexcept {
        if (Handle != nullptr) {
            if (CloseAllConnectionsOnDelete) {
                MsQuic->RegistrationShutdown(Handle, QUIC_CONNECTION_SHUTDOWN_FLAG_SILENT, 1);
            }
    ...
    void Shutdown(_In_ QUIC_CONNECTION_SHUTDOWN_FLAGS Flags, _In_ QUIC_UINT62 ErrorCode) noexcept {
        MsQuic->RegistrationShutdown(Handle, Flags, ErrorCode);
    }
#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES
    void CloseAsync(_In_ QUIC_REGISTRATION_CLOSE_CALLBACK_HANDLER Handler, _In_opt_ void* Context) noexcept {
        if (Handle != nullptr) {
...
class MsQuicAlpn {
  ...
  public:
    ...
    operator const QUIC_BUFFER*() const noexcept {
        return Buffers;
    }
...
class MsQuicVersionSettings : public QUIC_VERSION_SETTINGS {
  public:
    ...
    SetGlobal() const noexcept {
        ...
        return MsQuic->SetParam(nullptr, QUIC_PARAM_GLOBAL_VERSION_SETTINGS, sizeof(*Settings), Settings);
    }
    QUIC_STATUS
    GetGlobal() noexcept {
        QUIC_VERSION_SETTINGS* Settings = this;
        uint32_t Size = sizeof(*Settings);
        return MsQuic->GetParam(nullptr, QUIC_PARAM_GLOBAL_VERSION_SETTINGS, &Size, Settings);
    }
};
...
class MsQuicGlobalSettings : public QUIC_GLOBAL_SETTINGS {
  public:
    MsQuicGlobalSettings() noexcept {
        IsSetFlags = 0;
    }
    MsQuicGlobalSettings& SetRetryMemoryLimit(uint16_t Value) {
        RetryMemoryLimit = Value;
        IsSet.RetryMemoryLimit = TRUE;
        return *this;
    }
    MsQuicGlobalSettings& SetLoadBalancingMode(uint16_t Value) {
        LoadBalancingMode = Value;
        IsSet.LoadBalancingMode = TRUE;
        return *this;
    }
    MsQuicGlobalSettings& SetFixedServerID(uint32_t Value) {
        FixedServerID = Value;
        IsSet.FixedServerID = TRUE;
        return *this;
    }

    QUIC_STATUS Set() const noexcept {
        const QUIC_GLOBAL_SETTINGS* Settings = this;
        return MsQuic->SetParam(nullptr, QUIC_PARAM_GLOBAL_GLOBAL_SETTINGS, sizeof(*Settings), Settings);
    }

    QUIC_STATUS Get() noexcept {
        QUIC_GLOBAL_SETTINGS* Settings = this;
        uint32_t Size = sizeof(*Settings);
        return MsQuic->GetParam(nullptr, QUIC_PARAM_GLOBAL_GLOBAL_SETTINGS, &Size, Settings);
    }
};

class MsQuicSettings : public QUIC_SETTINGS {
  public:
    MsQuicSettings() noexcept {
        IsSetFlags = 0;
    }
    MsQuicSettings& SetSendBufferingEnabled(bool Value) {
        SendBufferingEnabled = Value;
        IsSet.SendBufferingEnabled = TRUE;
        return *this;
    }
    MsQuicSettings& SetPacingEnabled(bool Value) {
        PacingEnabled = Value;
        IsSet.PacingEnabled = TRUE;
        return *this;
    }
    MsQuicSettings& SetMigrationEnabled(bool Value) {
        MigrationEnabled = Value;
        IsSet.MigrationEnabled = TRUE;
        return *this;
    }
    MsQuicSettings& SetDatagramReceiveEnabled(bool Value) {
        DatagramReceiveEnabled = Value;
        IsSet.DatagramReceiveEnabled = TRUE;
        return *this;
    }
    MsQuicSettings& SetServerResumptionLevel(QUIC_SERVER_RESUMPTION_LEVEL Value) {
        ServerResumptionLevel = (uint8_t)Value;
        IsSet.ServerResumptionLevel = TRUE;
        return *this;
    }
    MsQuicSettings& SetInitialRttMs(uint32_t Value) {
        InitialRttMs = Value;
        IsSet.InitialRttMs = TRUE;
        return *this;
    }
    MsQuicSettings& SetIdleTimeoutMs(uint64_t Value) {
        IdleTimeoutMs = Value;
        IsSet.IdleTimeoutMs = TRUE;
        return *this;
    }
    MsQuicSettings& SetHandshakeIdleTimeoutMs(uint64_t Value) {
        HandshakeIdleTimeoutMs = Value;
        IsSet.HandshakeIdleTimeoutMs = TRUE;
        return *this;
    }
    MsQuicSettings& SetDisconnectTimeoutMs(uint32_t Value) {
        DisconnectTimeoutMs = Value;
        IsSet.DisconnectTimeoutMs = TRUE;
        return *this;
    }
    MsQuicSettings& SetPeerBidiStreamCount(uint16_t Value) {
        PeerBidiStreamCount = Value;
        IsSet.PeerBidiStreamCount = TRUE;
        return *this;
    }
    MsQuicSettings& SetPeerUnidiStreamCount(uint16_t Value) {
        PeerUnidiStreamCount = Value;
        IsSet.PeerUnidiStreamCount = TRUE;
        return *this;
    }
    MsQuicSettings& SetStreamRecvWindowDefault(uint32_t Value) {
        StreamRecvWindowDefault = Value;
        IsSet.StreamRecvWindowDefault = TRUE;
        return *this;
    }
    MsQuicSettings& SetMaxBytesPerKey(uint64_t Value) {
        MaxBytesPerKey = Value;
        IsSet.MaxBytesPerKey = TRUE;
        return *this;
    }
    MsQuicSettings& SetMaxAckDelayMs(uint32_t Value) {
        MaxAckDelayMs = Value;
        IsSet.MaxAckDelayMs = TRUE;
        return *this;
    }
    MsQuicSettings& SetMaximumMtu(uint16_t Mtu) {
        MaximumMtu = Mtu;
        IsSet.MaximumMtu = TRUE;
        return *this;
    }
    MsQuicSettings& SetMinimumMtu(uint16_t Mtu) {
        MinimumMtu = Mtu;
        IsSet.MinimumMtu = TRUE;
        return *this;
    }
    MsQuicSettings& SetMtuDiscoverySearchCompleteTimeoutUs(uint64_t Time) {
        MtuDiscoverySearchCompleteTimeoutUs = Time;
        IsSet.MtuDiscoverySearchCompleteTimeoutUs = TRUE;
        return *this;
    }
    MsQuicSettings& SetMtuDiscoveryMissingProbeCount(uint8_t Count) {
        MtuDiscoveryMissingProbeCount = Count;
        IsSet.MtuDiscoveryMissingProbeCount = TRUE;
        return *this;
    }
    MsQuicSettings& SetKeepAlive(uint32_t Time) {
        KeepAliveIntervalMs = Time;
        IsSet.KeepAliveIntervalMs = TRUE;
        return *this;
    }
    MsQuicSettings& SetConnFlowControlWindow(uint32_t Window) {
        ConnFlowControlWindow = Window;
        IsSet.ConnFlowControlWindow = TRUE;
        return *this;
    }
    MsQuicSettings& SetCongestionControlAlgorithm(QUIC_CONGESTION_CONTROL_ALGORITHM Cc) {
        CongestionControlAlgorithm = (uint8_t)Cc;
        IsSet.CongestionControlAlgorithm = TRUE;
        return *this;
    }
    MsQuicSettings& SetDestCidUpdateIdleTimeoutMs(uint32_t Value) {
        DestCidUpdateIdleTimeoutMs = Value;
        IsSet.DestCidUpdateIdleTimeoutMs = TRUE;
        return *this;
    }
    MsQuicSettings& SetGreaseQuicBitEnabled(bool Value) {
        GreaseQuicBitEnabled = Value;
        IsSet.GreaseQuicBitEnabled = TRUE;
        return *this;
    }
    MsQuicSettings& SetEcnEnabled(bool Value) {
        EcnEnabled = Value;
        IsSet.EcnEnabled = TRUE;
        return *this;
    }
    MsQuicSettings& SetHyStartEnabled(bool Value) {
        HyStartEnabled = Value;
        IsSet.HyStartEnabled = TRUE;
        return *this;
    }
#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES
    MsQuicSettings& SetEncryptionOffloadAllowed(bool Value) {
        EncryptionOffloadAllowed = Value;
        IsSet.EncryptionOffloadAllowed = TRUE;
        return *this;
    }
    // Deprecated compatibility shorthand: TRUE requests all supported modern
    // RESET_STREAM_AT dialects; FALSE requests none.
    MsQuicSettings& SetReliableResetEnabled(bool value) {
        ReliableResetEnabled = value;
        IsSet.ReliableResetEnabled = TRUE;
        return *this;
    }
    MsQuicSettings& SetXdpEnabled(bool value) {
        XdpEnabled = value;
        IsSet.XdpEnabled = TRUE;
        return *this;
    }
    MsQuicSettings& SetQtipEnabled(bool value) {
        QTIPEnabled = value;
        IsSet.QTIPEnabled = TRUE;
        return *this;
    }
    MsQuicSettings& SetOneWayDelayEnabled(bool value) {
        OneWayDelayEnabled = value;
        IsSet.OneWayDelayEnabled = TRUE;
        return *this;
    }
    MsQuicSettings& SetNetStatsEventEnabled(bool value) {
        NetStatsEventEnabled = value;
        IsSet.NetStatsEventEnabled = TRUE;
        return *this;
    }
    MsQuicSettings& SetStreamMultiReceiveEnabled(bool value) {
        StreamMultiReceiveEnabled = value;
        IsSet.StreamMultiReceiveEnabled = TRUE;
        return *this;
    }
#endif
    ...
    SetGlobal() const noexcept {
        ...
        return MsQuic->SetParam(nullptr, QUIC_PARAM_GLOBAL_SETTINGS, sizeof(*Settings), Settings);
    }
    ...
    GetGlobal() noexcept {
        ...
        uint32_t Size = sizeof(*Settings);
        return MsQuic->GetParam(nullptr, QUIC_PARAM_GLOBAL_SETTINGS, &Size, Settings);
    }
};
...
struct MsQuicConfiguration {
    ...
    MsQuicConfiguration(_In_ const MsQuicRegistration& Reg, _In_ const MsQuicAlpn& Alpns) {
        InitStatus = !Reg.IsValid()
                         ? Reg.GetInitStatus()
                         : MsQuic->ConfigurationOpen(Reg, Alpns, Alpns.Length(), nullptr, 0, nullptr, &Handle);
    }
    MsQuicConfiguration(_In_ const MsQuicRegistration& Reg,
    ...
    MsQuicConfiguration(_In_ const MsQuicRegistration& Reg,
        _In_ const MsQuicAlpn& Alpns,
        _In_ const MsQuicSettings& Settings) noexcept {
        InitStatus = !Reg.IsValid() ? Reg.GetInitStatus()
                                    : MsQuic->ConfigurationOpen(
                                          Reg, Alpns, Alpns.Length(), &Settings, sizeof(Settings), nullptr, &Handle);
    }
    MsQuicConfiguration(_In_ const MsQuicRegistration& Reg,
        _In_ const MsQuicAlpn& Alpns,
        _In_ const MsQuicSettings& Settings,
        _In_ const MsQuicCredentialConfig& CredConfig) noexcept {
        InitStatus = !Reg.IsValid() ? Reg.GetInitStatus()
                                    : MsQuic->ConfigurationOpen(
                                          Reg, Alpns, Alpns.Length(), &Settings, sizeof(Settings), nullptr, &Handle);
        if (IsValid()) {
    ...
    SetTicketKey(_In_ const QUIC_TICKET_KEY_CONFIG* KeyConfig) noexcept {
        return MsQuic->SetParam(
            Handle, QUIC_PARAM_CONFIGURATION_TICKET_KEYS, sizeof(QUIC_TICKET_KEY_CONFIG), KeyConfig);
    }
    QUIC_STATUS
    SetTicketKeys(_In_reads_(KeyCount) const QUIC_TICKET_KEY_CONFIG* KeyConfig, uint8_t KeyCount) noexcept {
        return MsQuic->SetParam(
            Handle, QUIC_PARAM_CONFIGURATION_TICKET_KEYS, KeyCount * sizeof(QUIC_TICKET_KEY_CONFIG), KeyConfig);
    }
    QUIC_STATUS
    SetSettings(_In_ const MsQuicSettings& Settings) noexcept {
        ...
        return MsQuic->SetParam(Handle, QUIC_PARAM_CONFIGURATION_SETTINGS, sizeof(*QSettings), QSettings);
    }
    ...
    GetSettings(_Out_ MsQuicSettings& Settings) noexcept {
        ...
        return MsQuic->GetParam(Handle, QUIC_PARAM_CONFIGURATION_SETTINGS, &Size, QSettings);
    }

#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES
    QUIC_STATUS
    SetVersionSettings(_In_ const MsQuicVersionSettings& Settings) noexcept {
        const QUIC_VERSION_SETTINGS* QSettings = &Settings;
        return MsQuic->SetParam(Handle, QUIC_PARAM_CONFIGURATION_VERSION_SETTINGS, sizeof(*QSettings), QSettings);
    }

    QUIC_STATUS
    GetVersionSettings(_Out_ MsQuicVersionSettings& Settings, _Inout_ uint32_t* SettingsLength) noexcept {
        QUIC_VERSION_SETTINGS* VSettings = &Settings;
        return MsQuic->GetParam(Handle, QUIC_PARAM_CONFIGURATION_VERSION_SETTINGS, SettingsLength, VSettings);
    }

    QUIC_STATUS
    SetVersionNegotiationExtEnabled(_In_ const bool Value = true) noexcept {
        BOOLEAN _Value = Value;
        return MsQuic->SetParam(Handle, QUIC_PARAM_CONFIGURATION_VERSION_NEG_ENABLED, sizeof(_Value), &_Value);
    }

    QUIC_STATUS
    SetResetStreamAtDialectMask(_In_ uint32_t DialectMask) noexcept {
        return MsQuic->SetParam(
            Handle, QUIC_PARAM_CONFIGURATION_RESET_STREAM_AT_DIALECT_MASK, sizeof(DialectMask), &DialectMask);
    }
#endif
};
...
typedef QUIC_STATUS QUIC_API MsQuicListenerCallback(
    _In_ struct MsQuicListener* Listener, _In_opt_ void* Context, _Inout_ QUIC_LISTENER_EVENT* Event);
...
struct MsQuicListener {
    ...
    MsQuicListener(_In_ const MsQuicRegistration& Registration,
        _In_ MsQuicCleanUpMode CleanUpMode,
        _In_ MsQuicListenerCallback* Callback,
        _In_ void* Context = nullptr) noexcept
        : CleanUpMode(CleanUpMode), Callback(Callback), Context(Context) {
        if (!Registration.IsValid()) {
            InitStatus = Registration.GetInitStatus();
            return;
        }
        if (QUIC_FAILED(InitStatus = MsQuic->ListenerOpen(
                            Registration, (QUIC_LISTENER_CALLBACK_HANDLER)MsQuicCallback, this, &Handle))) {
            Handle = nullptr;
        }
    }
    ...
    QUIC_STATUS
    Start(_In_ const MsQuicAlpn& Alpns, _In_opt_ const QUIC_ADDR* Address = nullptr) noexcept {
    ...
    QUIC_STATUS
    SetParam(
        _In_ uint32_t Param, _In_ uint32_t BufferLength, _In_reads_bytes_(BufferLength) const void* Buffer) noexcept {
        return MsQuic->SetParam(Handle, Param, BufferLength, Buffer);
    }

    QUIC_STATUS
    GetParam(_In_ uint32_t Param,
        _Inout_ _Pre_defensive_ uint32_t* BufferLength,
        _Out_writes_bytes_opt_(*BufferLength) void* Buffer) noexcept {
    ...
    GetLocalAddr(_Out_ QuicAddr& Addr) {
        ...
        return GetParam(QUIC_PARAM_LISTENER_LOCAL_ADDRESS, &Size, &Addr.SockAddr);
    }

#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES
    QUIC_STATUS
    SetCibirId(_In_reads_(Length) const uint8_t* Value, _In_ uint8_t Length) noexcept {
        return MsQuic->SetParam(Handle, QUIC_PARAM_LISTENER_CIBIR_ID, Length, Value);
    }

    QUIC_STATUS
    SetPartitionId(_In_ const uint16_t Value) noexcept {
        return MsQuic->SetParam(Handle, QUIC_PARAM_LISTENER_PARTITION_INDEX, sizeof(Value), &Value);
    }
#endif
    ...
    GetStatistics(_Out_ QUIC_LISTENER_STATISTICS& Statistics) const noexcept {
        ...
        return MsQuic->GetParam(Handle, QUIC_PARAM_LISTENER_STATS, &Size, &Statistics);
    }
  ...
  private:
    _IRQL_requires_max_(PASSIVE_LEVEL) _Function_class_(QUIC_LISTENER_CALLBACK) static QUIC_STATUS QUIC_API
        MsQuicCallback(
            _In_ HQUIC /* Listener */, _In_opt_ MsQuicListener* pThis, _Inout_ QUIC_LISTENER_EVENT* Event) noexcept {
        CXPLAT_DBG_ASSERT(pThis);
        auto DeleteOnExit = Event->Type == QUIC_LISTENER_EVENT_STOP_COMPLETE && pThis->CleanUpMode == CleanUpAutoDelete;
...
typedef QUIC_STATUS QUIC_API MsQuicConnectionCallback(
    _In_ struct MsQuicConnection* Connection, _In_opt_ void* Context, _Inout_ QUIC_CONNECTION_EVENT* Event);
...
struct MsQuicConnection {
    ...
    MsQuicConnection(_In_ const MsQuicRegistration& Registration,
        ...
        if (QUIC_FAILED(InitStatus = MsQuic->ConnectionOpen(
                            Registration, (QUIC_CONNECTION_CALLBACK_HANDLER)MsQuicCallback, this, &Handle))) {
            Handle = nullptr;
        }
    }
    ...
    MsQuicConnection(_In_ const MsQuicRegistration& Registration,
        ...
        if (QUIC_FAILED(
                InitStatus = MsQuic->ConnectionOpenInPartition(
                    Registration, PartitionIndex, (QUIC_CONNECTION_CALLBACK_HANDLER)MsQuicCallback, this, &Handle))) {
            Handle = nullptr;
        }
    }

    MsQuicConnection(_In_ HQUIC ConnectionHandle,
        _In_ MsQuicCleanUpMode CleanUpMode,
    ...
    void Shutdown(_In_ _Pre_defensive_ QUIC_UINT62 ErrorCode, // Application defined error code
        _In_ QUIC_CONNECTION_SHUTDOWN_FLAGS Flags = QUIC_CONNECTION_SHUTDOWN_FLAG_NONE) noexcept {
    ...
    QUIC_STATUS
    Start(_In_ const MsQuicConfiguration& Config,
    ...
    QUIC_STATUS
    SetConfiguration(_In_ const MsQuicConfiguration& Config) noexcept {
        return MsQuic->ConnectionSetConfiguration(Handle, Config);
    }

    QUIC_STATUS
    SendResumptionTicket(_In_ QUIC_SEND_RESUMPTION_FLAGS Flags = QUIC_SEND_RESUMPTION_FLAG_NONE,
        _In_ uint16_t DataLength = 0,
        _In_reads_bytes_opt_(DataLength) const uint8_t* ResumptionData = nullptr) noexcept {
        return MsQuic->ConnectionSendResumptionTicket(Handle, Flags, DataLength, ResumptionData);
    }

#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES
    QUIC_STATUS
    ExportKeyingMaterial(_In_ const QUIC_KEYING_MATERIAL_CONFIG* Config,
        _Out_writes_bytes_(Config->OutputLength) uint8_t* Output) noexcept {
    ...
    GetLocalAddr(_Out_ QuicAddr& Addr) {
        uint32_t Size = sizeof(Addr.SockAddr);
        return GetParam(QUIC_PARAM_CONN_LOCAL_ADDRESS, &Size, &Addr.SockAddr);
    }

    QUIC_STATUS
    GetRemoteAddr(_Out_ QuicAddr& Addr) {
        uint32_t Size = sizeof(Addr.SockAddr);
        return GetParam(QUIC_PARAM_CONN_REMOTE_ADDRESS, &Size, &Addr.SockAddr);
    }

    QUIC_STATUS
    SetLocalAddr(_In_ const QuicAddr& Addr) noexcept {
        return MsQuic->SetParam(Handle, QUIC_PARAM_CONN_LOCAL_ADDRESS, sizeof(Addr.SockAddr), &Addr.SockAddr);
    }

    QUIC_STATUS
    SetRemoteAddr(_In_ const QuicAddr& Addr) noexcept {
        return MsQuic->SetParam(Handle, QUIC_PARAM_CONN_REMOTE_ADDRESS, sizeof(Addr.SockAddr), &Addr.SockAddr);
    }

    QUIC_STATUS
    SetLocalInterface(_In_ uint32_t Index) noexcept {
        return MsQuic->SetParam(Handle, QUIC_PARAM_CONN_LOCAL_INTERFACE, sizeof(Index), &Index);
    }

    QUIC_STATUS
    SetShareUdpBinding(_In_ bool ShareBinding = true) noexcept {
        BOOLEAN Value = ShareBinding ? TRUE : FALSE;
        return MsQuic->SetParam(Handle, QUIC_PARAM_CONN_SHARE_UDP_BINDING, sizeof(Value), &Value);
    }

    QUIC_STATUS
    SetResumptionTicket(_In_reads_(TicketLength) const uint8_t* Ticket, uint32_t TicketLength) noexcept {
        return MsQuic->SetParam(Handle, QUIC_PARAM_CONN_RESUMPTION_TICKET, TicketLength, Ticket);
    }

    QUIC_STATUS
    SetSettings(_In_ const MsQuicSettings& Settings) noexcept {
        const QUIC_SETTINGS* QSettings = &Settings;
        return MsQuic->SetParam(Handle, QUIC_PARAM_CONN_SETTINGS, sizeof(*QSettings), QSettings);
    }
    ...
    GetSettings(_Out_ MsQuicSettings* Settings) const noexcept {
        QUIC_SETTINGS* QSettings = Settings;
        uint32_t Size = sizeof(*QSettings);
        return MsQuic->GetParam(Handle, QUIC_PARAM_CONN_SETTINGS, &Size, QSettings);
    }

    QUIC_STATUS
    GetStatistics(_Out_ QUIC_STATISTICS_V2* Statistics) const noexcept {
        uint32_t Size = sizeof(*Statistics);
        return MsQuic->GetParam(Handle, QUIC_PARAM_CONN_STATISTICS_V2, &Size, Statistics);
    }

    QUIC_STATUS
    SetKeepAlivePadding(_In_ uint16_t Value) noexcept {
        return MsQuic->SetParam(Handle, QUIC_PARAM_CONN_KEEP_ALIVE_PADDING, sizeof(Value), &Value);
    }

#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES
    QUIC_STATUS
    SetCibirId(_In_reads_(Length) const uint8_t* Value, _In_ uint8_t Length) noexcept {
        return MsQuic->SetParam(Handle, QUIC_PARAM_CONN_CIBIR_ID, Length, Value);
    }
#endif
    ...
    static QUIC_STATUS QUIC_API NoOpCallback(_In_ MsQuicConnection* /* Connection */,
        _In_opt_ void* /* Context */,
        _Inout_ QUIC_CONNECTION_EVENT* Event) noexcept {
    ...
    static QUIC_STATUS QUIC_API SendResumptionCallback(_In_ MsQuicConnection* Connection,
        _In_opt_ void* /* Context */,
        _Inout_ QUIC_CONNECTION_EVENT* Event) noexcept {
  ...
  private:
    _IRQL_requires_max_(PASSIVE_LEVEL) _Function_class_(QUIC_CONNECTION_CALLBACK) static QUIC_STATUS QUIC_API
        MsQuicCallback(_In_ HQUIC /* Connection */,
            _In_opt_ MsQuicConnection* pThis,
            _Inout_ QUIC_CONNECTION_EVENT* Event) noexcept {
        CXPLAT_DBG_ASSERT(pThis);
        auto DeleteOnExit =
            Event->Type == QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE && pThis->CleanUpMode == CleanUpAutoDelete;
        if (Event->Type == QUIC_CONNECTION_EVENT_CONNECTED) {
            ...
            if (pThis->ResumptionTicket) {
                ...
                memcpy(pThis->ResumptionTicket,
                    Event->RESUMPTION_TICKET_RECEIVED.ResumptionTicket,
                    pThis->ResumptionTicketLength);
#ifdef CX_PLATFORM_TYPE
...
struct MsQuicAutoAcceptListener : public MsQuicListener {
    ...
    MsQuicAutoAcceptListener(_In_ const MsQuicRegistration& Registration,
        _In_ MsQuicConnectionCallback* _ConnectionHandler,
        _In_ void* _ConnectionContext = nullptr) noexcept
        : MsQuicListener(Registration, CleanUpManual, ListenerCallback, this), Configuration(nullptr),
          ConnectionHandler(_ConnectionHandler), ConnectionContext(_ConnectionContext) {
    }
    ...
    MsQuicAutoAcceptListener(_In_ const MsQuicRegistration& Registration,
        ...
        _In_ MsQuicConnectionCallback* _ConnectionHandler,
        _In_ void* _ConnectionContext = nullptr) noexcept
        : MsQuicListener(Registration, CleanUpManual, ListenerCallback, this), Configuration(&Config),
          ConnectionHandler(_ConnectionHandler), ConnectionContext(_ConnectionContext) {
    }

  private:
    static _IRQL_requires_max_(PASSIVE_LEVEL) _Function_class_(QUIC_LISTENER_CALLBACK) QUIC_STATUS QUIC_API
        ListenerCallback(
            _In_ MsQuicListener* /* Listener */, _In_opt_ void* Context, _Inout_ QUIC_LISTENER_EVENT* Event) noexcept {
        auto pThis = (MsQuicAutoAcceptListener*)Context;
        CXPLAT_DBG_ASSERT(pThis);
        QUIC_STATUS Status = QUIC_STATUS_INVALID_STATE;
        if (Event->Type == QUIC_LISTENER_EVENT_NEW_CONNECTION) {
            auto Connection = new (std::nothrow) MsQuicConnection(Event->NEW_CONNECTION.Connection,
                CleanUpAutoDelete,
                pThis->ConnectionHandler,
                pThis->ConnectionContext);
            if (Connection) {
...
typedef QUIC_STATUS QUIC_API MsQuicStreamCallback(
    _In_ struct MsQuicStream* Stream, _In_opt_ void* Context, _Inout_ QUIC_STREAM_EVENT* Event);
...
struct MsQuicStream {
    ...
    MsQuicStream(_In_ const MsQuicConnection& Connection,
        ...
        if (QUIC_FAILED(InitStatus = MsQuic->StreamOpen(
                            Connection, Flags, (QUIC_STREAM_CALLBACK_HANDLER)MsQuicCallback, this, &Handle))) {
            Handle = nullptr;
        }
    }

    MsQuicStream(_In_ HQUIC StreamHandle,
        _In_ MsQuicCleanUpMode CleanUpMode,
        _In_ MsQuicStreamCallback* Callback = NoOpCallback,
        _In_ void* Context = nullptr) noexcept
        : CleanUpMode(CleanUpMode), Callback(Callback), Context(Context) {
    ...
    QUIC_STATUS
    Start(_In_ QUIC_STREAM_START_FLAGS Flags = QUIC_STREAM_START_FLAG_NONE) noexcept {
        return MsQuic->StreamStart(Handle, Flags);
    }

    QUIC_STATUS
    Shutdown(_In_ _Pre_defensive_ QUIC_UINT62 ErrorCode, // Application defined error code
        _In_ QUIC_STREAM_SHUTDOWN_FLAGS Flags = QUIC_STREAM_SHUTDOWN_FLAG_ABORT) noexcept {
    ...
    void ConnectionShutdown(_In_ _Pre_defensive_ QUIC_UINT62 ErrorCode, // Application defined error code
        _In_ QUIC_CONNECTION_SHUTDOWN_FLAGS Flags = QUIC_CONNECTION_SHUTDOWN_FLAG_NONE) noexcept {
        MsQuic->ConnectionShutdown(Handle, Flags, ErrorCode);
    }

    QUIC_STATUS
    Send(_In_reads_(BufferCount) _Pre_defensive_ const QUIC_BUFFER* const Buffers,
        _In_ uint32_t BufferCount = 1,
        _In_ QUIC_SEND_FLAGS Flags = QUIC_SEND_FLAG_NONE,
        _In_opt_ void* ClientSendContext = nullptr) noexcept {
        return MsQuic->StreamSend(Handle, Buffers, BufferCount, Flags, ClientSendContext);
    }

    _IRQL_requires_max_(DISPATCH_LEVEL) void ReceiveComplete(_In_ uint64_t BufferLength) noexcept {
        MsQuic->StreamReceiveComplete(Handle, BufferLength);
    }

    _IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS ReceiveSetEnabled(_In_ bool IsEnabled = true) noexcept {
    ...
    GetID(_Out_ QUIC_UINT62* ID) const noexcept {
        ...
        return MsQuic->GetParam(Handle, QUIC_PARAM_STREAM_ID, &Size, ID);
    }
    ...
    SetPriority(_In_ uint16_t Priority) noexcept {
        return MsQuic->SetParam(Handle, QUIC_PARAM_STREAM_PRIORITY, sizeof(Priority), &Priority);
    }

    QUIC_STATUS
    GetPriority(_Out_ uint16_t* Priority) const noexcept {
        uint32_t Size = sizeof(*Priority);
        return MsQuic->GetParam(Handle, QUIC_PARAM_STREAM_PRIORITY, &Size, Priority);
    }

    QUIC_STATUS
    GetIdealSendBufferSize(_Out_ uint64_t* SendBufferSize) const noexcept {
        uint32_t Size = sizeof(*SendBufferSize);
        return MsQuic->GetParam(Handle, QUIC_PARAM_STREAM_IDEAL_SEND_BUFFER_SIZE, &Size, SendBufferSize);
    }

    QUIC_STATUS
    GetStatistics(_Out_ QUIC_STREAM_STATISTICS* Statistics) const noexcept {
        uint32_t Size = sizeof(*Statistics);
        return MsQuic->GetParam(Handle, QUIC_PARAM_STREAM_STATISTICS, &Size, Statistics);
    }

#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES
    QUIC_STATUS
    SetReliableOffset(_In_ uint64_t Offset) noexcept {
        return MsQuic->SetParam(Handle, QUIC_PARAM_STREAM_RELIABLE_OFFSET, sizeof(Offset), &Offset);
    }

    QUIC_STATUS
    GetReliableOffset(_Out_ uint64_t* Offset) const noexcept {
        uint32_t Size = sizeof(*Offset);
        return MsQuic->GetParam(Handle, QUIC_PARAM_STREAM_RELIABLE_OFFSET, &Size, Offset);
    }

    QUIC_STATUS
    ProvideReceiveBuffers(
        _In_ uint32_t BufferCount, _In_reads_(BufferCount) const QUIC_BUFFER* Buffers) const noexcept {
    ...
    GetReliableOffsetRecv(_Out_ uint64_t* Offset) const noexcept {
        uint32_t Size = sizeof(*Offset);
        return MsQuic->GetParam(Handle, QUIC_PARAM_STREAM_RELIABLE_OFFSET_RECV, &Size, Offset);
    }
#endif
    ...
    static QUIC_STATUS QUIC_API NoOpCallback(
        _In_ MsQuicStream* /* Stream */, _In_opt_ void* /* Context */, _Inout_ QUIC_STREAM_EVENT* /* Event */
        ) noexcept {
        return QUIC_STATUS_SUCCESS;
    }

  private:
    _IRQL_requires_max_(PASSIVE_LEVEL) _Function_class_(QUIC_STREAM_CALLBACK) static QUIC_STATUS QUIC_API
        MsQuicCallback(
            _In_ HQUIC /* Stream */, _In_opt_ MsQuicStream* pThis, _Inout_ QUIC_STREAM_EVENT* Event) noexcept {
        CXPLAT_DBG_ASSERT(pThis);
        auto DeleteOnExit =
            Event->Type == QUIC_STREAM_EVENT_SHUTDOWN_COMPLETE && pThis->CleanUpMode == CleanUpAutoDelete;
        auto Status = pThis->Callback(pThis, pThis->Context, Event);
        if (DeleteOnExit) {
            delete pThis;
        }
...
struct QuicBufferScope {
    ...
    QuicBufferScope(uint32_t Size) noexcept
        : Buffer((QUIC_BUFFER*)new (std::nothrow) uint8_t[sizeof(QUIC_BUFFER) + Size]) {
        CXPLAT_DBG_ASSERT(Buffer);
        memset(Buffer, 0, sizeof(*Buffer) + Size);
        Buffer->Length = Size;
        Buffer->Buffer = (uint8_t*)(Buffer + 1);
    }
    ~QuicBufferScope() noexcept {
        if (Buffer) {
    ...
    operator QUIC_BUFFER*() noexcept {
        return Buffer;
    }
};

static_assert(
    sizeof(QuicBufferScope) == sizeof(QUIC_BUFFER*), "Scope guards should be the same size as the guarded type");
