...
#pragma warning(                                                                                                       \
    disable : 5105) // The conformant preprocessor along with the newest SDK throws this warning for a macro.
#include <windows.h>
#include <winsock2.h>
#include <ws2ipdef.h>
#include <iphlpapi.h>
...
_When_(Status < 0, _Out_range_(>, 0)) _When_(Status >= 0, _Out_range_(==, 0)) ULONG NTAPI
...
_IRQL_requires_max_(DISPATCH_LEVEL) void CxPlatLogAssert(
    _In_z_ const char* File, _In_ int Line, _In_z_ const char* Expr);

#define CXPLAT_WIDE_STRING(_str) L##_str

#define CXPLAT_ASSERT_NOOP(_exp, _msg) (CXPLAT_ANALYSIS_ASSUME(_exp), 0)

#define CXPLAT_ASSERT_LOG(_exp, _msg)                                                                                  \
    (CXPLAT_ANALYSIS_ASSUME(_exp), ((!(_exp)) ? (CxPlatLogAssert(__FILE__, __LINE__, #_exp), FALSE) : TRUE))

#define CXPLAT_ASSERT_CRASH(_exp, _msg)                                                                                \
    (CXPLAT_ANALYSIS_ASSUME(_exp),                                                                                     \
        ((!(_exp)) ? (CxPlatLogAssert(__FILE__, __LINE__, #_exp),                                                      \
...
WINBASEAPI
_When_(lpModuleName == NULL, _Ret_notnull_) _When_(lpModuleName != NULL, _Ret_maybenull_) HMODULE WINAPI
...
#define CxPlatVerifierEnabled(Flags)                                                                                   \
    (GetModuleHandleW(L"verifier.dll") != NULL && GetModuleHandleW(L"vrfcore.dll") != NULL), Flags = 0
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_INLINE void CxPlatCloseHandle(_Pre_notnull_ HANDLE Handle) {
    CloseHandle(Handle);
}
...
typedef CXPLAT_POOL_HEADER* (*CXPLAT_POOL_ALLOC_FN)(_In_ uint32_t Size, _In_ uint32_t Tag, _Inout_ CXPLAT_POOL* Pool);

typedef void (*CXPLAT_POOL_FREE_FN)(_In_ CXPLAT_POOL_HEADER* Entry, _In_ uint32_t Tag, _Inout_ CXPLAT_POOL* Pool);
...
QUIC_INLINE
CXPLAT_POOL_HEADER* CxPlatPoolGenericAlloc(_In_ uint32_t Size, _In_ uint32_t Tag, _Inout_ CXPLAT_POOL* Pool) {
    UNREFERENCED_PARAMETER(Pool);
    return (CXPLAT_POOL_HEADER*)CxPlatAlloc(Size, Tag);
}

QUIC_INLINE
void CxPlatPoolGenericFree(_In_ CXPLAT_POOL_HEADER* Entry, _In_ uint32_t Tag, _Inout_ CXPLAT_POOL* Pool) {
    UNREFERENCED_PARAMETER(Pool);
...
QUIC_INLINE
void CxPlatPoolInitializeEx(_In_ BOOLEAN IsPaged,
    _In_ uint32_t Size,
    _In_ uint32_t Tag,
...
CxPlatPoolPrune(_Inout_ CXPLAT_POOL* Pool) {
    CXPLAT_POOL_HEADER* Entry = (CXPLAT_POOL_HEADER*)InterlockedPopEntrySList(&Pool->ListHead);
    if (Entry == NULL) {
        return FALSE;
    }
#if DEBUG
...
QUIC_INLINE
void CxPlatRefInitialize(_Out_ CXPLAT_REF_COUNT* RefCount) {
    *RefCount = 1;
}

QUIC_INLINE
void CxPlatRefInitializeEx(_Out_ CXPLAT_REF_COUNT* RefCount, _In_ uint32_t Initial) {
...
QUIC_INLINE
BOOLEAN
CxPlatRefIncrementNonZero(_Inout_ volatile CXPLAT_REF_COUNT* RefCount, _In_ ULONG Bias) {
...
QUIC_INLINE
BOOLEAN
CxPlatRefDecrement(_Inout_ CXPLAT_REF_COUNT* RefCount) {
    CXPLAT_REF_COUNT NewValue;
...
QUIC_INLINE
BOOLEAN
CxPlatEventWaitWithTimeout(_In_ CXPLAT_EVENT Event, _In_ uint32_t TimeoutMs) {
...
QUIC_INLINE
BOOLEAN
CxPlatEventQAssociateHandle(_In_ CXPLAT_EVENTQ* queue, _In_ HANDLE fileHandle) {
...
QUIC_INLINE
void CxPlatSqeInitializeEx(_In_ CXPLAT_EVENT_COMPLETION_HANDLER completion, _Out_ CXPLAT_SQE* sqe) {
    sqe->Completion = completion;
    CxPlatZeroMemory(&sqe->Overlapped, sizeof(sqe->Overlapped));
#if DEBUG
    sqe->IsQueued = FALSE;
...
#ifdef QUIC_UWP_BUILD
WINBASEAPI
_Success_(return != FALSE) BOOL WINAPI GetSystemTimeAdjustment(
    _Out_ PDWORD lpTimeAdjustment, _Out_ PDWORD lpTimeIncrement, _Out_ PBOOL lpTimeAdjustmentDisabled);
#endif
...
//
QUIC_INLINE
uint64_t CxPlatGetTimerResolution() {
...
//
QUIC_INLINE
uint64_t QuicTimePlatToUs64(uint64_t Count) {
    //
    ...
    return ((High / CxPlatPerfFreq) << 32) + ((Low + ((High % CxPlatPerfFreq) << 32)) / CxPlatPerfFreq);
}
...
//
QUIC_INLINE
uint64_t CxPlatTimeUs64ToPlat(uint64_t TimeUs) {
    uint64_t High = (TimeUs >> 32) * CxPlatPerfFreq;
    uint64_t Low = (TimeUs & 0xFFFFFFFF) * CxPlatPerfFreq;
    return ((High / 1000000) << 32) + ((Low + ((High % 1000000) << 32)) / CxPlatPerfFreq);
}
...
//
QUIC_INLINE
uint64_t CxPlatTimeDiff64(_In_ uint64_t T1, // First time measured
...
//
QUIC_INLINE
uint32_t CxPlatTimeDiff32(_In_ uint32_t T1, // First time measured
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_INLINE uint32_t CxPlatProcNumberToIndex(PROCESSOR_NUMBER* ProcNumber) {
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_INLINE uint32_t CxPlatProcCurrentNumber(void) {
...
__kernel_entry NTSTATUS NTAPI NtSetInformationThread(_In_ HANDLE ThreadHandle,
...
BOOL WINAPI SetThreadGroupAffinity(
    _In_ HANDLE hThread, _In_ CONST GROUP_AFFINITY* GroupAffinity, _Out_opt_ PGROUP_AFFINITY PreviousGroupAffinity);
#endif
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS
    CxPlatUtf8ToWideChar(_In_z_ const char* const Input, _In_ uint32_t Tag, _Outptr_result_z_ PWSTR* Output);
...
#define QuicCompartmentIdSetCurrent(CompartmentId) HRESULT_FROM_WIN32(SetCurrentThreadCompartmentId(CompartmentId))
