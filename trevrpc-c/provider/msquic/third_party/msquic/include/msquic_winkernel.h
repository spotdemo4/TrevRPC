...
QUIC_INLINE
void QuicAddrSetFamily(_Out_ QUIC_ADDR* Addr, _In_ QUIC_ADDRESS_FAMILY Family) {
...
QUIC_INLINE
void QuicAddrSetPort(_Inout_ QUIC_ADDR* Addr,
...
QuicAddrFromString(_In_z_ const char* AddrStr,
    ...
    if (RtlIpv4StringToAddressExA(AddrStr, FALSE, &Addr->Ipv4.sin_addr, &Addr->Ipv4.sin_port) == STATUS_SUCCESS) {
        Addr->si_family = QUIC_ADDRESS_FAMILY_INET;
                   ...
                   AddrStr, &Addr->Ipv6.sin6_addr, &Addr->Ipv6.sin6_scope_id, &Addr->Ipv6.sin6_port) ==
               STATUS_SUCCESS) {
        Addr->si_family = QUIC_ADDRESS_FAMILY_INET6;
    } else {
        return FALSE;
...
CxPlatIsIpLiteral(_In_z_ const char* AddrStr) {
    ...
    IN_ADDR Ipv4Addr = {0};
    if (RtlIpv4StringToAddressA(AddrStr, TRUE, &Terminator, &Ipv4Addr) == STATUS_SUCCESS && *Terminator == '\0') {
        return TRUE;
    }

    IN6_ADDR Ipv6Addr = {0};
    if (RtlIpv6StringToAddressA(AddrStr, &Terminator, &Ipv6Addr) == STATUS_SUCCESS && *Terminator == '\0') {
        return TRUE;
    }
