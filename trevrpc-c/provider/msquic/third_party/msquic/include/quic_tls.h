...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL) void(CXPLAT_TLS_PROCESS_COMPLETE_CALLBACK)(
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) BOOLEAN(CXPLAT_TLS_RECEIVE_TP_CALLBACK)(
    _In_ QUIC_CONNECTION* Connection, _In_ uint16_t TPLength, _In_reads_(TPLength) const uint8_t* TPBuffer);
...
// in the context of a QuicTlsProcessData call; not on a separate thread.
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) BOOLEAN(CXPLAT_TLS_RECEIVE_TICKET_CALLBACK)(
    _In_ QUIC_CONNECTION* Connection, _In_ uint32_t TicketLength, _In_reads_(TicketLength) const uint8_t* Ticket);
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    BOOLEAN(CXPLAT_TLS_PEER_CERTIFICATE_RECEIVED_CALLBACK)(_In_ QUIC_CONNECTION* Connection,
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL) _Function_class_(CXPLAT_SEC_CONFIG_CREATE_COMPLETE) void(
    QUIC_API CXPLAT_SEC_CONFIG_CREATE_COMPLETE)(_In_ const QUIC_CREDENTIAL_CONFIG* CredConfig,
...
//
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_TLS_PROVIDER CxPlatTlsGetProvider(void);

//
// Creates a new TLS security configuration.
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatTlsSecConfigCreate(_In_ const QUIC_CREDENTIAL_CONFIG* CredConfig,
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatTlsSecConfigDelete(
    __drv_freesMem(ServerConfig) _Frees_ptr_ _In_ CXPLAT_SEC_CONFIG* SecurityConfig);

//
// Sets a NST ticket key for a security configuration.
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatTlsSecConfigSetTicketKeys(_In_ CXPLAT_SEC_CONFIG* SecurityConfig,
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatTlsInitialize(
    _In_ const CXPLAT_TLS_CONFIG* Config, _Inout_ CXPLAT_TLS_PROCESS_STATE* State, _Out_ CXPLAT_TLS** NewTlsContext);

//
// Uninitializes an existing TLS context.
//
_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatTlsUninitialize(_In_opt_ CXPLAT_TLS* TlsContext);
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) CXPLAT_TLS_RESULT_FLAGS CxPlatTlsProcessData(_In_ CXPLAT_TLS* TlsContext,
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatSecConfigParamSet(_In_ CXPLAT_SEC_CONFIG* TlsContext,
    _In_ uint32_t Param,
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatSecConfigParamGet(_In_ CXPLAT_SEC_CONFIG* TlsContext,
    _In_ uint32_t Param,
    _Inout_ uint32_t* BufferLength,
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatTlsParamSet(_In_ CXPLAT_TLS* TlsContext,
    _In_ uint32_t Param,
    _In_ uint32_t BufferLength,
    _In_reads_bytes_(BufferLength) const void* Buffer);
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatTlsParamGet(_In_ CXPLAT_TLS* TlsContext,
    _In_ uint32_t Param,
    _Inout_ uint32_t* BufferLength,
    _Inout_updates_bytes_opt_(*BufferLength) void* Buffer);
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatTlsExportKeyingMaterial(_In_ CXPLAT_TLS* TlsContext,
...
//
QUIC_INLINE
const uint8_t* CxPlatTlsAlpnFindInList(_In_ uint16_t AlpnListLength,
    _In_reads_(AlpnListLength) const uint8_t* AlpnList,
    _In_ uint8_t FindAlpnLength,
    _In_reads_(FindAlpnLength) const uint8_t* FindAlpn) {
    while (AlpnListLength != 0) {
        CXPLAT_DBG_ASSERT(AlpnList[0] + 1 <= AlpnListLength);
        if (AlpnList[0] == FindAlpnLength && memcmp(AlpnList + 1, FindAlpn, FindAlpnLength) == 0) {
            return AlpnList;
        }
...
//
_Success_(return == TRUE) BOOLEAN QuicTlsPopulateOffloadKeys(_Inout_ CXPLAT_TLS* TlsContext,
