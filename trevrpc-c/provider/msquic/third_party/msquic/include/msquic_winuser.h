...
#define SUCCESS_HRESULT_FROM_WIN32(x) ((HRESULT)(((x) & 0x0000FFFF) | (FACILITY_WIN32 << 16)))
...
QuicAddrIsValid(_In_ const QUIC_ADDR* const Addr) {
    return Addr->si_family == QUIC_ADDRESS_FAMILY_UNSPEC || Addr->si_family == QUIC_ADDRESS_FAMILY_INET ||
...
QUIC_INLINE
void QuicAddrSetFamily(_Inout_ QUIC_ADDR* Addr, _In_ QUIC_ADDRESS_FAMILY Family) {
    Addr->si_family = (ADDRESS_FAMILY)Family;
}

QUIC_INLINE
uint16_t // Returns in host byte order.
QuicAddrGetPort(_In_ const QUIC_ADDR* const Addr) {
    return QuicNetByteSwapShort(Addr->Ipv4.sin_port);
}

QUIC_INLINE
void QuicAddrSetPort(_Out_ QUIC_ADDR* Addr,
...
QUIC_INLINE
uint32_t QuicAddrHash(_In_ const QUIC_ADDR* Addr) {
...
QUIC_INLINE
_Success_(return != FALSE) BOOLEAN QuicAddrFromString(_In_z_ const char* AddrStr,
    _In_ uint16_t Port, // Host byte order
    ...
    } else if (RtlIpv6StringToAddressExA(
                   AddrStr, &Addr->Ipv6.sin6_addr, &Addr->Ipv6.sin6_scope_id, &Addr->Ipv6.sin6_port) == NO_ERROR) {
        Addr->si_family = QUIC_ADDRESS_FAMILY_INET6;
    } else {
        return FALSE;
    }
...
QUIC_INLINE
_Success_(return != FALSE) BOOLEAN CxPlatIsIpLiteral(_In_z_ const char* AddrStr) {
    const char* Terminator = NULL;
    IN_ADDR Ipv4Addr = {0};
    if (RtlIpv4StringToAddressA(AddrStr, TRUE, &Terminator, &Ipv4Addr) == NO_ERROR && *Terminator == '\0') {
        return TRUE;
    }

    IN6_ADDR Ipv6Addr = {0};
    if (RtlIpv6StringToAddressA(AddrStr, &Terminator, &Ipv6Addr) == NO_ERROR && *Terminator == '\0') {
        return TRUE;
    }
...
QUIC_INLINE
_Success_(return != FALSE) BOOLEAN QuicAddrToString(_In_ const QUIC_ADDR* Addr, _Out_ QUIC_ADDR_STR* AddrStr) {
    LONG Status;
    ...
    if (Addr->si_family == QUIC_ADDRESS_FAMILY_INET) {
        ...
        Status =
            RtlIpv6AddressToStringExA(&Addr->Ipv6.sin6_addr, 0, Addr->Ipv6.sin6_port, AddrStr->Address, &AddrStrLen);
    }
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(QUIC_EVENT_COMPLETION)(_In_ QUIC_CQE* Cqe);
