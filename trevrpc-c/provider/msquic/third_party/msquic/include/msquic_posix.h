...
#define DEFINE_ENUM_FLAG_OPERATORS(ENUMTYPE)                                                                           \
    extern "C++" {                                                                                                     \
    QUIC_INLINE ENUMTYPE operator|(ENUMTYPE a, ENUMTYPE b) throw() {                                                   \
        return ENUMTYPE(                                                                                               \
            ((_ENUM_FLAG_SIZED_INTEGER<ENUMTYPE>::type)a) | ((_ENUM_FLAG_SIZED_INTEGER<ENUMTYPE>::type)b));            \
    }                                                                                                                  \
    QUIC_INLINE ENUMTYPE& operator|=(ENUMTYPE& a, ENUMTYPE b) throw() {                                                \
    ...
    QUIC_INLINE ENUMTYPE operator~(ENUMTYPE a) throw() {                                                               \
        return ENUMTYPE(~((_ENUM_FLAG_SIZED_INTEGER<ENUMTYPE>::type)a));                                               \
    }                                                                                                                  \
    QUIC_INLINE ENUMTYPE operator^(ENUMTYPE a, ENUMTYPE b) throw() {                                                   \
        return ENUMTYPE(                                                                                               \
...
QUIC_INLINE
BOOLEAN
QuicAddrFamilyIsValid(_In_ QUIC_ADDRESS_FAMILY Family) {
    return Family == QUIC_ADDRESS_FAMILY_UNSPEC || Family == QUIC_ADDRESS_FAMILY_INET ||
           Family == QUIC_ADDRESS_FAMILY_INET6;
}

QUIC_INLINE
BOOLEAN
QuicAddrIsValid(_In_ const QUIC_ADDR* const Addr) {
    return QuicAddrFamilyIsValid(Addr->Ip.sa_family);
}

QUIC_INLINE
BOOLEAN
QuicAddrCompareIp(_In_ const QUIC_ADDR* const Addr1, _In_ const QUIC_ADDR* const Addr2) {
...
QUIC_INLINE
BOOLEAN
QuicAddrCompare(_In_ const QUIC_ADDR* const Addr1, _In_ const QUIC_ADDR* const Addr2) {
    if (Addr1->Ip.sa_family != Addr2->Ip.sa_family || Addr1->Ipv4.sin_port != Addr2->Ipv4.sin_port) {
        return FALSE;
    }
...
QUIC_INLINE
QUIC_ADDRESS_FAMILY
QuicAddrGetFamily(_In_ const QUIC_ADDR* const Addr) {
    return Addr->Ip.sa_family;
}

QUIC_INLINE
void QuicAddrSetFamily(_In_ QUIC_ADDR* Addr, _In_ QUIC_ADDRESS_FAMILY Family) {
    Addr->Ip.sa_family = Family;
}

QUIC_INLINE
uint16_t QuicAddrGetPort(_In_ const QUIC_ADDR* const Addr) {
...
QUIC_INLINE
void QuicAddrSetPort(_Out_ QUIC_ADDR* Addr, _In_ uint16_t Port) {
    if (QUIC_ADDRESS_FAMILY_INET == Addr->Ip.sa_family) {
...
//
QUIC_INLINE
void QuicAddrIncrement(_Inout_ QUIC_ADDR* Addr) {
...
QUIC_INLINE
void QuicAddrSetToLoopback(_Inout_ QUIC_ADDR* Addr) {
    if (Addr->Ip.sa_family == QUIC_ADDRESS_FAMILY_INET) {
...
QUIC_INLINE
uint32_t QUIC_NO_SANITIZE("unsigned-integer-overflow") QuicAddrHash(_In_ const QUIC_ADDR* Addr) {
...
QUIC_INLINE
BOOLEAN
QuicAddrIsWildCard(_In_ const QUIC_ADDR* const Addr) {
...
QUIC_INLINE
BOOLEAN
QuicAddr4FromString(_In_z_ const char* AddrStr, _Out_ QUIC_ADDR* Addr) {
    if (AddrStr[0] == '[') {
        return FALSE;
    }
...
QUIC_INLINE
BOOLEAN
QuicAddr6FromString(_In_z_ const char* AddrStr, _Out_ QUIC_ADDR* Addr) {
    if (AddrStr[0] == '[') {
...
QUIC_INLINE
BOOLEAN
QuicAddrFromString(_In_z_ const char* AddrStr,
    _In_ uint16_t Port, // Host byte order
    _Out_ QUIC_ADDR* Addr) {
    Addr->Ipv4.sin_port = htons(Port);
    return QuicAddr4FromString(AddrStr, Addr) || QuicAddr6FromString(AddrStr, Addr);
}
...
QUIC_INLINE
BOOLEAN
CxPlatIsIpLiteral(_In_z_ const char* AddrStr) {
    IN_ADDR Ipv4Addr = {0};
    IN6_ADDR Ipv6Addr = {0};
    return inet_pton(AF_INET, AddrStr, &Ipv4Addr) == 1 || inet_pton(AF_INET6, AddrStr, &Ipv6Addr) == 1;
}
...
QUIC_INLINE
BOOLEAN
QuicAddrToString(_In_ const QUIC_ADDR* Addr, _Out_ QUIC_ADDR_STR* AddrStr) {
    ...
    if (inet_ntop(Addr->Ip.sa_family == QUIC_ADDRESS_FAMILY_INET ? AF_INET : AF_INET6,
...
typedef void(QUIC_EVENT_COMPLETION)(_In_ QUIC_CQE* Cqe);
