...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL) _Function_class_(CXPLAT_PCP_CALLBACK) void(CXPLAT_PCP_CALLBACK)(
    _In_ CXPLAT_PCP* PcpContext, _In_ void* Context, _In_ const CXPLAT_PCP_EVENT* Event);
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatPcpInitialize(_In_ CXPLAT_DATAPATH* Datapath,
    _In_ void* Context,
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatPcpUninitialize(_In_ CXPLAT_PCP* PcpContext);

//
// Send a MAP request for an internal UDP port.
//
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS CxPlatPcpSendMapRequest(_In_ CXPLAT_PCP* PcpContext,
    _In_reads_(CXPLAT_PCP_NONCE_LENGTH) const uint8_t* Nonce,
...
//
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS CxPlatPcpSendPeerRequest(_In_ CXPLAT_PCP* PcpContext,
    _In_reads_(CXPLAT_PCP_NONCE_LENGTH) const uint8_t* Nonce,
    _In_opt_ const QUIC_ADDR* LocalAddress,
