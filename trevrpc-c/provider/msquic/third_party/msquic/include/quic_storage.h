...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    _Function_class_(CXPLAT_STORAGE_CHANGE_CALLBACK) void(CXPLAT_STORAGE_CHANGE_CALLBACK)(_In_ void* Context);
...
// handle to it.
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatStorageOpen(_In_opt_z_ const char* Path,
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatStorageClose(_In_opt_ _Post_invalid_ CXPLAT_STORAGE* Storage);

//
// Reads a value from the storage context.
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatStorageReadValue(_In_ CXPLAT_STORAGE* Storage,
    _In_opt_z_ const char* Name,
    _Out_writes_bytes_to_opt_(*BufferLength, *BufferLength) uint8_t* Buffer,
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatStorageWriteValue(_In_ CXPLAT_STORAGE* Storage,
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS
    CxPlatStorageDeleteValue(_In_ CXPLAT_STORAGE* Storage, _In_z_ const char* Name);

//
// Clears all settings from a storage context.
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatStorageClear(_In_ CXPLAT_STORAGE* Storage);
