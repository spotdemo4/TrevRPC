...
typedef enum QUIC_SEND_FLAGS {
    ...
    QUIC_SEND_FLAG_CANCEL_ON_BLOCKED =
        0x0080, // Indicates that a frame should be dropped when it can't be sent immediately.
} QUIC_SEND_FLAGS;
...
// by MsQuic.
//
#define QUIC_DATAGRAM_SEND_STATE_IS_FINAL(State) ((State) >= QUIC_DATAGRAM_SEND_LOST_DISCARDED)
...
typedef struct QUIC_GLOBAL_EXECUTION_CONFIG {
    ...
    _Field_size_(ProcessorCount) uint16_t ProcessorList[1]; // List of processors to use for threads.

} QUIC_GLOBAL_EXECUTION_CONFIG;

#define QUIC_GLOBAL_EXECUTION_CONFIG_MIN_SIZE (uint32_t)FIELD_OFFSET(QUIC_GLOBAL_EXECUTION_CONFIG, ProcessorList)
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS(QUIC_API* QUIC_EXECUTION_CREATE_FN)(
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(QUIC_API* QUIC_EXECUTION_DELETE_FN)(
    _In_ uint32_t Count, _In_reads_(Count) QUIC_EXECUTION** Executions);
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) uint32_t(QUIC_API* QUIC_EXECUTION_POLL_FN)(_In_ QUIC_EXECUTION* Execution);
...
#define QUIC_RESET_STREAM_AT_SUPPORTED_DIALECT_MASK                                                                    \
    (QUIC_RESET_STREAM_AT_DIALECT_DRAFT_07 | QUIC_RESET_STREAM_AT_DIALECT_DRAFT_10)
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL) _Function_class_(QUIC_CREDENTIAL_LOAD_COMPLETE) void(
    QUIC_API QUIC_CREDENTIAL_LOAD_COMPLETE)(_In_ HQUIC Configuration, _In_opt_ void* Context, _In_ QUIC_STATUS Status);
...
typedef struct QUIC_NETWORK_STATISTICS {
    ...
    uint64_t PostedBytes;      // Total bytes queued, but not yet acked. These may contain sent bytes that may have
                               // potentially lost too.
...
#define QUIC_STRUCT_SIZE_THRU_FIELD(Struct, Field) (FIELD_OFFSET(Struct, Field) + sizeof(((Struct*)0)->Field))

#define QUIC_STATISTICS_V2_SIZE_1                                                                                      \
    QUIC_STRUCT_SIZE_THRU_FIELD(QUIC_STATISTICS_V2, KeyUpdateCount) // MsQuic v2.0 final size
#define QUIC_STATISTICS_V2_SIZE_2                                                                                      \
    QUIC_STRUCT_SIZE_THRU_FIELD(QUIC_STATISTICS_V2, DestCidUpdateCount) // MsQuic v2.1 final size
#define QUIC_STATISTICS_V2_SIZE_3                                                                                      \
    QUIC_STRUCT_SIZE_THRU_FIELD(QUIC_STATISTICS_V2, SendEcnCongestionCount)                    // MsQuic v2.2 final size
#define QUIC_STATISTICS_V2_SIZE_4 QUIC_STRUCT_SIZE_THRU_FIELD(QUIC_STATISTICS_V2, RttVariance) // MsQuic v2.5 final size
#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES
#define QUIC_STATISTICS_V2_SIZE_5                                                                                      \
    QUIC_STRUCT_SIZE_THRU_FIELD(QUIC_STATISTICS_V2, ReceiveQueueDelayMaxUs) // MsQuic v2.6 preview size
#endif
...
typedef struct QUIC_STATELESS_RETRY_CONFIG {
    ...
    _Field_size_bytes_(SecretLength) const uint8_t* Secret; // Secret to generate the key.
...
typedef _IRQL_requires_max_(DISPATCH_LEVEL) void*(QUIC_API* QUIC_GET_CONTEXT_FN)(_In_ _Pre_defensive_ HQUIC Handle);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL) void(QUIC_API* QUIC_SET_CALLBACK_HANDLER_FN)(
    _In_ _Pre_defensive_ HQUIC Handle, _In_ void* Handler, _In_opt_ void* Context);
...
#define QUIC_PARAM_GLOBAL_STATISTICS_V2_SIZES                                                                          \
    0x0100000C // uint32_t[] - Array of sizes for each QUIC_STATISTICS_V2 version. Get-only. Pass a buffer of uint32_t,
               // output count is variable. See documentation for details.
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS(QUIC_API* QUIC_SET_PARAM_FN)(
    _When_(QUIC_PARAM_IS_GLOBAL(Param), _Reserved_) _When_(!QUIC_PARAM_IS_GLOBAL(Param), _In_ _Pre_defensive_)
        HQUIC Handle,
    _In_ uint32_t Param,
    _In_ uint32_t BufferLength,
    _In_reads_bytes_(BufferLength) const void* Buffer);

typedef _IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS(QUIC_API* QUIC_GET_PARAM_FN)(
    _When_(QUIC_PARAM_IS_GLOBAL(Param), _Reserved_) _When_(!QUIC_PARAM_IS_GLOBAL(Param), _In_ _Pre_defensive_)
        HQUIC Handle,
    _In_ uint32_t Param,
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_REGISTRATION_OPEN_FN)(_In_opt_ const QUIC_REGISTRATION_CONFIG* Config,
        _Outptr_ _At_(*Registration, __drv_allocatesMem(Mem)) _Pre_defensive_ HQUIC* Registration);
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(QUIC_API* QUIC_REGISTRATION_CLOSE_FN)(
    _In_ _Pre_defensive_ __drv_freesMem(Mem) HQUIC Registration);

#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES

typedef _Function_class_(QUIC_REGISTRATION_CLOSE_CALLBACK) void(QUIC_API QUIC_REGISTRATION_CLOSE_CALLBACK)(
...
// been closed by the application.
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(QUIC_API* QUIC_REGISTRATION_CLOSE2_FN)(
    _In_ _Pre_defensive_ __drv_freesMem(Mem) HQUIC Registration,
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL) void(QUIC_API* QUIC_REGISTRATION_SHUTDOWN_FN)(
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONFIGURATION_OPEN_FN)(_In_ _Pre_defensive_ HQUIC Registration,
        _In_reads_(AlpnBufferCount) _Pre_defensive_ const QUIC_BUFFER* const AlpnBuffers,
        _In_range_(>, 0) uint32_t AlpnBufferCount,
        _In_reads_bytes_opt_(SettingsSize) const QUIC_SETTINGS* Settings,
        _In_ uint32_t SettingsSize,
        _In_opt_ void* Context,
        _Outptr_ _At_(*Configuration, __drv_allocatesMem(Mem)) _Pre_defensive_ HQUIC* Configuration);

//
// Closes an existing configuration.
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(QUIC_API* QUIC_CONFIGURATION_CLOSE_FN)(
    _In_ _Pre_defensive_ __drv_freesMem(Mem) HQUIC Configuration);

//
// Loads the credentials based on the input configuration.
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS(QUIC_API* QUIC_CONFIGURATION_LOAD_CREDENTIAL_FN)(
    _In_ _Pre_defensive_ HQUIC Configuration, _In_ _Pre_defensive_ const QUIC_CREDENTIAL_CONFIG* CredConfig);
...
typedef _When_(Event->Type != QUIC_LISTENER_EVENT_DOS_MODE_CHANGED, _IRQL_requires_max_(PASSIVE_LEVEL))
    _When_(Event->Type == QUIC_LISTENER_EVENT_DOS_MODE_CHANGED, _IRQL_requires_max_(DISPATCH_LEVEL))
        _Function_class_(QUIC_LISTENER_CALLBACK) QUIC_STATUS(QUIC_API QUIC_LISTENER_CALLBACK)(
            _In_ HQUIC Listener, _In_opt_ void* Context, _Inout_ QUIC_LISTENER_EVENT* Event);
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_LISTENER_OPEN_FN)(_In_ _Pre_defensive_ HQUIC Registration,
        _In_ _Pre_defensive_ QUIC_LISTENER_CALLBACK_HANDLER Handler,
        _In_opt_ void* Context,
        _Outptr_ _At_(*Listener, __drv_allocatesMem(Mem)) _Pre_defensive_ HQUIC* Listener);

//
// Closes an existing listener.
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(QUIC_API* QUIC_LISTENER_CLOSE_FN)(
    _In_ _Pre_defensive_ __drv_freesMem(Mem) HQUIC Listener);

//
// Starts the listener processing incoming connections.
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_LISTENER_START_FN)(_In_ _Pre_defensive_ HQUIC Listener,
        _In_reads_(AlpnBufferCount) _Pre_defensive_ const QUIC_BUFFER* const AlpnBuffers,
        _In_range_(>, 0) uint32_t AlpnBufferCount,
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(QUIC_API* QUIC_LISTENER_STOP_FN)(_In_ _Pre_defensive_ HQUIC Listener);
...
typedef enum QUIC_CONNECTION_EVENT_TYPE {
    ...
    QUIC_CONNECTION_EVENT_RELIABLE_RESET_NEGOTIATED =
        16, // Only indicated if a nonzero RESET_STREAM_AT dialect mask was requested.
...
typedef struct QUIC_CONNECTION_EVENT {
    ...
    union {
        ...
        struct {
            ...
            _Field_size_(ResumptionTicketLength) const uint8_t* ResumptionTicket;
        } RESUMPTION_TICKET_RECEIVED;
        struct {
            QUIC_CERTIFICATE* Certificate; // Peer certificate (platform specific). Valid only during
                                           // QUIC_CONNECTION_EVENT_PEER_CERTIFICATE_RECEIVED callback.
            uint32_t DeferredErrorFlags;   // Bit flag of errors (only valid with
                                           // QUIC_CREDENTIAL_FLAG_DEFER_CERTIFICATE_VALIDATION) - Schannel only, zero
                                           // otherwise.
            QUIC_STATUS DeferredStatus;    // Most severe error status (only valid with
                                           // QUIC_CREDENTIAL_FLAG_DEFER_CERTIFICATE_VALIDATION)
            QUIC_CERTIFICATE_CHAIN* Chain; // Peer certificate chain (platform specific). Valid only during
                                           // QUIC_CONNECTION_EVENT_PEER_CERTIFICATE_RECEIVED callback.
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL) _Function_class_(QUIC_CONNECTION_CALLBACK)
    QUIC_STATUS(QUIC_API QUIC_CONNECTION_CALLBACK)(
        _In_ HQUIC Connection, _In_opt_ void* Context, _Inout_ QUIC_CONNECTION_EVENT* Event);
...
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONNECTION_OPEN_FN)(_In_ _Pre_defensive_ HQUIC Registration,
        _In_ _Pre_defensive_ QUIC_CONNECTION_CALLBACK_HANDLER Handler,
        _In_opt_ void* Context,
        _Outptr_ _At_(*Connection, __drv_allocatesMem(Mem)) _Pre_defensive_ HQUIC* Connection);
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONNECTION_OPEN_IN_PARTITION_FN)(_In_ _Pre_defensive_ HQUIC Registration,
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(QUIC_API* QUIC_CONNECTION_CLOSE_FN)(
    _In_ _Pre_defensive_ __drv_freesMem(Mem) HQUIC Connection);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL) void(QUIC_API* QUIC_CONNECTION_SHUTDOWN_FN)(
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONNECTION_START_FN)(_In_ _Pre_defensive_ HQUIC Connection,
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS(QUIC_API* QUIC_CONNECTION_SET_CONFIGURATION_FN)(
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONNECTION_SEND_RESUMPTION_FN)(_In_ _Pre_defensive_ HQUIC Connection,
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS(QUIC_API* QUIC_CONNECTION_COMP_RESUMPTION_FN)(
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS(QUIC_API* QUIC_CONNECTION_COMP_CERT_FN)(
    _In_ _Pre_defensive_ HQUIC Connection, _In_ BOOLEAN Result, _In_ QUIC_TLS_ALERT_CODES TlsAlert);
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_CONNECTION_EXPORT_KEYING_MATERIAL_FN)(_In_ _Pre_defensive_ HQUIC Connection,
        _In_ _Pre_defensive_ const QUIC_KEYING_MATERIAL_CONFIG* Config,
        _Out_writes_bytes_(Config->OutputLength) uint8_t* Output);
#endif // QUIC_API_ENABLE_PREVIEW_FEATURES
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL) _Function_class_(QUIC_STREAM_CALLBACK) QUIC_STATUS(
    QUIC_API QUIC_STREAM_CALLBACK)(_In_ HQUIC Stream, _In_opt_ void* Context, _Inout_ QUIC_STREAM_EVENT* Event);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_STREAM_OPEN_FN)(_In_ _Pre_defensive_ HQUIC Connection,
        _In_ QUIC_STREAM_OPEN_FLAGS Flags,
        _In_ _Pre_defensive_ QUIC_STREAM_CALLBACK_HANDLER Handler,
        _In_opt_ void* Context,
        _Outptr_ _At_(*Stream, __drv_allocatesMem(Mem)) _Pre_defensive_ HQUIC* Stream);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_STREAM_START_FN)(_In_ _Pre_defensive_ HQUIC Stream, _In_ QUIC_STREAM_START_FLAGS Flags);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_STREAM_SHUTDOWN_FN)(_In_ _Pre_defensive_ HQUIC Stream,
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_STREAM_SEND_FN)(_In_ _Pre_defensive_ HQUIC Stream,
        _In_reads_(BufferCount) _Pre_defensive_ const QUIC_BUFFER* const Buffers,
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS(QUIC_API* QUIC_STREAM_RECEIVE_SET_ENABLED_FN)(
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS(QUIC_API* QUIC_STREAM_PROVIDE_RECEIVE_BUFFERS_FN)(
    _In_ _Pre_defensive_ HQUIC Stream, _In_ uint32_t BufferCount, _In_reads_(BufferCount) const QUIC_BUFFER* Buffers);
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    QUIC_STATUS(QUIC_API* QUIC_DATAGRAM_SEND_FN)(_In_ _Pre_defensive_ HQUIC Connection,
        _In_reads_(BufferCount) _Pre_defensive_ const QUIC_BUFFER* const Buffers,
        _In_ uint32_t BufferCount,
        _In_ QUIC_SEND_FLAGS Flags,
        _In_opt_ void* ClientSendContext);
...
typedef struct QUIC_CONNECTION_POOL_CONFIG {
    ...
    _Field_size_opt_(NumberOfConnections) void** Context; // Optional
    ...
    _At_buffer_(_Curr_, _Iter_, NumberOfConnections, _Field_size_(CibirIdLength))
        _Field_size_opt_(NumberOfConnections) uint8_t** CibirIds; // Optional
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS(QUIC_API* QUIC_CONN_POOL_CREATE_FN)(
    _In_ QUIC_CONNECTION_POOL_CONFIG* Config, _Out_writes_(Config->NumberOfConnections) HQUIC* ConnectionPool);
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Check_return_
    ...
    [[nodiscard]]
#endif
    QUIC_STATUS QUIC_API MsQuicOpenVersion(_In_ uint32_t Version, _Out_ _Pre_defensive_ const void** QuicApi);

//
// Cleans up the function table returned from MsQuicOpenVersion and releases the
// reference on the API.
//
_IRQL_requires_max_(PASSIVE_LEVEL) void QUIC_API MsQuicClose(_In_ _Pre_defensive_ const void* QuicApi);

#endif

_IRQL_requires_max_(PASSIVE_LEVEL) _Check_return_ typedef QUIC_STATUS(QUIC_API* MsQuicOpenVersionFn)(
    _In_ uint32_t Version, _Out_ _Pre_defensive_ const void** QuicApi);

_IRQL_requires_max_(PASSIVE_LEVEL) typedef void(QUIC_API* MsQuicCloseFn)(_In_ _Pre_defensive_ const void* QuicApi);

#ifdef _KERNEL_MODE

DECLSPEC_SELECTANY GUID MSQUIC_NPI_ID = {0xC43138E3,
    0xCD13,
    0x4CB1,
    {0x9C, 0xAE, 0xE0, 0x05, 0xC8, 0x55, 0x7A, 0xBA}}; // C43138E3-CD13-4CB1-9CAE-E005C8557ABA

DECLSPEC_SELECTANY GUID MSQUIC_MODULE_ID = {0x698F7C72,
    0xC2E6,
    0x49CD,
    {0x8C, 0x39, 0x98, 0x85, 0x1D, 0x50, 0x19, 0x01}}; // 698F7C72-C2E6-49CD-8C39-98851D501901
...
static NTSTATUS __MsQuicClientAttachProvider(_In_ HANDLE NmrBindingHandle,
...
#pragma warning(suppress : 6387) // _Param_(2) could be '0' - by design.
        Status = NmrClientAttachProvider(
            NmrBindingHandle, Client, NULL, &ProviderContext, (const void**)&Client->ProviderDispatch);
...
_IRQL_requires_max_(PASSIVE_LEVEL) __forceinline void MsQuicNmrClientDeregister(_Inout_ HANDLE* ClientHandle) {
...
_IRQL_requires_max_(PASSIVE_LEVEL) __forceinline NTSTATUS MsQuicNmrClientRegister(_Out_ HANDLE* ClientHandle,
    ...
    __MSQUIC_NMR_CLIENT* Client = (__MSQUIC_NMR_CLIENT*)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(*Client), 'cNQM');
    if (Client == NULL) {
    ...
    Status = NmrRegisterClient(&Client->NpiClientCharacteristics, Client, &Client->NmrClientHandle);
    if (!NT_SUCCESS(Status)) {
        goto Exit;
    }
    ...
    Status = KeWaitForSingleObject(&Client->RegistrationCompleteEvent, Executive, KernelMode, FALSE, &Timeout);
...
#endif
    QUIC_STATUS MsQuicOpen2(_Out_ _Pre_defensive_ const QUIC_API_TABLE** QuicApi) {
