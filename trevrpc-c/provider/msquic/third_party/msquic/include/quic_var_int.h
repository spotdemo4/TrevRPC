...
#define QuicVarIntSize(Value)                                                                                          \
    ((QUIC_VAR_INT)Value < 0x40 ? sizeof(uint8_t)                                                                      \
                                : ((QUIC_VAR_INT)Value < 0x4000                                                        \
                                          ? sizeof(uint16_t)                                                           \
                                          : ((QUIC_VAR_INT)Value < 0x40000000 ? sizeof(uint32_t) : sizeof(uint64_t))))
...
_When_(Value < 0x40, _Post_equal_to_(Buffer + sizeof(uint8_t)))
    _When_(Value >= 0x40 && Value < 0x4000, _Post_equal_to_(Buffer + sizeof(uint16_t)))
        _When_(Value >= 0x4000 && Value < 0x40000000, _Post_equal_to_(Buffer + sizeof(uint32_t)))
            _When_(Value >= 0x40000000, _Post_equal_to_(Buffer + sizeof(uint64_t))) uint8_t* QuicVarIntEncode(
                ...
                _When_(Value < 0x40, _Out_writes_bytes_(sizeof(uint8_t)))
                    _When_(Value >= 0x40 && Value < 0x4000, _Out_writes_bytes_(sizeof(uint16_t)))
                        _When_(Value >= 0x4000 && Value < 0x40000000, _Out_writes_bytes_(sizeof(uint32_t)))
                            _When_(Value >= 0x40000000, _Out_writes_bytes_(sizeof(uint64_t))) uint8_t* Buffer) {
...
//
QUIC_INLINE
_Post_equal_to_(Buffer + sizeof(uint16_t)) uint8_t* QuicVarIntEncode2Bytes(
    _In_range_(0, 0x3FFF) QUIC_VAR_INT Value, _Out_writes_bytes_(sizeof(uint16_t)) uint8_t* Buffer) {
...
//
QUIC_INLINE
_Success_(return != FALSE) BOOLEAN QuicVarIntDecode(_In_ uint16_t BufferLength,
    _In_reads_bytes_(BufferLength) const uint8_t* const Buffer,
    _Inout_ _Deref_in_range_(0, BufferLength) _Deref_out_range_(0, BufferLength) uint16_t* Offset,
