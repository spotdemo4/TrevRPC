...
CXPLAT_STATIC_ASSERT((uint32_t)CXPLAT_AEAD_AES_128_GCM == (uint32_t)QUIC_AEAD_ALGORITHM_AES_128_GCM &&
...
QUIC_INLINE
uint16_t CxPlatKeyLength(CXPLAT_AEAD_TYPE Type) {
    switch (Type) {
...
QUIC_INLINE
uint16_t CxPlatHashLength(CXPLAT_HASH_TYPE Type) {
    switch (Type) {
...
//
_IRQL_requires_max_(DISPATCH_LEVEL) _When_(ReadKey != NULL, _At_(*ReadKey, __drv_allocatesMem(Mem)))
    _When_(WriteKey != NULL, _At_(*WriteKey, __drv_allocatesMem(Mem))) QUIC_STATUS
    QuicPacketKeyCreateInitial(_In_ BOOLEAN IsServer,
        _In_ const QUIC_HKDF_LABELS* HkdfLabels,
        _In_reads_(CXPLAT_VERSION_SALT_LENGTH) const uint8_t* const Salt, // Version Specific
...
//
_IRQL_requires_max_(DISPATCH_LEVEL) void QuicPacketKeyFree(_In_opt_ __drv_freesMem(Mem) QUIC_PACKET_KEY* Key);

//
// Calculates the updated packet key from the current packet key.
//
_IRQL_requires_max_(DISPATCH_LEVEL) _At_(*NewKey, __drv_allocatesMem(Mem)) QUIC_STATUS QuicPacketKeyUpdate(
    _In_ const QUIC_HKDF_LABELS* HkdfLabels, _In_ QUIC_PACKET_KEY* OldKey, _Out_ QUIC_PACKET_KEY** NewKey);

_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS CxPlatKeyCreate(_In_ CXPLAT_AEAD_TYPE AeadType,
    _When_(AeadType == CXPLAT_AEAD_AES_128_GCM, _In_reads_(16)) _When_(AeadType == CXPLAT_AEAD_AES_256_GCM,
        _In_reads_(32)) _When_(AeadType == CXPLAT_AEAD_CHACHA20_POLY1305, _In_reads_(32)) const uint8_t* const RawKey,
    _Out_ CXPLAT_KEY** Key);

_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QuicPacketKeyDerive(_In_ QUIC_PACKET_KEY_TYPE KeyType,
    _In_ const QUIC_HKDF_LABELS* HkdfLabels,
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QuicPacketKeyDeriveOffload(_In_ const QUIC_HKDF_LABELS* HkdfLabels,
    _In_ const QUIC_PACKET_KEY* const PacketKey,
    _In_z_ const char* const SecretName,
    _Inout_ CXPLAT_QEO_CONNECTION* Offload);

_IRQL_requires_max_(DISPATCH_LEVEL) void CxPlatKeyFree(_In_opt_ CXPLAT_KEY* Key);

CXPLAT_STATIC_ASSERT(sizeof(uint64_t) < CXPLAT_IV_LENGTH, "Packet Number Length is less than IV Length");

QUIC_INLINE
void QuicCryptoCombineIvAndPacketNumber(_In_reads_bytes_(CXPLAT_IV_LENGTH) const uint8_t* const IvIn,
    _In_reads_bytes_(sizeof(uint64_t)) const uint8_t* const PacketNumber,
    _Out_writes_bytes_(CXPLAT_IV_LENGTH) uint8_t* IvOut) {
    //
...
//
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS CxPlatEncrypt(_In_ CXPLAT_KEY* Key,
    ...
    _When_(BufferLength > CXPLAT_ENCRYPTION_OVERHEAD, _Inout_updates_bytes_(BufferLength))
        _When_(BufferLength <= CXPLAT_ENCRYPTION_OVERHEAD, _Out_writes_bytes_(BufferLength)) uint8_t* Buffer);
...
// CXPLAT_ENCRYPTION_OVERHEAD.
//
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS CxPlatDecrypt(_In_ CXPLAT_KEY* Key,
    _In_reads_bytes_(CXPLAT_IV_LENGTH) const uint8_t* const Iv,
    _In_ uint16_t AuthDataLength,
    _In_reads_bytes_opt_(AuthDataLength) const uint8_t* const AuthData,
    _In_ uint16_t BufferLength,
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS CxPlatHpKeyCreate(_In_ CXPLAT_AEAD_TYPE AeadType,
    _When_(AeadType == CXPLAT_AEAD_AES_128_GCM, _In_reads_(16)) _When_(AeadType == CXPLAT_AEAD_AES_256_GCM,
        _In_reads_(32)) _When_(AeadType == CXPLAT_AEAD_CHACHA20_POLY1305, _In_reads_(32)) const uint8_t* const RawKey,
    _Out_ CXPLAT_HP_KEY** Key);

_IRQL_requires_max_(DISPATCH_LEVEL) void CxPlatHpKeyFree(_In_opt_ CXPLAT_HP_KEY* Key);

//
// Calculates the header protection mask, to be XOR'ed with the QUIC packet
// header.
//
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS CxPlatHpComputeMask(_In_ CXPLAT_HP_KEY* Key,
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS CxPlatHashCreate(_In_ CXPLAT_HASH_TYPE HashType,
    _In_reads_(SaltLength) const uint8_t* const Salt,
    _In_ uint32_t SaltLength,
    _Out_ CXPLAT_HASH** Hash);

_IRQL_requires_max_(DISPATCH_LEVEL) void CxPlatHashFree(_In_opt_ CXPLAT_HASH* Hash);

_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS CxPlatHashCompute(_In_ CXPLAT_HASH* Hash,
...
BOOLEAN
CxPlatCryptSupports(CXPLAT_AEAD_TYPE AeadType);

_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatKbKdfDerive(_In_reads_(SecretLength) const uint8_t* Secret,
