...
//
void CxPlatToeplitzHashInitialize(_Inout_ CXPLAT_TOEPLITZ_HASH* Toeplitz);
...
//
uint32_t CxPlatToeplitzHashCompute(_In_ const CXPLAT_TOEPLITZ_HASH* Toeplitz,
...
//
QUIC_INLINE
void CxPlatToeplitzHashComputeAddr(_In_ const CXPLAT_TOEPLITZ_HASH* Toeplitz,
    _In_ const QUIC_ADDR* Addr,
    _Inout_ uint32_t* Key,
    _Out_ uint32_t* Offset) {
    if (QuicAddrGetFamily(Addr) == QUIC_ADDRESS_FAMILY_INET) {
        *Key ^= CxPlatToeplitzHashCompute(Toeplitz, ((uint8_t*)Addr) + QUIC_ADDR_V4_PORT_OFFSET, 2, 0);
        *Key ^= CxPlatToeplitzHashCompute(Toeplitz, ((uint8_t*)Addr) + QUIC_ADDR_V4_IP_OFFSET, 4, 2);
        *Offset = 2 + 4;
    } else {
        *Key ^= CxPlatToeplitzHashCompute(Toeplitz, ((uint8_t*)Addr) + QUIC_ADDR_V6_PORT_OFFSET, 2, 0);
        *Key ^= CxPlatToeplitzHashCompute(Toeplitz, ((uint8_t*)Addr) + QUIC_ADDR_V6_IP_OFFSET, 16, 2);
        *Offset = 2 + 16;
    }
}
...
//
QUIC_INLINE
void CxPlatToeplitzHashComputeRss(_In_ const CXPLAT_TOEPLITZ_HASH* Toeplitz,
    ...
    if (QuicAddrGetFamily(SrcAddr) == QUIC_ADDRESS_FAMILY_INET) {
        *Key ^= CxPlatToeplitzHashCompute(Toeplitz, ((uint8_t*)SrcAddr) + QUIC_ADDR_V4_IP_OFFSET, 4, 0);
        *Key ^= CxPlatToeplitzHashCompute(Toeplitz, ((uint8_t*)DestAddr) + QUIC_ADDR_V4_IP_OFFSET, 4, 4);
        *Key ^= CxPlatToeplitzHashCompute(Toeplitz, ((uint8_t*)SrcAddr) + QUIC_ADDR_V4_PORT_OFFSET, 2, 8);
        *Key ^= CxPlatToeplitzHashCompute(Toeplitz, ((uint8_t*)DestAddr) + QUIC_ADDR_V4_PORT_OFFSET, 2, 10);
        *Offset = 4 + 4 + 2 + 2;
    } else {
        CXPLAT_DBG_ASSERT(QuicAddrGetFamily(SrcAddr) == QUIC_ADDRESS_FAMILY_INET6);

        *Key ^= CxPlatToeplitzHashCompute(Toeplitz, ((uint8_t*)SrcAddr) + QUIC_ADDR_V6_IP_OFFSET, 16, 0);
        *Key ^= CxPlatToeplitzHashCompute(Toeplitz, ((uint8_t*)DestAddr) + QUIC_ADDR_V6_IP_OFFSET, 16, 16);
        *Key ^= CxPlatToeplitzHashCompute(Toeplitz, ((uint8_t*)SrcAddr) + QUIC_ADDR_V6_PORT_OFFSET, 2, 32);
        *Key ^= CxPlatToeplitzHashCompute(Toeplitz, ((uint8_t*)DestAddr) + QUIC_ADDR_V6_PORT_OFFSET, 2, 34);
        *Offset = 16 + 16 + 2 + 2;
    }
}
