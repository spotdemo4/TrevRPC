...
// via OS sockets.
//
typedef void (*QUIC_FUZZ_SEND_CALLBACK_FN)(
    _Inout_ void* CallbackContext, _Inout_updates_bytes_(Length) uint8_t* Buffer, _In_ DWORD Length);

//
// Callback to be registered and called each time msquic receives a packet.
// In 'Simulated' mode this is still called.
//
typedef void (*QUIC_FUZZ_RECV_CALLBACK_FN)(
    _Inout_ void* CallbackContext, _Inout_updates_bytes_(Length) uint8_t* Buffer, _In_ DWORD Length);
...
// datagram's data.
//
typedef void (*QUIC_FUZZ_INJECT_CALLBACK_FN)(_Inout_ void* CallbackContext,
...
//
typedef void (*QUIC_FUZZ_ENCRYPT_CALLBACK_FN)(
    _Inout_ void* CallbackContext, _Inout_updates_bytes_(Length) uint8_t* Buffer, _In_ DWORD Length);
...
//
void MsQuicFuzzInit(_Inout_ void* CallbackContext, _In_ uint8_t RedirectDataPath);

//
// Sets callback to be invoked each time msquic_fuzz sends a datagram.
//
void MsQuicFuzzRegisterSendCallback(_In_ QUIC_FUZZ_SEND_CALLBACK_FN Callback);

//
// Sets callback to be invoked each time msquic_fuzz receives a datagram.
//
void MsQuicFuzzRegisterRecvCallback(_In_ QUIC_FUZZ_RECV_CALLBACK_FN Callback);

//
// Sets callback to be invoked each time msquic_fuzz creates a new datagram.
// to be sent.
//
void MsQuicFuzzRegisterInjectCallback(_In_ QUIC_FUZZ_INJECT_CALLBACK_FN Callback);

//
// Sets callback to be invoked each time msquic_fuzz encrypts a datagram.
//
void MsQuicFuzzRegisterEncryptCallback(_In_ QUIC_FUZZ_ENCRYPT_CALLBACK_FN Callback);

//
// When operating in 'Simulate' mode, can be called to deliver a datagram.
// to the last-used connection in an msquic_fuzz session.
//
void MsQuicFuzzSimulateReceive(
    _In_ const QUIC_ADDR* SourceAddress, _In_reads_(PacketLength) uint8_t* PacketData, _In_ uint16_t PacketLength);
