...
_Must_inspect_result_ _Success_(return != FALSE) BOOLEAN CxPlatHashtableInitialize(
...
QUIC_INLINE
_Must_inspect_result_ _Success_(return != FALSE) BOOLEAN
    CxPlatHashtableInitializeEx(_Inout_ CXPLAT_HASHTABLE* HashTable, _In_ uint32_t InitialSize) {
    return CxPlatHashtableInitialize(&HashTable, InitialSize);
}

void CxPlatHashtableUninitialize(
    _In_ _When_((HashTable->Flags & CXPLAT_HASH_ALLOCATED_HEADER), __drv_freesMem(Mem) _Post_invalid_)
        _At_(HashTable->Directory, __drv_freesMem(Mem) _Post_invalid_) CXPLAT_HASHTABLE* HashTable);

void CxPlatHashtableInsert(_In_ CXPLAT_HASHTABLE* HashTable,
...
void CxPlatHashtableRemove(_In_ CXPLAT_HASHTABLE* HashTable,
    _In_ CXPLAT_HASHTABLE_ENTRY* Entry,
    _Inout_opt_ CXPLAT_HASHTABLE_LOOKUP_CONTEXT* Context);

_Must_inspect_result_ CXPLAT_HASHTABLE_ENTRY* CxPlatHashtableLookup(_In_ const CXPLAT_HASHTABLE* HashTable,
    _In_ uint64_t Signature,
    _Out_opt_ CXPLAT_HASHTABLE_LOOKUP_CONTEXT* Context);

_Must_inspect_result_ CXPLAT_HASHTABLE_ENTRY* CxPlatHashtableLookupNext(
    _In_ const CXPLAT_HASHTABLE* HashTable, _Inout_ CXPLAT_HASHTABLE_LOOKUP_CONTEXT* Context);

void CxPlatHashtableEnumerateBegin(_In_ CXPLAT_HASHTABLE* HashTable, _Out_ CXPLAT_HASHTABLE_ENUMERATOR* Enumerator);

_Must_inspect_result_ CXPLAT_HASHTABLE_ENTRY* CxPlatHashtableEnumerateNext(
    _In_ CXPLAT_HASHTABLE* HashTable, _Inout_ CXPLAT_HASHTABLE_ENUMERATOR* Enumerator);

void CxPlatHashtableEnumerateEnd(_In_ CXPLAT_HASHTABLE* HashTable, _Inout_ CXPLAT_HASHTABLE_ENUMERATOR* Enumerator);
...
uint32_t CxPlatHashSimple(_In_ uint16_t Length, _In_reads_(Length) const uint8_t* const Buffer) {
