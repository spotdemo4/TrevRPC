...
//
QUIC_INLINE
_Null_terminated_ const char* QuicStatusToString(_In_ QUIC_STATUS Status) {
    switch (Status) {
...
//
QUIC_INLINE
uint32_t GetConnRtt(_In_ const QUIC_API_TABLE* MsQuicTable, _In_ HQUIC Handle) {
    QUIC_STATISTICS Value;
    uint32_t ValueSize = sizeof(Value);
    MsQuicTable->GetParam(Handle, QUIC_PARAM_CONN_STATISTICS, &ValueSize, &Value);
    return Value.Rtt;
}
...
//
QUIC_INLINE
uint64_t GetStreamID(_In_ const QUIC_API_TABLE* MsQuicTable, _In_ HQUIC Handle) {
    uint64_t ID = (uint32_t)(-1);
    uint32_t IDLen = sizeof(ID);
    MsQuicTable->GetParam(Handle, QUIC_PARAM_STREAM_ID, &IDLen, &ID);
    return ID;
}
...
QUIC_INLINE
QUIC_ADDR_STR
GetRemoteAddr(_In_ const QUIC_API_TABLE* MsQuicTable, _In_ HQUIC Handle) {
    QUIC_ADDR addr;
    uint32_t addrLen = sizeof(addr);
    QUIC_ADDR_STR addrStr = {0};
    QUIC_STATUS status = MsQuicTable->GetParam(Handle, QUIC_PARAM_CONN_REMOTE_ADDRESS, &addrLen, &addr);
...
QUIC_INLINE
QUIC_STATUS
QuicForceRetry(_In_ const QUIC_API_TABLE* MsQuicTable, _In_ BOOLEAN Enabled) {
    uint16_t value = Enabled ? 0 : 65;
    return MsQuicTable->SetParam(NULL, QUIC_PARAM_GLOBAL_RETRY_MEMORY_PERCENT, sizeof(value), &value);
}

QUIC_INLINE
void DumpMsQuicPerfCounters(_In_ const QUIC_API_TABLE* MsQuicTable) {
    uint64_t Counters[QUIC_PERF_COUNTER_MAX] = {0};
    uint32_t Lenth = sizeof(Counters);
    MsQuicTable->GetParam(NULL, QUIC_PARAM_GLOBAL_PERF_COUNTERS, &Lenth, &Counters);
...
QUIC_INLINE
BOOLEAN
ConvertArgToAddress(_In_z_ const char* Arg,
...
void EncodeHexBuffer(
    _In_reads_(BufferLen) uint8_t* Buffer, _In_ uint8_t BufferLen, _Out_writes_bytes_(2 * BufferLen) char* HexString) {
...
QUIC_INLINE
bool IsArg(_In_z_ const char* Arg, _In_z_ const char* toTestAgainst) {
    return Arg[0] && (_strnicmp(Arg + 1, toTestAgainst, strlen(toTestAgainst)) == 0);
}

QUIC_INLINE
bool IsValue(_In_z_ const char* name, _In_z_ const char* toTestAgainst) {
    return _strnicmp(name, toTestAgainst, strlen(toTestAgainst)) == 0;
}

QUIC_INLINE
bool GetFlag(_In_ int argc, _In_reads_(argc) _Null_terminated_ char* argv[], _In_z_ const char* name) {
...
_Ret_maybenull_ _Null_terminated_ const char* GetValue(
    _In_ int argc, _In_reads_(argc) _Null_terminated_ char* argv[], _In_z_ const char* name) {
    const size_t nameLen = strlen(name);
    for (int i = 0; i < argc; i++) {
...
QUIC_INLINE
_Success_(return != false) bool TryGetValue(_In_ int argc,
...
QUIC_INLINE
_Success_(return != false) bool TryGetValue(
    _In_ int argc, _In_reads_(argc) _Null_terminated_ char* argv[], _In_z_ const char* name, _Out_ uint8_t* pValue) {
    auto value = GetValue(argc, argv, name);
    if (!value)
        return false;
...
QUIC_INLINE
_Success_(return != false) bool TryGetValue(
    _In_ int argc, _In_reads_(argc) _Null_terminated_ char* argv[], _In_z_ const char* name, _Out_ uint16_t* pValue) {
    auto value = GetValue(argc, argv, name);
    if (!value)
        return false;
...
QUIC_INLINE
_Success_(return != false) bool TryGetValue(
    _In_ int argc, _In_reads_(argc) _Null_terminated_ char* argv[], _In_z_ const char* name, _Out_ uint32_t* pValue) {
    auto value = GetValue(argc, argv, name);
    if (!value)
        return false;
    char* End;
...
QUIC_INLINE
_Success_(return != false) bool TryGetValue(
    _In_ int argc, _In_reads_(argc) _Null_terminated_ char* argv[], _In_z_ const char* name, _Out_ int32_t* pValue) {
    auto value = GetValue(argc, argv, name);
    if (!value)
        return false;
...
QUIC_INLINE
_Success_(return != false) bool TryGetValue(
    _In_ int argc, _In_reads_(argc) _Null_terminated_ char* argv[], _In_z_ const char* name, _Out_ uint64_t* pValue) {
    auto value = GetValue(argc, argv, name);
    if (!value)
        return false;
    char* End;
...
QUIC_INLINE
_Success_(return != false) HQUIC GetServerConfigurationFromArgs(_In_ int argc,
    _In_reads_(argc) _Null_terminated_ char* argv[],
    ...
    if (((Cert = GetValue(argc, argv, "thumbprint")) != nullptr) ||
        ((Cert = GetValue(argc, argv, "cert_hash")) != nullptr) || ((Cert = GetValue(argc, argv, "hash")) != nullptr)) {
        uint32_t CertHashLen =
            DecodeHexBuffer(Cert, sizeof(Helper.CertHashStore.ShaHash), Helper.CertHashStore.ShaHash);
        ...
        Helper.CertHashStore.Flags = GetValue(argc, argv, "machine") ? QUIC_CERTIFICATE_HASH_STORE_FLAG_MACHINE_STORE
                                                                     : QUIC_CERTIFICATE_HASH_STORE_FLAG_NONE;

    } else if ((((Cert = GetValue(argc, argv, "file")) != nullptr) &&
    ...
    if (QUIC_SUCCEEDED(MsQuicTable->ConfigurationOpen(
            Registration, AlpnBuffers, AlpnBufferCount, Settings, SettingsSize, Context, &Configuration)) &&
        QUIC_FAILED(MsQuicTable->ConfigurationLoadCredential(Configuration, Config))) {
...
QUIC_INLINE
void FreeServerConfiguration(_In_ const QUIC_API_TABLE* MsQuicTable, _In_ HQUIC Configuration) {
#ifdef QUIC_TEST_APIS
...
#ifdef _KERNEL_MODE
QUIC_INLINE
void WriteSslKeyLogFileKernelMode(_In_z_ const char* FileName, _In_ QUIC_TLS_SECRETS& TlsSecrets) {
    ...
    Status = RtlUTF8ToUnicodeN(
        ConvertedFileName, sizeof(ConvertedFileName), &StringLengthBytes, FileName, (ULONG)FileNameLength);
    ...
    InitializeObjectAttributes(
        &ObjectAttributes, &FileNameString, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
    ...
    Status = ZwCreateFile(&Handle,
        ...
        0);
    if (!NT_SUCCESS(Status)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Open sslkeylogfile for append");
        goto Error;
    }

    if (IoStatusBlock.Information == FILE_CREATED) {
        CHAR Header[] = "# TLS 1.3 secrets log file, generated by quicinterop\n";
        Status = ZwWriteFile(Handle, NULL, NULL, NULL, &IoStatusBlock, Header, sizeof(Header) - 1, NULL, NULL);
        if (!NT_SUCCESS(Status)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Write header to sslkeylogfile");
            goto WriteError;
        }
    }
    ...
    if (TlsSecrets.IsSet.ClientEarlyTrafficSecret) {
        ...
        Status = RtlStringCbPrintfExA(TempLogBuffer,
            ...
            TempHexBuffer);
        if (!NT_SUCCESS(Status)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Format CLIENT_EARLY_TRAFFIC_SECRET");
            goto WriteError;
        }
        ...
        Status = ZwWriteFile(Handle,
            ...
            NULL);
        if (!NT_SUCCESS(Status)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Write CLIENT_EARLY_TRAFFIC_SECRET");
            goto WriteError;
        }
    }
    ...
    if (TlsSecrets.IsSet.ClientHandshakeTrafficSecret) {
        ...
        if (!NT_SUCCESS(Status)) {
            QuicTraceEvent(
                LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Format CLIENT_HANDSHAKE_TRAFFIC_SECRET");
            goto WriteError;
        }
        ...
        if (!NT_SUCCESS(Status)) {
            QuicTraceEvent(
                LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Write CLIENT_HANDSHAKE_TRAFFIC_SECRET");
            goto WriteError;
        }
    }
    ...
    if (TlsSecrets.IsSet.ServerHandshakeTrafficSecret) {
        ...
        if (!NT_SUCCESS(Status)) {
            QuicTraceEvent(
                LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Format SERVER_HANDSHAKE_TRAFFIC_SECRET");
            goto WriteError;
        }
        ...
        if (!NT_SUCCESS(Status)) {
            QuicTraceEvent(
                LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Write SERVER_HANDSHAKE_TRAFFIC_SECRET");
            goto WriteError;
        }
    }
    ...
    if (TlsSecrets.IsSet.ClientTrafficSecret0) {
        ...
        Status = RtlStringCbPrintfExA(TempLogBuffer,
            ...
            TempHexBuffer);
        if (!NT_SUCCESS(Status)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Format CLIENT_TRAFFIC_SECRET_0");
            goto WriteError;
        }
        ...
        Status = ZwWriteFile(Handle,
            ...
            NULL);
        if (!NT_SUCCESS(Status)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Write CLIENT_TRAFFIC_SECRET_0");
            goto WriteError;
        }
    }
    ...
    if (TlsSecrets.IsSet.ServerTrafficSecret0) {
        ...
        Status = RtlStringCbPrintfExA(TempLogBuffer,
            ...
            TempHexBuffer);
        if (!NT_SUCCESS(Status)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Format SERVER_TRAFFIC_SECRET_0");
            goto WriteError;
        }
        ...
        Status = ZwWriteFile(Handle,
            ...
            NULL);
        if (!NT_SUCCESS(Status)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Write SERVER_TRAFFIC_SECRET_0");
            goto WriteError;
        }
    }
...
QUIC_INLINE
void WriteSslKeyLogFileUserMode(_In_z_ const char* FileName, _In_ QUIC_TLS_SECRETS& TlsSecrets) {
    FILE* File = nullptr;
#ifdef _WIN32
...
QUIC_INLINE
void WriteSslKeyLogFile(_In_z_ const char* FileName, _In_ QUIC_TLS_SECRETS& TlsSecrets) {
#ifdef _KERNEL_MODE
...
struct StrBuffer {
    ...
    StrBuffer(const char* HexBytes) {
        ...
        for (uint16_t i = 0; i < Length; ++i) {
            Data[i] = (DecodeHexChar(HexBytes[i * 2]) << 4) | DecodeHexChar(HexBytes[i * 2 + 1]);
        }
    }
