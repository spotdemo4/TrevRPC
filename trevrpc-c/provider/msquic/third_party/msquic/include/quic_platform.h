...
#define CXPLAT_STRUCT_SIZE_THRU_FIELD(Type, Field) (offsetof(Type, Field) + sizeof(((Type*)0)->Field))

#define CXPLAT_STRUCT_HAS_FIELD(Type, Size, Field) (Size >= CXPLAT_STRUCT_SIZE_THRU_FIELD(Type, Field))
...
//
QUIC_INLINE
uint64_t CxPlatEwma(_In_ uint64_t Average, _In_ uint64_t Sample, _In_ uint64_t Weight) {
...
//
PAGEDX
_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatSystemLoad(void);

//
// Called in main (exit), DLLMain or DriverUnload.
//
PAGEDX
_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatSystemUnload(void);
...
//
PAGEDX
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatInitialize(void);
...
//
PAGEDX
_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatUninitialize(void);
...
#define QuicListEntryValidate(Entry)                                                                                   \
    CXPLAT_DBG_ASSERT((((Entry->Flink)->Blink) == Entry) && (((Entry->Blink)->Flink) == Entry))

FORCEINLINE
void CxPlatListInitializeHead(_Out_ CXPLAT_LIST_ENTRY* ListHead) {
    ListHead->Flink = ListHead->Blink = ListHead;
}

_Must_inspect_result_ FORCEINLINE BOOLEAN CxPlatListIsEmpty(_In_ const CXPLAT_LIST_ENTRY* ListHead) {
    CXPLAT_DBG_ASSERT(ListHead->Flink != NULL);
    return (BOOLEAN)(ListHead->Flink == ListHead);
}

_Must_inspect_result_ FORCEINLINE BOOLEAN CxPlatListIsEmptyNoFence(_In_ const CXPLAT_LIST_ENTRY* ListHead) {
    return (BOOLEAN)(QuicReadPtrNoFence((void**)&ListHead->Flink) == ListHead);
}

FORCEINLINE
void CxPlatListInsertHead(_Inout_ CXPLAT_LIST_ENTRY* ListHead, _Out_ __drv_aliasesMem CXPLAT_LIST_ENTRY* Entry) {
...
FORCEINLINE
void CxPlatListInsertTail(_Inout_ CXPLAT_LIST_ENTRY* ListHead, _Inout_ __drv_aliasesMem CXPLAT_LIST_ENTRY* Entry) {
    QuicListEntryValidate(ListHead);
...
FORCEINLINE
void CxPlatListInsertAfter(_Inout_ CXPLAT_LIST_ENTRY* ListEntry, _Inout_ __drv_aliasesMem CXPLAT_LIST_ENTRY* NewEntry) {
...
FORCEINLINE
CXPLAT_LIST_ENTRY* CxPlatListRemoveHead(_Inout_ CXPLAT_LIST_ENTRY* ListHead) {
    QuicListEntryValidate(ListHead);
...
FORCEINLINE
BOOLEAN
CxPlatListEntryRemove(_In_ CXPLAT_LIST_ENTRY* Entry) {
...
QUIC_INLINE
void CxPlatListMoveItems(_Inout_ CXPLAT_LIST_ENTRY* Source, _Inout_ CXPLAT_LIST_ENTRY* Destination) {
    //
...
CXPLAT_WORKER_POOL* CxPlatWorkerPoolCreate(
    _In_opt_ QUIC_GLOBAL_EXECUTION_CONFIG* Config, _In_ CXPLAT_WORKER_POOL_REF RefType);

_Success_(return != NULL) CXPLAT_WORKER_POOL* CxPlatWorkerPoolCreateExternal(_In_ uint32_t Count,
    _In_reads_(Count) QUIC_EXECUTION_CONFIG* Configs,
    _Out_writes_(Count) QUIC_EXECUTION** Executions);

void CxPlatWorkerPoolDelete(_In_opt_ CXPLAT_WORKER_POOL* WorkerPool, _In_ CXPLAT_WORKER_POOL_REF RefType);

uint32_t CxPlatWorkerPoolGetCount(_In_ CXPLAT_WORKER_POOL* WorkerPool);

BOOLEAN
CxPlatWorkerPoolAddRef(_In_ CXPLAT_WORKER_POOL* WorkerPool, _In_ CXPLAT_WORKER_POOL_REF RefType);

void CxPlatWorkerPoolRelease(_In_ CXPLAT_WORKER_POOL* WorkerPool, _In_ CXPLAT_WORKER_POOL_REF RefType);

uint32_t CxPlatWorkerPoolGetIdealProcessor(_In_ CXPLAT_WORKER_POOL* WorkerPool,
    _In_ uint32_t Index // Into the worker pool
);

CXPLAT_EVENTQ* CxPlatWorkerPoolGetEventQ(_In_ CXPLAT_WORKER_POOL* WorkerPool,
    _In_ uint16_t Index // Into the worker pool
);

void CxPlatWorkerPoolAddExecutionContext(_In_ CXPLAT_WORKER_POOL* WorkerPool,
    _Inout_ CXPLAT_EXECUTION_CONTEXT* Context,
    _In_ uint16_t Index // Into the worker pool
);

uint32_t CxPlatWorkerPoolWorkerPoll(_In_ QUIC_EXECUTION* Execution);
...
void CxPlatAddDynamicPoolAllocator(_In_ CXPLAT_WORKER_POOL* WorkerPool,
    _Inout_ CXPLAT_POOL_EX* Pool,
    _In_ uint16_t Index // Into the execution config processor array
);

void CxPlatRemoveDynamicPoolAllocator(_Inout_ CXPLAT_POOL_EX* Pool);
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    BOOLEAN (*CXPLAT_EXECUTION_FN)(_Inout_ void* Context, _Inout_ CXPLAT_EXECUTION_STATE* State);
...
#endif
#else  // _KERNEL_MODE
void CxPlatWakeExecutionContext(_In_ CXPLAT_EXECUTION_CONTEXT* Context);

BOOLEAN
CxPlatWorkerIsThisThread(_In_ CXPLAT_EXECUTION_CONTEXT* Context);
#endif // _KERNEL_MODE
...
_IRQL_requires_max_(PASSIVE_LEVEL) const
    char* CxPlatGetSelfSignedCertCaCertificateFileName(_In_ BOOLEAN ClientCertificate);

_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_CREDENTIAL_CONFIG* CxPlatGetSelfSignedCert(
    _In_ CXPLAT_SELF_SIGN_CERT_TYPE Type, _In_ BOOLEAN ClientCertificate, _In_z_ const char* CaCertificateFile);

_IRQL_requires_max_(PASSIVE_LEVEL) _Success_(return == TRUE) BOOLEAN
    CxPlatGetTestCertificate(_In_ CXPLAT_TEST_CERT_TYPE Type,
        ...
        _When_(CredType == QUIC_CREDENTIAL_TYPE_CERTIFICATE_HASH, _Out_)
            _When_(CredType != QUIC_CREDENTIAL_TYPE_CERTIFICATE_HASH, _Reserved_) QUIC_CERTIFICATE_HASH* CertHash,
        ...
        _When_(CredType == QUIC_CREDENTIAL_TYPE_CERTIFICATE_FILE, _Out_)
            _When_(CredType != QUIC_CREDENTIAL_TYPE_CERTIFICATE_FILE, _Reserved_) QUIC_CERTIFICATE_FILE* CertFile,
        ...
        _When_(CredType == QUIC_CREDENTIAL_TYPE_CERTIFICATE_PKCS12, _Out_)
            _When_(CredType != QUIC_CREDENTIAL_TYPE_CERTIFICATE_PKCS12, _Reserved_) QUIC_CERTIFICATE_PKCS12* Pkcs12,
        _When_(CredType == QUIC_CREDENTIAL_TYPE_NONE, _Out_z_bytecap_(100))
            _When_(CredType != QUIC_CREDENTIAL_TYPE_NONE, _Reserved_) char Principal[100]);

_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatFreeSelfSignedCert(_In_ const QUIC_CREDENTIAL_CONFIG* CredConfig);

_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatFreeSelfSignedCertCaFile(_In_z_ const char* CaFile);

_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatFreeTestCert(_In_ QUIC_CREDENTIAL_CONFIG* Params);
