...
_IRQL_requires_max_(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI ZwClose(_In_ HANDLE Handle);

_IRQL_requires_max_(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI ZwQueryInformationThread(_In_ HANDLE ThreadHandle,
...
#define CXPLAT_ALLOC_PAGED_UNINITIALIZED(Size, Tag)                                                                    \
    ExAllocatePool2(POOL_FLAG_PAGED | POOL_FLAG_UNINITIALIZED, Size, Tag)
#define CXPLAT_ALLOC_NONPAGED_UNINITIALIZED(Size, Tag)                                                                 \
...
//
CXPLAT_STATIC_ASSERT(NTDDI_VERSION == NTDDI_WIN10_FE, // 0x0A00000A
...
#define CxPlatPoolInitialize(IsPaged, Size, Tag, Pool)                                                                 \
    ExInitializeLookasideListEx(                                                                                       \
        Pool, NULL, NULL, (IsPaged) ? PagedPool : NonPagedPoolNx, 0, (Size) + sizeof(CXPLAT_POOL_HEADER), Tag, 1024)

#define CxPlatPoolUninitialize(Pool) ExDeleteLookasideListEx(Pool)
QUIC_INLINE
void* CxPlatPoolAlloc(_Inout_ CXPLAT_POOL* Pool) {
    CXPLAT_POOL_HEADER* Header = (CXPLAT_POOL_HEADER*)ExAllocateFromLookasideListEx(Pool);
    if (Header == NULL) {
        return NULL;
    }
    Header->Owner = Pool;
...
#define CxPlatEventWaitForever(Event) KeWaitForSingleObject(&(Event), Executive, KernelMode, FALSE, NULL)
QUIC_INLINE
NTSTATUS
_CxPlatEventWaitWithTimeout(_In_ CXPLAT_EVENT* Event, _In_ uint32_t TimeoutMs) {
...
#define CxPlatEventWaitWithTimeout(Event, TimeoutMs) (STATUS_SUCCESS == _CxPlatEventWaitWithTimeout(&Event, TimeoutMs))
...
QUIC_INLINE
BOOLEAN
_CxPlatEventQEnqueue(_In_ CXPLAT_EVENTQ* queue) {
...
QUIC_INLINE
void* CxPlatCqeUserData(_In_ const CXPLAT_CQE* cqe) {
    return *cqe;
}
...
uint64_t CxPlatTimeUs64ToPlat(uint64_t TimeUs) {
    ...
    return ((High / 1000000) << 32) + ((Low + ((High % 1000000) << 32)) / 1000000);
}
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_INLINE void CxPlatSleep(_In_ uint32_t DurationMs) {
...
#define CXPLAT_THREAD_CALLBACK(FuncName, CtxVarName)                                                                   \
    _Function_class_(KSTART_ROUTINE) _IRQL_requires_same_ void FuncName(_In_ void* CtxVarName)
...
CxPlatThreadCreate(_In_ CXPLAT_THREAD_CONFIG* Config, _Out_ CXPLAT_THREAD* Thread) {
    ...
    HANDLE ThreadHandle;
    Status =
        PsCreateSystemThread(&ThreadHandle, THREAD_ALL_ACCESS, NULL, NULL, NULL, Config->Callback, Config->Context);
    ...
    Status =
        ObReferenceObjectByHandle(ThreadHandle, THREAD_ALL_ACCESS, *PsThreadType, KernelMode, (void**)Thread, NULL);
    CXPLAT_DBG_ASSERT(QUIC_SUCCEEDED(Status));
    if (QUIC_FAILED(Status)) {
        *Thread = NULL;
        goto Cleanup;
    ...
    Status = KeGetProcessorNumberFromIndex(Config->IdealProcessor, &Processor);
    if (QUIC_FAILED(Status)) {
    ...
    if (Config->Flags & CXPLAT_THREAD_FLAG_SET_AFFINITIZE) {
        ...
        Status = ZwSetInformationThread(ThreadHandle, ThreadGroupInformation, &Affinity, sizeof(Affinity));
        CXPLAT_DBG_ASSERT(QUIC_SUCCEEDED(Status));
        if (QUIC_FAILED(Status)) {
            goto Cleanup;
        }
        ...
        Status = KeQueryLogicalProcessorRelationship(&Processor, RelationNumaNode, &Info, &InfoLength);
        CXPLAT_DBG_ASSERT(QUIC_SUCCEEDED(Status));
        if (QUIC_FAILED(Status)) {
            goto Cleanup;
        }
        Status = ZwSetInformationThread(
            ThreadHandle, ThreadGroupInformation, &Info.NumaNode.GroupMask, sizeof(GROUP_AFFINITY));
        CXPLAT_DBG_ASSERT(QUIC_SUCCEEDED(Status));
        if (QUIC_FAILED(Status)) {
            goto Cleanup;
        }
...
SetPriority:
    if (Config->Flags & CXPLAT_THREAD_FLAG_HIGH_PRIORITY) {
        KeSetBasePriorityThread((PKTHREAD)(*Thread), IO_NETWORK_INCREMENT + 1);
    }
    if (Config->Name) {
        DECLARE_UNICODE_STRING_SIZE(UnicodeName, 64);
        ULONG UnicodeNameLength = 0;
        Status = RtlUTF8ToUnicodeN(UnicodeName.Buffer,
...
#define ThreadNameInformation ((THREADINFOCLASS)38)
        Status = ZwSetInformationThread(ThreadHandle, ThreadNameInformation, &UnicodeName, sizeof(UNICODE_STRING));
        CXPLAT_DBG_ASSERT(QUIC_SUCCEEDED(Status));
...
#define CxPlatThreadWait(Thread) KeWaitForSingleObject(*(Thread), Executive, KernelMode, FALSE, NULL)
...
#define QuicProcessAddRef(Process)                                                                                     \
    if (Process != NULL) {                                                                                             \
        ObReferenceObjectWithTag(Process, QUIC_POOL_PROCESS);                                                          \
    }
#define QuicProcessRelease(Process)                                                                                    \
    if (Process != NULL) {                                                                                             \
        ObDereferenceObjectWithTag(Process, QUIC_POOL_PROCESS);                                                        \
    }
...
#define QuicSiloAddRef(Silo)                                                                                           \
    if (Silo != NULL) {                                                                                                \
        ObReferenceObjectWithTag(Silo, QUIC_POOL_SILO);                                                                \
    }
#define QuicSiloRelease(Silo)                                                                                          \
    if (Silo != NULL) {                                                                                                \
        ObDereferenceObjectWithTag(Silo, QUIC_POOL_SILO);                                                              \
    }
...
COMPARTMENT_ID
NdisGetThreadObjectCompartmentId(IN PETHREAD ThreadObject);

NTSTATUS
NdisSetThreadObjectCompartmentId(IN PETHREAD ThreadObject, IN NET_IF_COMPARTMENT_ID CompartmentId);

#define QuicCompartmentIdGetCurrent() NdisGetThreadObjectCompartmentId(PsGetCurrentThread())
#define QuicCompartmentIdSetCurrent(CompartmentId) NdisSetThreadObjectCompartmentId(PsGetCurrentThread(), CompartmentId)
