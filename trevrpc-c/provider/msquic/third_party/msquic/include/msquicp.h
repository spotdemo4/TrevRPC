...
typedef _IRQL_requires_max_(DISPATCH_LEVEL) void(QUIC_API* QUIC_TEST_DATAPATH_CREATE_HOOK)(
    _Inout_opt_ QUIC_ADDR* RemoteAddress, _Inout_opt_ QUIC_ADDR* LocalAddress);

typedef _IRQL_requires_max_(DISPATCH_LEVEL) void(QUIC_API* QUIC_TEST_DATAPATH_GET_ADDRESS_HOOK)(
...
typedef _IRQL_requires_max_(DISPATCH_LEVEL)
    BOOLEAN(QUIC_API* QUIC_TEST_DATAPATH_RECEIVE_HOOK)(_Inout_ CXPLAT_RECV_DATA* Datagram);

//
// Returns TRUE to drop the packet.
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL) BOOLEAN(QUIC_API* QUIC_TEST_DATAPATH_SEND_HOOK)(
    _Inout_ QUIC_ADDR* RemoteAddress, _Inout_opt_ QUIC_ADDR* LocalAddress, _Inout_ CXPLAT_SEND_DATA* SendData);
...
typedef struct QUIC_PRIVATE_TRANSPORT_PARAMETER {
    uint32_t Type;
    uint16_t Length;
    _Field_size_(Length) const uint8_t* Buffer;
