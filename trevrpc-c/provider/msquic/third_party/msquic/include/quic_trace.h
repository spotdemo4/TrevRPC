...
extern "C"
#endif
    typedef _Function_class_(QUIC_TRACE_RUNDOWN_CALLBACK)
        _IRQL_requires_max_(PASSIVE_LEVEL) void(QUIC_TRACE_RUNDOWN_CALLBACK)(void);
...
#if defined(QUIC_EVENTS_STDOUT) || defined(QUIC_LOGS_STDOUT) || defined(QUIC_EVENTS_STUB) || defined(QUIC_LOGS_STUB)
...
#define QuicTrace(Name, Fmt, ...) clog((Fmt " [" #Name ":%s:%d]\n"), ##__VA_ARGS__, __FILE__, __LINE__)
...
#endif
    casted_clog_bytearray(const uint8_t* const data, const size_t len, struct clog_param** head);
#else
QUIC_INLINE char*
#ifndef _WIN32
    __attribute__((no_instrument_function))
...
#define CASTED_CLOG_BYTEARRAY(Len, Data) casted_clog_bytearray((const uint8_t*)(Data), (Len), &__head)

#define CASTED_CLOG_BYTEARRAY16(Len, Data) casted_clog_bytearray((const uint8_t*)(Data), (Len), &__head)
...
extern "C"
#endif
    _IRQL_requires_max_(PASSIVE_LEVEL) _IRQL_requires_same_ void NTAPI QuicEtwCallback(_In_ LPCGUID SourceId,
...
#define QuicTraceLogStreamError(Name, X, Fmt, ...) QuicTrace(Name, Fmt, ##__VA_ARGS__)
#define QuicTraceLogStreamWarning(Name, X, Fmt, ...) QuicTrace(Name, Fmt, ##__VA_ARGS__)
#define QuicTraceLogStreamInfo(Name, X, Fmt, ...) QuicTrace(Name, Fmt, ##__VA_ARGS__)
#define QuicTraceLogStreamVerbose(Name, X, Fmt, ...) QuicTrace(Name, Fmt, ##__VA_ARGS__)
