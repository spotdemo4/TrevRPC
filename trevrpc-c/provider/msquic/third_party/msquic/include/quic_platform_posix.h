...
#define ALIGN_DOWN(length, type) ((unsigned long)(length) & ~(sizeof(type) - 1))

#define ALIGN_UP(length, type) (ALIGN_DOWN(((unsigned long)(length) + sizeof(type) - 1), type))

#ifndef ALIGN_DOWN_BY
#define ALIGN_DOWN_BY(Length, Alignment) ((uintptr_t)(Length) & ~((uintptr_t)(Alignment) - 1))
#endif

#ifndef ALIGN_UP_BY
#define ALIGN_UP_BY(Length, Alignment) (ALIGN_DOWN_BY(((uintptr_t)(Length) + (Alignment) - 1), Alignment))
#endif
...
QUIC_INLINE
long InterlockedAnd(_Inout_ _Interlocked_operand_ long volatile* Destination, _In_ long Value) {
    return __sync_and_and_fetch(Destination, Value);
}

QUIC_INLINE
long InterlockedOr(_Inout_ _Interlocked_operand_ long volatile* Destination, _In_ long Value) {
    return __sync_or_and_fetch(Destination, Value);
}

QUIC_INLINE
int64_t InterlockedOr64(_Inout_ _Interlocked_operand_ int64_t volatile* Destination, _In_ int64_t Value) {
    return __sync_fetch_and_or(Destination, Value);
}

QUIC_INLINE
int32_t InterlockedExchange32(_Inout_ _Interlocked_operand_ int32_t volatile* Target, _In_ int32_t Value) {
    return __sync_lock_test_and_set(Target, Value);
}

QUIC_INLINE
int64_t InterlockedExchange64(_Inout_ _Interlocked_operand_ int64_t volatile* Target, _In_ int64_t Value) {
    return __sync_lock_test_and_set(Target, Value);
}

QUIC_INLINE
int64_t InterlockedExchangeAdd64(_Inout_ _Interlocked_operand_ int64_t volatile* Addend, _In_ int64_t Value) {
    return __sync_fetch_and_add(Addend, Value);
}

QUIC_INLINE
short InterlockedCompareExchange16(
    _Inout_ _Interlocked_operand_ short volatile* Destination, _In_ short ExChange, _In_ short Comperand) {
    return __sync_val_compare_and_swap(Destination, Comperand, ExChange);
}

QUIC_INLINE
short InterlockedCompareExchange(
    _Inout_ _Interlocked_operand_ long volatile* Destination, _In_ long ExChange, _In_ long Comperand) {
    return __sync_val_compare_and_swap(Destination, Comperand, ExChange);
}

QUIC_INLINE
int64_t InterlockedCompareExchange64(
    _Inout_ _Interlocked_operand_ int64_t volatile* Destination, _In_ int64_t ExChange, _In_ int64_t Comperand) {
    return __sync_val_compare_and_swap(Destination, Comperand, ExChange);
}

QUIC_INLINE
BOOLEAN
InterlockedFetchAndClearBoolean(_Inout_ _Interlocked_operand_ BOOLEAN volatile* Target) {
    return __sync_fetch_and_and(Target, 0);
}

QUIC_INLINE
BOOLEAN
InterlockedFetchAndSetBoolean(_Inout_ _Interlocked_operand_ BOOLEAN volatile* Target) {
    return __sync_fetch_and_or(Target, 1);
}

QUIC_INLINE
void* InterlockedExchangePointer(_Inout_ _Interlocked_operand_ void* volatile* Target, _In_opt_ void* Value) {
    return __sync_lock_test_and_set(Target, Value);
}

QUIC_INLINE
void* InterlockedFetchAndClearPointer(_Inout_ _Interlocked_operand_ void* volatile* Target) {
    return __sync_fetch_and_and(Target, 0);
}

QUIC_INLINE
short InterlockedIncrement16(_Inout_ _Interlocked_operand_ short volatile* Addend) {
...
QUIC_INLINE
int64_t InterlockedIncrement64(_Inout_ _Interlocked_operand_ int64_t volatile* Addend) {
    return __sync_add_and_fetch(Addend, (int64_t)1);
}

QUIC_INLINE
int64_t InterlockedDecrement64(_Inout_ _Interlocked_operand_ int64_t volatile* Addend) {
...
__attribute__((noinline, noreturn)) void quic_bugcheck(_In_z_ const char* File, _In_ int Line, _In_z_ const char* Expr);

void CxPlatLogAssert(_In_z_ const char* File, _In_ int Line, _In_z_ const char* Expr);

#define CXPLAT_STATIC_ASSERT(X, Y) static_assert(X, #Y);
#define CXPLAT_ANALYSIS_ASSERT(X)
#define CXPLAT_ANALYSIS_ASSUME(X)
#define CXPLAT_FRE_ASSERT(exp)                                                                                         \
    ((exp) ? (void)0 : (CxPlatLogAssert(__FILE__, __LINE__, #exp), quic_bugcheck(__FILE__, __LINE__, #exp)));
...
_Ret_maybenull_ void* CxPlatAlloc(_In_ size_t ByteCount, _In_ uint32_t Tag);

_Ret_maybenull_ void* CxPlatAllocUninitialized(_In_ size_t ByteCount, _In_ uint32_t Tag);
...
#define CxPlatLockUninitialize(Lock) CXPLAT_FRE_ASSERT(pthread_mutex_destroy(&(Lock)->Mutex) == 0);

#define CxPlatLockAcquire(Lock) CXPLAT_FRE_ASSERT(pthread_mutex_lock(&(Lock)->Mutex) == 0);

#define CxPlatLockRelease(Lock) CXPLAT_FRE_ASSERT(pthread_mutex_unlock(&(Lock)->Mutex) == 0);
...
#define CxPlatRwLockInitialize(Lock) CXPLAT_FRE_ASSERT(pthread_rwlock_init(&(Lock)->RwLock, NULL) == 0);

#define CxPlatRwLockUninitialize(Lock) CXPLAT_FRE_ASSERT(pthread_rwlock_destroy(&(Lock)->RwLock) == 0);

#define CxPlatRwLockAcquireShared(Lock) CXPLAT_FRE_ASSERT(pthread_rwlock_rdlock(&(Lock)->RwLock) == 0);

#define CxPlatRwLockAcquireExclusive(Lock) CXPLAT_FRE_ASSERT(pthread_rwlock_wrlock(&(Lock)->RwLock) == 0);

#define CxPlatRwLockReleaseShared(Lock) CXPLAT_FRE_ASSERT(pthread_rwlock_unlock(&(Lock)->RwLock) == 0);

#define CxPlatRwLockReleaseExclusive(Lock) CXPLAT_FRE_ASSERT(pthread_rwlock_unlock(&(Lock)->RwLock) == 0);
...
FORCEINLINE
void CxPlatListPushEntry(_Inout_ CXPLAT_SLIST_ENTRY* ListHead, _Inout_ __drv_aliasesMem CXPLAT_SLIST_ENTRY* Entry);

FORCEINLINE
CXPLAT_SLIST_ENTRY* CxPlatListPopEntry(_Inout_ CXPLAT_SLIST_ENTRY* ListHead);
...
QUIC_INLINE
void CxPlatPoolInitialize(_In_ BOOLEAN IsPaged, _In_ uint32_t Size, _In_ uint32_t Tag, _Inout_ CXPLAT_POOL* Pool) {
...
QUIC_INLINE
void CxPlatPoolUninitialize(_Inout_ CXPLAT_POOL* Pool) {
...
QUIC_INLINE
void* CxPlatPoolAlloc(_Inout_ CXPLAT_POOL* Pool) {
...
QUIC_INLINE
void* CxPlatPoolAllocUninitialized(_Inout_ CXPLAT_POOL* Pool) {
    CxPlatLockAcquire(&Pool->Lock);
    CXPLAT_POOL_HEADER* Header =
#if DEBUG
        CxPlatGetAllocFailDenominator() ? NULL : // No pool when using simulated alloc failures
...
QUIC_INLINE
void CxPlatPoolFree(_In_ void* Memory) {
...
QUIC_INLINE
BOOLEAN
CxPlatPoolPrune(_Inout_ CXPLAT_POOL* Pool) {
    CxPlatLockAcquire(&Pool->Lock);
...
void CxPlatRefInitialize(_Inout_ CXPLAT_REF_COUNT* RefCount);

void CxPlatRefInitializeEx(_Inout_ CXPLAT_REF_COUNT* RefCount, _In_ uint32_t Initial);

void CxPlatRefInitializeMultiple(_Out_writes_(Count) CXPLAT_REF_COUNT* RefCounts, _In_ uint32_t Count);

void CxPlatRefIncrement(_Inout_ CXPLAT_REF_COUNT* RefCount);

BOOLEAN
CxPlatRefIncrementNonZero(_Inout_ volatile CXPLAT_REF_COUNT* RefCount, _In_ uint32_t Bias);
...
void CxPlatGetAbsoluteTime(_In_ unsigned long DeltaMs, _Out_ struct timespec* Time);
...
QUIC_INLINE
uint64_t CxPlatTimeDiff64(_In_ uint64_t T1, _In_ uint64_t T2) {
    //
...
QUIC_INLINE
uint32_t QUIC_NO_SANITIZE("unsigned-integer-overflow") CxPlatTimeDiff32(_In_ uint32_t T1, // First time measured
...
QUIC_INLINE
BOOLEAN
CxPlatTimeAtOrBefore64(_In_ uint64_t T1, _In_ uint64_t T2) {
    //
    // Assume no wrap around.
    //
...
CxPlatTimeAtOrBefore32(_In_ uint32_t T1, _In_ uint32_t T2) {
...
QUIC_INLINE
void CxPlatEventInitialize(_Out_ CXPLAT_EVENT* Event, _In_ BOOLEAN ManualReset, _In_ BOOLEAN InitialState) {
...
QUIC_INLINE
void CxPlatInternalEventUninitialize(_Inout_ CXPLAT_EVENT* Event) {
    int Result;
...
QUIC_INLINE
void CxPlatInternalEventSet(_Inout_ CXPLAT_EVENT* Event) {
    int Result;
...
QUIC_INLINE
void CxPlatInternalEventReset(_Inout_ CXPLAT_EVENT* Event) {
    int Result;
...
QUIC_INLINE
void CxPlatInternalEventWaitForever(_Inout_ CXPLAT_EVENT* Event) {
    int Result;
...
QUIC_INLINE
BOOLEAN
CxPlatInternalEventWaitWithTimeout(_Inout_ CXPLAT_EVENT* Event, _In_ uint32_t TimeoutMs) {
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(CXPLAT_EVENT_COMPLETION)(_In_ CXPLAT_CQE* Cqe);
typedef CXPLAT_EVENT_COMPLETION* CXPLAT_EVENT_COMPLETION_HANDLER;
#define CXPLAT_USE_EVENT_BATCH_COMPLETION
typedef _IRQL_requires_max_(PASSIVE_LEVEL) void(CXPLAT_EVENT_BATCH_COMPLETION)(
    _Inout_ CXPLAT_CQE** Cqes, _Inout_ uint32_t* Count);
...
QUIC_INLINE
BOOLEAN
CxPlatEventQInitialize(_Out_ CXPLAT_EVENTQ* Queue) {
...
QUIC_INLINE
void CxPlatEventQCleanup(_In_ CXPLAT_EVENTQ* Queue) {
    io_uring_queue_exit(&Queue->Ring);
}

QUIC_INLINE
struct io_uring_sqe* CxPlatEventGetSqe(_In_ CXPLAT_EVENTQ* Queue) {
...
QUIC_INLINE
void CxPlatEventQSubmit(_In_ CXPLAT_EVENTQ* Queue) {
#if DEBUG
    CxPlatLockAcquire(&Queue->Lock);
    CXPLAT_DBG_ASSERT(Queue->SqContentionCount++ == 0);
    CxPlatLockRelease(&Queue->Lock);
...
QUIC_INLINE
BOOLEAN
CxPlatEventQEnqueue(_In_ CXPLAT_EVENTQ* Queue, _In_ CXPLAT_SQE* Sqe) {
...
QUIC_INLINE
uint32_t CxPlatEventQDequeue(_In_ CXPLAT_EVENTQ* Queue,
...
QUIC_INLINE
void CxPlatEventQReturn(_In_ CXPLAT_EVENTQ* Queue, _In_ uint32_t Count) {
#if DEBUG
    CxPlatLockAcquire(&Queue->Lock);
    CXPLAT_DBG_ASSERT(Queue->CqContentionCount++ == 0);
    CxPlatLockRelease(&Queue->Lock);
...
QUIC_INLINE
CXPLAT_SQE* CxPlatCqeGetSqe(_In_ const CXPLAT_CQE* cqe) {
    return (CXPLAT_SQE*)(uintptr_t)(*cqe)->user_data;
}

QUIC_INLINE
_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatSqeClassicCompletion(_Inout_ CXPLAT_CQE** Cqes, _Inout_ uint32_t* Count) {
...
QUIC_INLINE
BOOLEAN
CxPlatSqeInitialize(_In_ CXPLAT_EVENTQ* queue, _In_ CXPLAT_EVENT_COMPLETION completion, _Out_ CXPLAT_SQE* sqe) {
...
CxPlatBatchSqeInitialize(
    _In_ CXPLAT_EVENTQ* queue, _In_ CXPLAT_EVENT_BATCH_COMPLETION_HANDLER completion, _Out_ CXPLAT_SQE* sqe) {
    UNREFERENCED_PARAMETER(queue);
...
QUIC_INLINE
void CxPlatSqeCleanup(_In_ CXPLAT_EVENTQ* queue, _In_ CXPLAT_SQE* sqe) {
#if DEBUG
...
typedef void(CXPLAT_EVENT_COMPLETION)(_In_ CXPLAT_CQE* Cqe);
typedef CXPLAT_EVENT_COMPLETION* CXPLAT_EVENT_COMPLETION_HANDLER;
typedef struct CXPLAT_SQE {
    int fd;
...
CxPlatSqeInitialize(_In_ CXPLAT_EVENTQ* queue, _In_ CXPLAT_EVENT_COMPLETION completion, _Out_ CXPLAT_SQE* sqe) {
    ...
    sqe->Completion = completion;
    if ((sqe->fd = eventfd(0, EFD_CLOEXEC)) == -1)
        return FALSE;
...
CxPlatEventQEnqueue(_In_ CXPLAT_EVENTQ* queue, _In_ CXPLAT_SQE* sqe) {
    ...
    struct kevent event = {.ident = sqe->Handle,
        .filter = EVFILT_USER,
...
QUIC_INLINE
BOOLEAN
CxPlatEventQEnqueueEx(_In_ CXPLAT_EVENTQ* queue, _In_ CXPLAT_SQE* sqe, _In_ short filter, _In_ unsigned short flags) {
    struct kevent event = {
        .ident = sqe->Handle, .filter = filter, .flags = flags, .fflags = 0, .data = 0, .udata = sqe};
    return kevent(*queue, &event, 1, NULL, 0, NULL) == 0;
}
...
QUIC_INLINE
void CxPlatSqeInitializeEx(
    _In_ CXPLAT_EVENTQ* queue, _In_ uintptr_t handle, _In_ CXPLAT_EVENT_COMPLETION completion, _Out_ CXPLAT_SQE* sqe) {
    UNREFERENCED_PARAMETER(queue);
    sqe->Handle = handle;
    sqe->Completion = completion;
}
...
#define CXPLAT_THREAD_CALLBACK(FuncName, CtxVarName) void* FuncName(void* CtxVarName)
...
QUIC_STATUS
CxPlatThreadCreate(_In_ CXPLAT_THREAD_CONFIG* Config, _Out_ CXPLAT_THREAD* Thread);
...
QUIC_STATUS
CxPlatRandom(_In_ uint32_t BufferLen, _Out_writes_bytes_(BufferLen) void* Buffer);

//
// Tracing stuff.
//

void CxPlatConvertToMappedV6(_In_ const QUIC_ADDR* InAddr, _Out_ QUIC_ADDR* OutAddr);

void CxPlatConvertFromMappedV6(_In_ const QUIC_ADDR* InAddr, _Out_ QUIC_ADDR* OutAddr);
