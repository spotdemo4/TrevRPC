...
int trevrpc_client_call_unary(
    ...
    *out_response = NULL;
    ...
    uint8_t* frame = NULL;
    size_t frame_len = 0;
    err = trevrpc_wire_encode_request(service, method, body, body_len, client->max_frame_size, &frame, &frame_len);
    if (err == 0) {
...
static void trevrpc_handle_stream(trevrpc_server* server, trevrpc_msquic_stream* stream) {
    ...
    if (request.kind != TREVRPC_RPC_KIND_UNARY) {
        trevrpc_server_write_status(stream, server->max_frame_size, TREVRPC_STATUS_UNIMPLEMENTED, "C runtime currently handles unary RPCs");
        trevrpc_msquic_free(body);
        return;
    }
