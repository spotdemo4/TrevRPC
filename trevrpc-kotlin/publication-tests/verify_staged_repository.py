...
def verify_archive(
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        require(bool(names), f"empty archive {path}")
        ...
        for info in archive.infolist():
            require(
                info.date_time == (1980, 2, 1, 0, 0, 0),
                f"non-reproducible timestamp in {path}: {info.filename}",
            )
        if expected_major is not None or maximum_major is not None:
            classes = [name for name in names if name.endswith(".class")]
            require(bool(classes), f"no class files in {path}")
            for name in classes:
                data = archive.read(name)
                require(
                    data[:4] == b"\xca\xfe\xba\xbe", f"invalid class file {path}!{name}"
                )
                major = int.from_bytes(data[6:8], byteorder="big")
                if expected_major is not None:
                    require(
                        major == expected_major,
                        f"{path}!{name} has class major {major}, expected {expected_major}",
                    )
                if maximum_major is not None:
                    require(
                        major <= maximum_major,
                        f"{path}!{name} has class major {major}, maximum {maximum_major}",
                    )
    ...
    for dependency in root.findall("m:dependencies/m:dependency", POM_NAMESPACE):
        rows.append(
            (
                ...
                dependency.findtext(
                    "m:scope", default="compile", namespaces=POM_NAMESPACE
                ),
...
def verify_pom(path: pathlib.Path, module: str, version: str = VERSION) -> None:
    ...
    require(
        root.findtext("m:groupId", namespaces=POM_NAMESPACE) == GROUP,
        f"wrong group in {path}",
    )
    require(
        root.findtext("m:artifactId", namespaces=POM_NAMESPACE) == module,
        f"wrong artifact in {path}",
    )
    require(
        root.findtext("m:version", namespaces=POM_NAMESPACE) == version,
        f"wrong version in {path}",
    )
    require(
        root.findtext("m:url", namespaces=POM_NAMESPACE)
        == "https://trev.zip/llc/TrevRPC",
        f"wrong URL in {path}",
    )
    require(
        root.findtext("m:inceptionYear", namespaces=POM_NAMESPACE) == "2026",
        f"wrong inception year in {path}",
    )
    require(
        root.findtext("m:licenses/m:license/m:name", namespaces=POM_NAMESPACE)
        == "MIT License",
        f"missing MIT metadata in {path}",
    )
    require(
        root.findtext("m:developers/m:developer/m:id", namespaces=POM_NAMESPACE)
        == "trev",
        f"missing developer metadata in {path}",
    )
    require(
        root.findtext("m:scm/m:connection", namespaces=POM_NAMESPACE)
        == "scm:git:https://trev.zip/llc/TrevRPC.git",
        f"wrong SCM metadata in {path}",
    )
    ...
    if module == "core":
        ...
        require(
            (GROUP, "core", version, "compile", None) in rows,
            "Netty core must be compile scope",
        )
        require(
            any(
                row[1] == "netty-codec-classes-quic" and row[3] == "compile"
                for row in rows
            ),
            "Netty QUIC classes must be compile scope",
        )
        require(
            any(row[1] == "netty-codec-http3" and row[3] == "compile" for row in rows),
            "Netty HTTP/3 must be compile scope",
        )
        natives = [
            row for row in rows if row[0:2] == ("io.netty", "netty-codec-native-quic")
        ]
        require(
            {row[4] for row in natives} == NATIVE_CLASSIFIERS,
            "Netty POM must contain exactly five native classifiers",
        )
        require(
            all(row[3] == "runtime" for row in natives),
            "Netty native classifiers must be runtime scope",
        )
        require(
            all(row[4] is not None for row in natives),
            "Netty POM contains an unclassified native dependency",
        )
    elif module == "transport-cronet":
        require(
            (GROUP, "core", version, "compile", None) in rows,
            "Cronet core must be compile scope",
        )
        require(
            not any(row[0] == "org.chromium.net" for row in rows),
            "Cronet POM must not expose bundled API classes or choose a provider",
        )
        description = root.findtext("m:description", namespaces=POM_NAMESPACE) or ""
        require(
            "JVM 17" in description,
            f"Cronet POM has an untruthful platform description: {description!r}",
        )
        license_names = {
        ...
        require(
            "Chromium and built-in dependencies" in license_names,
            "Cronet POM omits bundled class licensing",
        )
    else:
        require(
            any(row[1] == "protobuf-java" and row[3] == "runtime" for row in rows),
            "generator protobuf must be runtime scope",
        )
...
def verify_module_metadata(path: pathlib.Path, module: str) -> None:
    ...
    library_variants = [
        ...
        if variant.get("name") in {"apiElements", "runtimeElements"}
    ]
    require(len(library_variants) == 2, f"missing library variants in {path}")
    for variant in library_variants:
        jvm_version = typing.cast(int, variant["attributes"]["org.gradle.jvm.version"])
        require(jvm_version == TARGET_JVM[module], f"wrong JVM attributes in {path}")
    ...
    if module == "transport-netty":
        ...
        runtime_deps = typing.cast(list[dict[str, typing.Any]], runtime["dependencies"])
        selectors: set[typing.Any] = set()
        for dependency in runtime_deps:
            if dependency.get("module") != "netty-codec-native-quic":
                continue
            compat = typing.cast(
                dict[str, typing.Any], dependency.get("thirdPartyCompatibility", {})
            )
            artifact_selector = typing.cast(
                dict[str, typing.Any], compat.get("artifactSelector", {})
            )
            selectors.add(artifact_selector.get("classifier"))
        require(
            selectors == NATIVE_CLASSIFIERS,
            "Gradle metadata must select all five Netty native classifiers",
        )
        require(
            None not in selectors,
            "Gradle metadata contains an unclassified Netty native",
        )
    elif module == "transport-cronet":
        require(
            not any(
                typing.cast(str, dependency.get("group")) == "org.chromium.net"
                for variant in library_variants
                for dependency in typing.cast(
                    list[dict[str, typing.Any]], variant.get("dependencies", [])
                )
            ),
...
def verify_generator_legal_metadata(
    ...
    require(
        names >= GENERATOR_LEGAL_ENTRIES,
        f"missing generator third-party legal metadata in {path}",
    )
    for name in GENERATOR_LEGAL_ENTRIES:
        require(
            archive.getinfo(name).file_size > 0,
            f"empty generator legal metadata {path}!{name}",
        )
    ...
    require(
        "Apache License"
        in archive.read("META-INF/third-party/kotlin-stdlib/LICENSE.txt").decode(
            "utf-8"
        ),
        f"wrong Kotlin license in {path}",
    )
    require(
        "NOTICE file"
        in archive.read("META-INF/third-party/kotlin-stdlib/NOTICE.txt").decode(
            "utf-8"
        ),
        f"wrong Kotlin notice in {path}",
    )
    require(
        "Copyright 2008 Google Inc."
        in archive.read("META-INF/third-party/protobuf-java/LICENSE.txt").decode(
            "utf-8"
        ),
        f"wrong protobuf license in {path}",
    )
...
def main() -> None:
    ...
    require(
        modules == set(MODULE_TARGETS),
        f"unexpected staged coordinates: {sorted(modules)}",
    )

    for module, class_major in MODULE_TARGETS.items():
        base = artifact_path(repository, module, "", version)
        main_jar = pathlib.Path(f"{base}.jar")
        sources_jar = artifact_path(repository, module, "-sources.jar", version)
        javadoc_jar = artifact_path(repository, module, "-javadoc.jar", version)
        ...
        if module == "transport-cronet":
            with zipfile.ZipFile(main_jar) as archive:
                names = set(archive.namelist())
                require(
                    "org/chromium/net/CronetEngine.class" in names,
                    "Cronet JAR does not bundle its public API classes",
                )
                require(
                    "META-INF/LICENSE.chromium-cronet" in names,
                    "Cronet JAR omits bundled class licensing",
                )
        ...
        if module == "protoc-gen-trevrpc-kotlin":
            with zipfile.ZipFile(main_jar) as archive:
                ...
                require(
                    "Main-Class: zip.trev.trevrpc.generator.MainKt" in manifest,
                    "thin generator has wrong Main-Class",
                )
            executable = artifact_path(repository, module, "-jdk21.jar", version)
            verify_archive(executable, maximum_major=65)
            with zipfile.ZipFile(executable) as archive:
                manifest = archive.read("META-INF/MANIFEST.MF").decode()
                require(
                    "Main-Class: zip.trev.trevrpc.generator.MainKt" in manifest,
                    "executable generator has wrong Main-Class",
                )
                verify_class_major(
            ...
            require(
                ...
                f"executable generator failed: {process.stderr.decode(errors='replace')}",
            )
            allowed_artifacts.add(executable.name)
        else:
            require(
                not list(version_directory.glob(f"{module}-{version}-jdk21.*")),
                f"unexpected jdk21 classifier for {module}",
            )
        ...
        require(
            published == allowed_artifacts,
            f"unexpected artifacts for {module}: {sorted(published - allowed_artifacts)}",
        )
        for asc_name in asc_files:
            ...
            require(
                asc_path.read_bytes().startswith(b"-----BEGIN PGP"),
                f"invalid PGP signature {asc_path}",
            )
