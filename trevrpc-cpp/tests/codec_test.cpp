...
int main() {
  ...
  assert(!invalid_server);
  invalid_server_config.keep_alive = std::chrono::milliseconds(0);
  invalid_server_config.initial_request_timeout = std::chrono::nanoseconds(-1);
  invalid_server = trevrpc::Server::listen(invalid_server_config);
  assert(!invalid_server);
  invalid_server_config.initial_request_timeout = std::chrono::nanoseconds(0);
  invalid_server_config.stream_idle_timeout = std::chrono::nanoseconds(-1);
  invalid_server = trevrpc::Server::listen(invalid_server_config);
  assert(!invalid_server);
  invalid_server_config.stream_idle_timeout = std::chrono::nanoseconds(0);
  invalid_server_config.max_stream_messages = -2;
  invalid_server = trevrpc::Server::listen(invalid_server_config);
  assert(!invalid_server);
  invalid_server_config.max_stream_messages = -1;
  invalid_server_config.max_stream_body_size = -2;
  invalid_server = trevrpc::Server::listen(invalid_server_config);
  assert(!invalid_server);
  invalid_server_config.max_stream_body_size = -1;
  invalid_server_config.worker_count = 0;
  invalid_server = trevrpc::Server::listen(invalid_server_config);
  assert(!invalid_server);
  invalid_server_config.worker_count = 1;
  invalid_server_config.worker_queue_capacity = 0;
  invalid_server = trevrpc::Server::listen(invalid_server_config);
  assert(!invalid_server);
