...
const CodeGeneratorResponse = pluginRoot.lookupType("google.protobuf.compiler.CodeGeneratorResponse");
...
function serviceDescriptor(file, service) {
  const fullName = file.package === "" || file.package == null ? service.name : `${file.package}.${service.name}`;
  const methods = {};
...
function mapEntryForField(field, mapEntries) {
  if (field.label !== FieldLabel.Repeated || field.type !== FieldType.Message || field.typeName == null) {
    return null;
  }
...
function oneofsToJson(message) {
  ...
  for (const [index, oneof] of (message.oneofDecl ?? []).entries()) {
    const fields = (message.field ?? []).filter((field) => field.oneofIndex === index).map(fieldJsonName);
    if (fields.length > 0) {
