  ...
  for (const option of parameter.split(",")) {
    ...
    switch (key) {
      ...
      case "file_suffix":
        ...
        break;
      case "outbound_zero_copy":
        if (value !== "true" && value !== "false") {
          throw new Error("trevrpc-js outbound_zero_copy must be true or false");
        }
        options.outboundZeroCopy = value === "true";
        break;
      default:
...
function generateFile(file, rootJson, options) {
  ...
  const serviceExports = services
    .map((service) => generateService(file, service, options))
    .join("\n");
...
function generateService(_file, service, options) {
