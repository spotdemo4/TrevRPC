...
export function generate(request) {
  try {
    ...
    for (const file of protoFiles) {
      ...
      const nodeFileName = outputFileName(file.name, `.node${options.fileSuffix}`);
      files.push({
        name: fileName,
      ...
      files.push({
        name: nodeFileName,
        content: generateNodeFile(file, fileName, options),
      });
      files.push({
        name: declarationFileName(nodeFileName),
        content: generateNodeDeclarationFile(file, fileName, typeNames, options),
      });
    }
...
export function parseOptions(parameter) {
  ...
  for (const option of parameter.split(",")) {
    ...
    switch (key) {
      case "runtime_import":
        ...
        break;
      case "runtime_type_import":
        options.runtimeTypeImport = value;
        break;
      case "node_runtime_import":
        options.nodeRuntimeImport = value;
        break;
      case "node_runtime_type_import":
        options.nodeRuntimeTypeImport = value;
        break;
      case "file_suffix":
  ...
  options.runtimeTypeImport = resolveTypeImport(
    "runtime_import",
    options.runtimeImport,
    options.runtimeTypeImport,
  );
  options.nodeRuntimeTypeImport = resolveTypeImport(
    "node_runtime_import",
    options.nodeRuntimeImport,
    options.nodeRuntimeTypeImport,
  );
  return options;
}

function resolveTypeImport(optionName, runtimeImport, typeImport) {
  if (typeImport != null) {
    return typeImport;
  }
  if (/^[A-Za-z][A-Za-z0-9+.-]*:/u.test(runtimeImport)) {
    throw new Error(
      `${optionName} ${JSON.stringify(runtimeImport)} requires an explicit ${optionName.replace("_import", "_type_import")}`,
    );
  }
...
function generateFile(file, rootJson, options) {
  ...
  const serviceExports = services.map((service) => generateService(service)).join("\n");
...
function generateDeclarationFile(protoFiles, file, typeNames, options) {
  ...
  return `// Code generated by protoc-gen-trevrpc-js. DO NOT EDIT.\n\nimport type { Long, Root } from "protobufjs";\n\nimport type { BidirectionalStreamingCall, CallOptions, ClientStreamingCall, ResponseAsyncIterable, RpcServiceDescriptor, Transport, UnaryResponse } from ${JSON.stringify(
    options.runtimeTypeImport,
  )};\n\nexport declare const root: Root;\n\n${sections}\n`;
}

function generateNodeFile(file, ordinaryFileName, options) {
  const services = (file.service ?? []).map((service) => serviceDescriptor(file, service));
  const serviceNames = services.map((service) => service.exportName).join(", ");
  const registrations = services.map(generateNodeServerRegistration).join("\n");
  return `// Code generated by protoc-gen-trevrpc-js. DO NOT EDIT.\n\nimport { registerTypedService } from ${JSON.stringify(
    options.nodeRuntimeImport,
  )};\nimport { ${serviceNames}, root } from ${JSON.stringify(localModuleSpecifier(ordinaryFileName))};\n\n${registrations}`;
}

function generateNodeDeclarationFile(file, ordinaryFileName, typeNames, options) {
  const services = (file.service ?? []).map((service) => serviceDescriptor(file, service));
  const messageNames = [...new Set(typeNames.values())].join(", ");
  const messageImport =
    messageNames === ""
      ? ""
      : `import type { ${messageNames} } from ${JSON.stringify(localModuleSpecifier(ordinaryFileName))};\n`;
  const declarations = services
    .map((service) => generateNodeServiceDeclaration(service, typeNames))
    .join("\n\n");
  return `// Code generated by protoc-gen-trevrpc-js. DO NOT EDIT.\n\nimport type { NodeServer, NodeServerCall, StreamingServerResponse, UnaryServerResponse } from ${JSON.stringify(
    options.nodeRuntimeTypeImport,
  )};\n${messageImport}\n${declarations}\n`;
}
...
function generateNodeServiceDeclaration(service, typeNames) {
...
function generateMethodDeclaration(jsName, method, typeNames) {
  ...
  switch (method.kind) {
    ...
    case "serverStreaming":
      return `  /** Calls the ${method.name} RPC. */\n  ${jsName}(request: ${inputType}, options?: CallOptions): Promise<ResponseAsyncIterable<${outputType}>>;`;
...
function generateHandlerDeclaration(jsName, method, typeNames) {
  const inputType = declarationTypeReference(method.inputType, typeNames);
  const outputType = declarationTypeReference(method.outputType, typeNames);
  const unaryResult = `${outputType} | UnaryServerResponse<${outputType}>`;
  const streamingResult = `Iterable<${outputType}> | AsyncIterable<${outputType}> | StreamingServerResponse<${outputType}>`;
  switch (method.kind) {
    case "unary":
      return `  ${jsName}?(request: ${inputType}, call: NodeServerCall): ${unaryResult} | Promise<${unaryResult}>;`;
    case "serverStreaming":
      return `  ${jsName}?(request: ${inputType}, call: NodeServerCall): ${streamingResult} | Promise<${streamingResult}>;`;
    case "clientStreaming":
      return `  ${jsName}?(requests: AsyncIterable<${inputType}>, call: NodeServerCall): ${unaryResult} | Promise<${unaryResult}>;`;
    case "bidirectionalStreaming":
      return `  ${jsName}?(requests: AsyncIterable<${inputType}>, call: NodeServerCall): ${streamingResult} | Promise<${streamingResult}>;`;
    default:
      throw new Error(`unsupported generated RPC kind ${JSON.stringify(method.kind)}`);
  }
}
...
function fieldTypeScriptType(field, mapEntries, typeNames, currentFullName) {
  ...
  if (mapEntry != null) {
    const [, valueField] = mapEntry.field ?? [];
...
function localModuleSpecifier(fileName) {
