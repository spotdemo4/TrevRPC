...
static void reject_native_error(napi_env env, napi_deferred deferred, int err,
                                const char *operation) {
  char message[256];
  const char *detail = err == TREV_NODE_ERR_CLOSED ? "native object is closed"
                                                   : trevrpc_error(err);
  snprintf(message, sizeof(message), "%s failed: %s", operation,
           detail != NULL ? detail : "native operation failed");
...
static bool get_bool_property(napi_env env, napi_value object, const char *name,
                              bool default_value) {
...
static uint32_t get_uint32_property(napi_env env, napi_value object,
...
static bool get_size_property(napi_env env, napi_value object, const char *name,
                              size_t *out_value) {
  bool has_property = false;
  napi_has_named_property(env, object, name, &has_property);
  if (!has_property) {
...
static char *copy_string_value(napi_env env, napi_value value) {
  ...
  if (napi_get_value_string_utf8(env, value, result, len + 1, &len) !=
      napi_ok) {
    free(result);
    return NULL;
  }
...
static char *get_string_property(napi_env env, napi_value object,
                                 const char *name) {
  bool has_property = false;
  napi_has_named_property(env, object, name, &has_property);
  if (!has_property) {
...
static int copy_bytes_arg(napi_env env, napi_value value, uint8_t **out,
                          size_t *out_len) {
  *out = NULL;
  *out_len = 0;
...
static napi_status set_uint32(napi_env env, napi_value object, const char *name,
                              uint32_t value) {
...
static napi_status set_string_bytes(napi_env env, napi_value object,
...
static napi_status set_bytes(napi_env env, napi_value object, const char *name,
                             const uint8_t *value, size_t len) {
  napi_value bytes = make_uint8_array(env, value, len);
  return napi_set_named_property(env, object, name, bytes);
}

static napi_value metadata_to_js(napi_env env,
...
static int metadata_from_js(napi_env env, napi_value value,
...
static napi_value request_to_js(napi_env env, const trevrpc_request *request) {
  ...
  set_string_bytes(env, object, "service", request->service,
                   request->service_len);
...
static int response_from_js(napi_env env, napi_value value,
                            trevrpc_response *response) {
  napi_valuetype type = napi_undefined;
  napi_typeof(env, value, &type);
  if (type == napi_undefined || type == napi_null) {
  ...
  response->status =
      get_uint32_property(env, value, "status", TREVRPC_STATUS_OK);
...
static napi_value response_to_js(napi_env env,
                                 const trevrpc_response *response) {
  napi_value object = NULL;
  napi_create_object(env, &object);
  set_uint32(env, object, "status",
             response != NULL ? response->status : TREVRPC_STATUS_UNKNOWN);
  set_string_bytes(env, object, "message",
                   response != NULL ? response->message : "",
                   response != NULL ? response->message_len : 0);
  set_bytes(env, object, "body", response != NULL ? response->body : NULL,
            response != NULL ? response->body_len : 0);
  napi_value metadata =
      metadata_to_js(env, response != NULL ? &response->metadata : NULL);
  napi_set_named_property(env, object, "metadata", metadata);
  return object;
}

static napi_value stream_frame_to_js(napi_env env,
                                     const trevrpc_stream_frame *frame) {
  napi_value object = NULL;
  napi_create_object(env, &object);
...
static void stream_frame_list_reset(trevrpc_stream_frame **frames,
                                    size_t frames_len) {
...
static int stream_frame_list_append(trevrpc_stream_frame ***frames,
                                    size_t *frames_len, size_t *frames_cap,
                                    trevrpc_stream_frame *frame) {
...
static int recv_many_from_stream(trevrpc_stream *stream, size_t max_frames,
                                 trevrpc_stream_frame ***frames,
                                 size_t *frames_len, size_t *frames_cap,
                                 bool *eof) {
...
static napi_value stream_frame_list_to_js(napi_env env,
                                          trevrpc_stream_frame **frames,
                                          size_t frames_len, bool eof) {
...
static bool recv_many_max_arg(napi_env env, size_t argc, napi_value *args,
                              size_t *max_frames) {
  *max_frames = TREV_NODE_RECV_MANY_DEFAULT;
  ...
  if (napi_get_value_uint32(env, args[0], &value) != napi_ok || value == 0) {
    napi_throw_type_error(env, NULL,
                          "recvMany requires a positive frame count");
    return false;
  }
  *max_frames =
      value > TREV_NODE_RECV_MANY_LIMIT ? TREV_NODE_RECV_MANY_LIMIT : value;
  return true;
}

static bool unwrap_native_client(napi_env env, napi_value receiver,
                                 native_client **out_client) {
  *out_client = NULL;
  if (napi_unwrap(env, receiver, (void **)out_client) != napi_ok ||
      *out_client == NULL) {
...
static bool unwrap_native_stream(napi_env env, napi_value receiver,
                                 native_stream **out_stream) {
  *out_stream = NULL;
  if (napi_unwrap(env, receiver, (void **)out_stream) != napi_ok ||
      *out_stream == NULL) {
...
static bool unwrap_native_server(napi_env env, napi_value receiver,
                                 native_server **out_server) {
  *out_server = NULL;
  if (napi_unwrap(env, receiver, (void **)out_server) != napi_ok ||
      *out_server == NULL) {
...
static bool unwrap_native_call(napi_env env, napi_value receiver,
                               native_call **out_call) {
  *out_call = NULL;
  if (napi_unwrap(env, receiver, (void **)out_call) != napi_ok ||
      *out_call == NULL) {
...
static bool create_receiver_ref(napi_env env, napi_value receiver,
                                napi_ref *out_ref) {
  *out_ref = NULL;
...
static int native_client_acquire(native_client *client,
...
static int native_stream_acquire(native_stream *stream,
...
static int native_call_acquire_completion(native_call *call,
                                          trevrpc_call **out_call) {
  if (call == NULL) {
    return TREV_NODE_ERR_CLOSED;
  }
  pthread_mutex_lock(&call->mutex);
...
static void native_call_release(native_call *call) {
  ...
  if (!call->js_alive && call->completing && call->refs == 0 &&
      call->call != NULL) {
...
static napi_value queue_work(napi_env env, base_work *work, const char *name,
  ...
  if (napi_create_async_work(env, NULL, resource_name, execute, complete, work,
                             &work->work) != napi_ok ||
...
static napi_value native_client_constructor(napi_env env,
  ...
  if (argc != 1 ||
      napi_get_value_external(env, args[0], (void **)&client) != napi_ok ||
      client == NULL) {
    napi_throw_type_error(env, NULL,
                          "native clients cannot be constructed directly");
    return NULL;
  }

  if (napi_wrap(env, this_arg, client, native_client_finalize, NULL, NULL) !=
      napi_ok) {
...
static napi_value native_stream_constructor(napi_env env,
                                            napi_callback_info info) {
  size_t argc = 1;
  napi_value args[1];
  napi_value this_arg = NULL;
  ...
  if (argc != 1 ||
      napi_get_value_external(env, args[0], (void **)&stream) != napi_ok ||
      stream == NULL) {
    napi_throw_type_error(env, NULL,
                          "native streams cannot be constructed directly");
    return NULL;
  }

  if (napi_wrap(env, this_arg, stream, native_stream_finalize, NULL, NULL) !=
      napi_ok) {
...
static napi_value native_server_constructor(napi_env env,
                                            napi_callback_info info) {
  size_t argc = 1;
  napi_value args[1];
  napi_value this_arg = NULL;
  ...
  if (argc != 1 ||
      napi_get_value_external(env, args[0], (void **)&server) != napi_ok ||
      server == NULL) {
    napi_throw_type_error(env, NULL,
                          "native servers cannot be constructed directly");
    return NULL;
  }

  if (napi_wrap(env, this_arg, server, native_server_finalize, NULL, NULL) !=
      napi_ok) {
...
static napi_value native_call_constructor(napi_env env,
                                          napi_callback_info info) {
  size_t argc = 1;
  napi_value args[1];
  napi_value this_arg = NULL;
  ...
  if (argc != 1 ||
      napi_get_value_external(env, args[0], (void **)&call) != napi_ok ||
      call == NULL) {
    napi_throw_type_error(env, NULL,
                          "native calls cannot be constructed directly");
    return NULL;
  }

  if (napi_wrap(env, this_arg, call, native_call_finalize, NULL, NULL) !=
      napi_ok) {
...
static void server_call_js(napi_env env, napi_value js_callback, void *context,
                           void *data) {
  (void)js_callback;
  (void)context;
  ...
  napi_status status =
      napi_get_reference_value(env, event->route->handler_ref, &handler);
  if (status == napi_ok) {
  ...
  if (status == napi_ok) {
    napi_value ignored = NULL;
    status =
        napi_call_function(env, global, handler, 1, &call_object, &ignored);
  }
  if (status != napi_ok) {
...
static void connect_execute(napi_env env, void *data) {
  ...
  if (work->max_streams_per_session > 0) {
    config.peer_bidi_stream_count =
        work->max_streams_per_session > UINT16_MAX
            ? UINT16_MAX
            : (uint16_t)work->max_streams_per_session;
  }
  ...
  work->base.err =
      trevrpc_client_connect(work->host, work->port, &config, &work->client);
}
...
static void connect_complete(napi_env env, napi_status status, void *data) {
  ...
  if (status != napi_ok) {
    ...
    reject_native_error(env, work->base.deferred, work->base.err,
                        "connectMsQuic");
  } else {
    ...
    if (client == NULL) {
      ...
      napi_status wrap_status =
          napi_get_reference_value(env, NativeClientConstructor, &ctor);
...
static napi_value connect_msquic(napi_env env, napi_callback_info info) {
  ...
  if (argc != 1) {
    napi_throw_type_error(env, NULL,
                          "connectMsQuic requires an options object");
    return NULL;
  }
  ...
  work->skip_certificate_validation =
      get_bool_property(env, args[0], "skipCertificateValidation", false) ? 1
                                                                          : 0;
  work->max_sessions_per_connection =
      get_uint32_property(env, args[0], "maxSessionsPerConnection", 0);
  work->max_streams_per_session =
      get_uint32_property(env, args[0], "maxStreamsPerSession", 0);
  ...
  return queue_work(env, &work->base, "connectMsQuic", connect_execute,
                    connect_complete);
}
...
static void listen_complete(napi_env env, napi_status status, void *data) {
  ...
  if (status != napi_ok) {
    ...
    if (server == NULL) {
      ...
      napi_value resource_name = NULL;
      napi_status wrap_status =
          napi_create_function(env, "serverCall", NAPI_AUTO_LENGTH,
                               noop_js_callback, NULL, &callback);
      if (wrap_status == napi_ok) {
        wrap_status = napi_create_string_utf8(env, "TrevRpcServerCall",
                                              NAPI_AUTO_LENGTH, &resource_name);
      }
      if (wrap_status == napi_ok) {
        wrap_status = napi_create_threadsafe_function(
            env, callback, NULL, resource_name, 0, 1, NULL, NULL, NULL,
            server_call_js, &server->call_tsfn);
      }
      ...
      if (wrap_status == napi_ok) {
        wrap_status =
            napi_get_reference_value(env, NativeServerConstructor, &ctor);
      }
      if (wrap_status == napi_ok) {
...
static napi_value listen_msquic(napi_env env, napi_callback_info info) {
  ...
  work->port = (uint16_t)get_uint32_property(env, args[0], "port", 0);
  work->max_sessions_per_connection =
      get_uint32_property(env, args[0], "maxSessionsPerConnection", 16);
  work->max_streams_per_session =
      get_uint32_property(env, args[0], "maxStreamsPerSession", 128);
  work->idle_timeout_ms =
      get_uint32_property(env, args[0], "idleTimeoutMs", 30000);
  get_size_property(env, args[0], "maxFrameSize", &work->max_frame_size);
  ...
  return queue_work(env, &work->base, "listenMsQuic", listen_execute,
                    listen_complete);
}

static napi_value native_server_register(napi_env env,
                                         napi_callback_info info) {
  size_t argc = 4;
  napi_value args[4];
  napi_value this_arg = NULL;
  ...
  if (argc != 4) {
    napi_throw_type_error(
        env, NULL, "register requires service, method, kind, and handler");
    return NULL;
  }
  ...
  if (err == 0 &&
      napi_get_value_uint32(env, args[2], &route->kind) != napi_ok) {
    err = -EINVAL;
  }
  ...
  if (err == 0 &&
      napi_create_reference(env, args[3], 1, &route->handler_ref) != napi_ok) {
    err = -ENOMEM;
  }
  if (err == 0) {
    route->server = server;
    ...
    pthread_mutex_unlock(&server->mutex);
    err = closing
              ? TREV_NODE_ERR_CLOSED
              : trevrpc_server_register_call(c_server, route->service,
                                             route->method, route->kind,
                                             native_server_call_handler, route);
  }
  if (err != 0) {
    if (route->handler_ref != NULL) {
      napi_delete_reference(env, route->handler_ref);
...
static void serve_execute(napi_env env, void *data) {
  ...
  pthread_mutex_unlock(&server->mutex);
  work->base.err =
      c_server == NULL ? TREV_NODE_ERR_CLOSED : trevrpc_server_serve(c_server);
}
...
static napi_value native_server_serve(napi_env env, napi_callback_info info) {
  ...
  pthread_mutex_lock(&server->mutex);
  bool can_start =
      !server->closing && !server->serving && server->server != NULL;
  if (can_start) {
    server->serving = true;
  }
  pthread_mutex_unlock(&server->mutex);
...
static void call_execute(napi_env env, void *data) {
  ...
  work->acquired = true;
  work->base.err =
      trevrpc_client_call_unary(client, work->service, work->method, work->body,
                                work->body_len, &work->response);
}
...
static napi_value native_client_start_stream(napi_env env,
  ...
  if (argc != 4) {
    napi_throw_type_error(
        env, NULL, "startStream requires service, method, kind, and body");
    return NULL;
  }
...
static napi_value native_stream_recv(napi_env env, napi_callback_info info) {
  ...
  return queue_work(env, &work->base, "recv", stream_recv_execute,
...
static void stream_recv_many_execute(napi_env env, void *data) {
  ...
  work->acquired = true;
  work->base.err =
      recv_many_from_stream(stream, work->max_frames, &work->frames,
                            &work->frames_len, &work->frames_cap, &work->eof);
}

static void stream_recv_many_complete(napi_env env, napi_status status,
                                      void *data) {
  stream_recv_many_work *work = data;
  if (work->base.err == 0 && status == napi_ok) {
    napi_value frames =
        stream_frame_list_to_js(env, work->frames, work->frames_len, work->eof);
...
static napi_value native_stream_recv_many(napi_env env,
                                          napi_callback_info info) {
  size_t argc = 1;
  napi_value args[1];
  napi_value this_arg = NULL;
  ...
  if (!recv_many_max_arg(env, argc, args, &work->max_frames) ||
      !unwrap_native_stream(env, this_arg, &work->stream) ||
      !create_receiver_ref(env, this_arg, &work->base.receiver_ref)) {
    free(work);
    return NULL;
  }
  return queue_work(env, &work->base, "recvMany", stream_recv_many_execute,
...
static void call_respond_complete(napi_env env, napi_status status,
                                  void *data) {
  call_response_work *work = data;
...
static napi_value native_call_respond(napi_env env, napi_callback_info info) {
  ...
  return queue_work(env, &work->base, "respond", call_respond_execute,
...
static void call_finish_execute(napi_env env, void *data) {
  ...
  work->acquired = true;
  work->base.err = trevrpc_call_finish_stream(call, work->status, work->message,
                                              work->message_len);
  if (work->base.err != 0) {
    trevrpc_call_close(call);
  }
...
static napi_value native_call_finish_stream(napi_env env,
                                            napi_callback_info info) {
  size_t argc = 2;
  napi_value args[2];
  napi_value this_arg = NULL;
  ...
  if (argc > 0 &&
      napi_get_value_uint32(env, args[0], &work->status) != napi_ok) {
    napi_delete_reference(env, work->base.receiver_ref);
    free(work);
  ...
  return queue_work(env, &work->base, "finishStream", call_finish_execute,
...
static void call_recv_execute(napi_env env, void *data) {
  ...
  work->base.err = stream == NULL ? TREVRPC_ERR_UNSUPPORTED_RPC_KIND
                                  : trevrpc_stream_recv(stream, &work->frame);
}
...
static napi_value native_call_recv(napi_env env, napi_callback_info info) {
  ...
  return queue_work(env, &work->base, "recv", call_recv_execute,
                    call_recv_complete);
}
...
static void call_recv_many_execute(napi_env env, void *data) {
  ...
  work->base.err = stream == NULL
                       ...
                       : recv_many_from_stream(stream, work->max_frames,
                                               &work->frames, &work->frames_len,
                                               &work->frames_cap, &work->eof);
}

static void call_recv_many_complete(napi_env env, napi_status status,
                                    void *data) {
  call_recv_many_work *work = data;
  if (work->base.err == 0 && status == napi_ok) {
    napi_value frames =
...
static napi_value native_call_recv_many(napi_env env, napi_callback_info info) {
  ...
  if (!recv_many_max_arg(env, argc, args, &work->max_frames) ||
      !unwrap_native_call(env, this_arg, &work->call) ||
      !create_receiver_ref(env, this_arg, &work->base.receiver_ref)) {
    free(work);
    return NULL;
  }
  return queue_work(env, &work->base, "recvMany", call_recv_many_execute,
...
static napi_value init(napi_env env, napi_value exports) {
  napi_property_descriptor client_methods[] = {
      ...
      {"startStream", NULL, native_client_start_stream, NULL, NULL, NULL,
       napi_default, NULL},
      {"close", NULL, native_client_close, NULL, NULL, NULL, napi_default,
       NULL},
  };
  napi_value client_ctor = NULL;
  napi_define_class(env, "NativeClient", NAPI_AUTO_LENGTH,
                    native_client_constructor, NULL,
                    sizeof(client_methods) / sizeof(client_methods[0]),
                    client_methods, &client_ctor);
  napi_create_reference(env, client_ctor, 1, &NativeClientConstructor);

  napi_property_descriptor stream_methods[] = {
      {"sendMessage", NULL, native_stream_send_message, NULL, NULL, NULL,
       napi_default, NULL},
      {"finishSend", NULL, native_stream_finish_send, NULL, NULL, NULL,
       napi_default, NULL},
      {"recv", NULL, native_stream_recv, NULL, NULL, NULL, napi_default, NULL},
      {"recvMany", NULL, native_stream_recv_many, NULL, NULL, NULL,
       napi_default, NULL},
      {"close", NULL, native_stream_close, NULL, NULL, NULL, napi_default,
       NULL},
  };
  napi_value stream_ctor = NULL;
  napi_define_class(env, "NativeStream", NAPI_AUTO_LENGTH,
                    native_stream_constructor, NULL,
                    sizeof(stream_methods) / sizeof(stream_methods[0]),
                    stream_methods, &stream_ctor);
  napi_create_reference(env, stream_ctor, 1, &NativeStreamConstructor);

  napi_property_descriptor server_methods[] = {
      {"register", NULL, native_server_register, NULL, NULL, NULL, napi_default,
       NULL},
      {"serve", NULL, native_server_serve, NULL, NULL, NULL, napi_default,
       NULL},
      {"close", NULL, native_server_close, NULL, NULL, NULL, napi_default,
       NULL},
  };
  napi_value server_ctor = NULL;
  napi_define_class(env, "NativeServer", NAPI_AUTO_LENGTH,
                    native_server_constructor, NULL,
                    sizeof(server_methods) / sizeof(server_methods[0]),
                    server_methods, &server_ctor);
  napi_create_reference(env, server_ctor, 1, &NativeServerConstructor);

  napi_property_descriptor call_methods[] = {
      {"respond", NULL, native_call_respond, NULL, NULL, NULL, napi_default,
       NULL},
      {"sendMessage", NULL, native_call_send_message, NULL, NULL, NULL,
       napi_default, NULL},
      {"finishStream", NULL, native_call_finish_stream, NULL, NULL, NULL,
       napi_default, NULL},
      {"recv", NULL, native_call_recv, NULL, NULL, NULL, napi_default, NULL},
      {"recvMany", NULL, native_call_recv_many, NULL, NULL, NULL, napi_default,
       NULL},
      {"close", NULL, native_call_close, NULL, NULL, NULL, napi_default, NULL},
  };
  napi_value call_ctor = NULL;
  napi_define_class(
      env, "NativeCall", NAPI_AUTO_LENGTH, native_call_constructor, NULL,
      sizeof(call_methods) / sizeof(call_methods[0]), call_methods, &call_ctor);
  napi_create_reference(env, call_ctor, 1, &NativeCallConstructor);

  napi_property_descriptor exports_desc[] = {
      {"connectMsQuic", NULL, connect_msquic, NULL, NULL, NULL, napi_default,
       NULL},
      {"listenMsQuic", NULL, listen_msquic, NULL, NULL, NULL, napi_default,
       NULL},
  };
  napi_define_properties(env, exports,
                         sizeof(exports_desc) / sizeof(exports_desc[0]),
                         exports_desc);
  return exports;
}
